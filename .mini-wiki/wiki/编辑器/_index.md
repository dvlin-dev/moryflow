...
`编辑器` 领域收敛相关模块的职责边界、接口形态与集成路径，面向架构评审与跨模块协作场景。

## 领域模块清单

| 模块     | 文档                          | API                         | 说明                                                       |
| -------- | ----------------------------- | --------------------------- | ---------------------------------------------------------- |
| `tiptap` | [@moryflow/tiptap](tiptap.md) | [API](../api/tiptap-api.md) | 编辑器扩展集合与交互能力封装，支持 Moryflow 富文本工作流。 |

## 领域关系图

```mermaid
flowchart TB
  Domain[编辑器] --> Modules[领域模块]
  Modules --> Docs[深度文档]
