...
`embed` 对应路径 `packages/embed`，嵌入式渲染能力包，提供页面嵌入与宿主通信协议。
...
```mermaid
flowchart LR
  Upstream[上游调用方] --> Entry[embed: 入口层]
  Entry --> Core[核心实现]
...
export async function runModuleDemo() {
  ...
  return { module: 'embed', apiCount: keys.length };
}
```
...
```bash
pnpm --filter @moryflow/embed typecheck
```
