# @moryflow/agents-sandbox

## 模块定位

`agents-sandbox` 对应路径 `packages/agents-sandbox`，Agent 沙盒执行能力，隔离命令执行、文件系统访问与安全边界。
...
```mermaid
flowchart LR
  Upstream[上游调用方] --> Entry[agents-sandbox: 入口层]
  Entry --> Core[核心实现]
...
export async function runModuleDemo() {
  ...
  return { module: 'agents-sandbox', apiCount: keys.length };
}
```
...
pnpm --filter @moryflow/agents-sandbox test:watch
```
...
- [Wiki 首页](../../index.md)
- [领域索引](../_index.md)
- [API 参考](../../api/agents-sandbox-api.md)
