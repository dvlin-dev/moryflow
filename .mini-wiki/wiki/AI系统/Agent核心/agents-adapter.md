# @moryflow/agents-adapter

## 模块定位

`agents-adapter` 对应路径 `packages/agents-adapter`，统一多模型 SDK 适配层，提供 provider 级别的调用桥接与参数收敛。
...
```mermaid
flowchart LR
  Upstream[上游调用方] --> Entry[agents-adapter: 入口层]
  Entry --> Core[核心实现]
...
```ts
import * as ModuleApi from '@moryflow/agents-adapter';
...
export async function runModuleDemo() {
  ...
  return { module: 'agents-adapter', apiCount: keys.length };
}
```
...
```bash
pnpm --filter @moryflow/agents-adapter build
pnpm --filter @moryflow/agents-adapter typecheck
```
...
- [Wiki 首页](../../index.md)
- [领域索引](../_index.md)
- [API 参考](../../api/agents-adapter-api.md)
