...
`sync` 对应路径 `packages/sync`，跨端同步协议与同步引擎，负责变更收敛、冲突处理与状态持久化。
...
```ts
import * as ModuleApi from '@moryflow/sync';
...
export async function runModuleDemo() {
  ...
  return { module: 'sync', apiCount: keys.length };
}
```
...
```bash
pnpm --filter @moryflow/sync build
pnpm --filter @moryflow/sync clean
```
