...
`config` 对应路径 `packages/config`，共享构建与运行配置层，统一 lint、tsconfig 与工程脚本约定。
...
```mermaid
flowchart LR
  Upstream[上游调用方] --> Entry[config: 入口层]
  Entry --> Core[核心实现]
...
```ts
import * as ModuleApi from '@moryflow/config';
...
export async function runModuleDemo() {
  ...
  return { module: 'config', apiCount: keys.length };
}
```
...
```bash
pnpm --filter @moryflow/config build
pnpm --filter @moryflow/config typecheck
pnpm --filter @moryflow/config lint
```
