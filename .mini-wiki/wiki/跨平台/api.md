...
`api` 对应路径 `packages/api`，函数式 API 客户端集合，统一请求鉴权、错误模型与数据映射。
...
```ts
import * as ModuleApi from '@moryflow/api';
...
export async function runModuleDemo() {
  ...
  return { module: 'api', apiCount: keys.length };
}
```
...
pnpm --filter @moryflow/api clean
