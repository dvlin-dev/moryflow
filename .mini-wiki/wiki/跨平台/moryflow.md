...
`moryflow` 对应路径 `apps/moryflow`，Moryflow 核心产品矩阵，包含 server、pc、mobile、docs 与站点能力。
...
```mermaid
flowchart LR
  Upstream[上游调用方] --> Entry[moryflow: 入口层]
  Entry --> Core[核心实现]
...
export async function runModuleDemo() {
  ...
  return { module: 'moryflow', apiCount: keys.length };
}
```
...
```bash
pnpm --filter moryflow build
pnpm --filter moryflow typecheck
```
