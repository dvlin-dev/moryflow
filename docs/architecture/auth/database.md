...
- `anyhunt-postgres`：console/auth/agentsbox/memox 元数据（非向量）
- `anyhunt-redis`：队列/限流/缓存
- `anyhunt-vector-postgres`：向量与索引（pgvector，独立实例）

## Anyhunt Dev 数据模型（最小可用）
...
Memox（向量，落在 `anyhunt-vector-postgres`）：
