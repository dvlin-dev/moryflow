...
const knowledgeReadSchema = z.object({
  ...
  path: z.string().optional().describe('Relative file path, used only if documentId is unavailable'),
  offsetChars: z.number().int().min(0).optional().describe('Start reading from this character offset'),
...
- `apps/moryflow/pc/src/main/app/memory-ipc-handlers.ts`（新增 handler）
...
**变更 4.2** — `apps/moryflow/pc/src/main/app/memory-ipc-handlers.ts`
...
| PC Memory IPC Handlers | `apps/moryflow/pc/src/main/app/memory-ipc-handlers.ts` |
