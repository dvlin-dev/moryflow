...
date: 2026-01-14
scope: aiget-server, aiget-www, aiget-admin-www
status: in_progress
...
- 功能需求：`docs/products/aiget-dev/features/v2-intelligent-digest.md`
...
> 说明：后端 review 的主范围是 `apps/aiget/server/src/digest/**`，并包含 Digest 队列定义所在的 `apps/aiget/server/src/queue/queue.constants.ts`。
...
- `apps/aiget/server/src/digest/CLAUDE.md`：Digest 模块边界与 contract（强制同步）
- `apps/aiget/server/src/digest/__tests__/`：Digest 单元测试（回归 + contract 解析）
...
> 说明：`apps/aiget/admin/www` 是 `/api/v1/admin/digest/*` 的直接消费者。Phase 0 合并路由冲突时必须以其调用为准对齐返回结构与分页协议。

| #   | 文件路径                                                    | 职责                                 |
| --- | ----------------------------------------------------------- | ------------------------------------ |
| 1   | `apps/aiget/admin/www/src/lib/api-paths.ts`                 | Admin API 路径常量（含 Digest 路由） |
| 2   | `apps/aiget/admin/www/src/features/digest-topics/api.ts`    | Digest Topics 管理 API               |
| 3   | `apps/aiget/admin/www/src/features/digest-topics/hooks.ts`  | Digest Topics hooks                  |
| 4   | `apps/aiget/admin/www/src/features/digest-topics/index.ts`  | 导出                                 |
| 5   | `apps/aiget/admin/www/src/features/digest-topics/types.ts`  | 类型定义                             |
| 6   | `apps/aiget/admin/www/src/features/digest-reports/api.ts`   | Digest Reports 管理 API              |
| 7   | `apps/aiget/admin/www/src/features/digest-reports/hooks.ts` | Digest Reports hooks                 |
| 8   | `apps/aiget/admin/www/src/features/digest-reports/index.ts` | 导出                                 |
| 9   | `apps/aiget/admin/www/src/features/digest-reports/types.ts` | 类型定义                             |
...
| 对齐 Admin API 合同（返回结构 / 分页协议 / 字段语义） | 明确并记录最终 contract（以 `apps/aiget/admin/www` 调用为准） |
...
1. 删除旧实现（避免重复注册路由）：
   - `apps/aiget/server/src/admin/admin-digest.controller.ts`
   - `apps/aiget/server/src/admin/admin-digest.service.ts`
2. 更新 `AdminModule` 移除引用，并清理 `apps/aiget/server/src/admin/dto.ts` 中 Digest 相关 schema
3. 保留并扩展 `apps/aiget/server/src/digest/controllers/digest-admin.controller.ts` 作为唯一 Admin Digest Controller
...
> Contract 的消费者以 `apps/aiget/admin/www` 为准；Admin Topics/Reports 已按 page/limit 对齐。

### 5.2 其他已修复的高优问题（摘要）

- Email 投递补齐 `unsubscribeUrl`，并基于 `AIGET_WWW_URL` 生成 Inbox/Unsubscribe 链接
