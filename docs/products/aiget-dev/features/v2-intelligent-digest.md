...
[PROTOCOL]: 本文件变更需同步更新 `docs/products/aiget-dev/index.md`。
-->
...
> 最终以 `apps/aiget/server/prisma/main/schema.prisma` 为准
...
- `aiget.dev` 仅作为官网落地页，未充分利用

**目标**：

1. 统一登录入口到 `aiget.dev/login`
2. 将 `aiget.dev` 从官网升级为主产品（面向 C 端用户）
3. Console 回归开发者工具定位
4. 跨子域 Session 共享（Cookie domain: `.aiget.dev`）

**实施阶段**：

| 子阶段      | 内容                                                                                                                    |
| ----------- | ----------------------------------------------------------------------------------------------------------------------- |
| Phase 2.6.1 | www 新增 `/login`、`/register`、`/forgot-password`；配置 Cookie domain `.aiget.dev`；Console/Admin 移除登录页改为重定向 |
