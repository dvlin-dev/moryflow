...
<!--
[INPUT]: 面向 C 端用户的「随时订阅信息」产品；支持任意 cron + IANA timezone；全局共享内容池（跨用户）；去重主键 canonicalUrlHash；允许二次投递（默认冷却 7 天）；Web Inbox（网页）为默认入口；输出语言跟随 UI（UserPreference.uiLocale）；允许任意 RSS feedUrl（需 SSRF/风控）；**全文仅用于分析，不永久存储**（抓取后临时缓存 1 小时供重试/调试，分析完成后生成 AI 总结并丢弃全文；用户查看原文通过链接跳转原站）；支持 Public Topics（aiget.dev SEO 页面）与一键订阅；计费为"按 run 结算一次，但按本次 Fetchx 实际调用成本汇总，且命中缓存也收费"
...
1. **30 秒创建订阅**：输入主题（自然语言也可以）→ 选择频率与投递时间（timezone + cron，高级模式支持任意 cron）→ AI 自动筛选有价值的内容。
...
- **默认做"智能筛选"**：AI 自由判断哪些内容有价值，用户只需设置"最少返回多少条"（`minItems`，默认 5）和"搜索候选上限"（`searchLimit`，默认 60，最大 100）。
- **默认做“解释性”**：每条内容给出“为什么入选”（命中的兴趣词/来源信誉/新鲜度），让用户信任而不是黑盒。
- **默认做“无配置可用”**：提供订阅模板（AI/产品/投融资/编程语言/地区新闻等），用户不需要懂 query/engines。
- **全文仅用于分析，不永久存储**：抓取全文用于 AI 分析、评分、生成总结；分析完成后全文丢弃（仅临时缓存 1 小时供重试/调试）。Web Inbox（以及 Email/SEO 页面）展示「标题 + AI 总结（200-500 字）+ 阅读原文链接」，用户想看完整内容直接点链接去原站。这样可以：显著降低存储成本、规避版权风险、减少阅读负担。
...
- ✅ 允许：加/减关键词（正向/负向）、调 `minScore`、调 `minItems`、调 `searchLimit`、调 cron/timezone、调语言模式
...
| 术语                            | 含义                                                          | 面向谁           | 备注                                                                 |
| ------------------------------- | ------------------------------------------------------------- | ---------------- | -------------------------------------------------------------------- |
| `ContentItem`                   | 全局内容池中的一条内容（唯一键 `canonicalUrlHash`）           | 系统/开发        | 存储 metadata + AI 总结 + 评分，跨用户复用（不存全文）               |
...
interface DigestSubscription {
  ...
  // 选题控制
  searchLimit: number; // 搜索候选数量上限（1-100，默认 60）
  scrapeLimit: number; // 抓取全文数量上限（0-100，默认 20）；只对评分最高的前 N 条抓全文，其余用标题+描述
  minItems: number; // 最少返回数量/保底（1-30，默认 5）
...
> **全文不存储原则**：抓取的全文仅用于 AI 分析和评分，分析完成后丢弃。系统只存储 metadata + AI 总结 + 评分。用户想看原文通过链接跳转原站。
...
interface ContentItem {
  ...
  firstSeenAt: Date;
  lastSeenAt: Date;
  lastAnalyzedAt?: Date; // 最后一次 AI 分析时间
...
- **分析重试**：若 AI 分析失败，可在 1 小时内重试而无需重新抓取
- **调试排查**：便于问题排查时查看原始内容
- **成本控制**：避免短时间内重复抓取同一 URL

缓存 key 建议：`content:fulltext:{canonicalUrlHash}`

```typescript
// Redis 临时缓存结构（1 小时 TTL）
interface ContentFullTextCache {
  canonicalUrlHash: string;
...
interface DigestTopic {
  id: string;
  slug: string; // URL path，全局唯一，如 "ai-agents-daily"；格式：^[a-z0-9]+(-[a-z0-9]+)*$，3-50 字符
...
- **AI 总结**：生成 `ContentItem.aiSummary`（200-500 字，根据原文信息密度自适应）
...
6. 抓全文（只对 top `scrapeLimit` 条，默认 20）：
   - 抓取结果写入 **Redis 临时缓存**（TTL=1 小时），不存入 DB
   - 用于后续 AI 分析和评分
7. AI 分析与总结生成（全局缓存）：
   - 基于全文生成 `ContentItem.aiSummary`（200-500 字，根据信息密度自适应）
   - 生成 `aiTags/contentType`（可选）
   - 分析完成后全文即可丢弃（Redis TTL 到期自动清理）
...
3. 只对 top `scrapeLimit` 条抓全文（全文写入 Redis 临时缓存，TTL=1 小时）
4. `ContentItem.aiSummary` 缺失时做 AI 分析：基于全文生成 200-500 字总结（全局缓存到 DB）
...
- **内容字段**：`description/content:encoded` 可作为 `ContentItem.description`；若需要更高质量的 AI 总结，则抓全文分析后生成 `aiSummary`
...
3. 计算订阅私有相关性（Relevance）：
   ...
   - 若为 Topic Follow：可直接复用 `DigestTopicRunItem.scoreOverall/aiSummarySnapshot`，并仅生成"解释性 reason"（或直接沿用 topic 的 reason）
4. 合成总分并排序：
   - `scoreOverall = 0.5 * relevance + 0.3 * impact + 0.2 * quality`
...
6. 智能筛选（AI 自由判断 + 保底机制）：
   - 筛选所有 `scoreOverall >= minScore` 的内容
   - 如果筛选后数量 < `minItems`：按评分从高到低补足到 `minItems`
   - 生成 run 快照：写 `DigestRunItem`（title/url/summary 的 snapshot + 分数快照 + deliveredAt）
7. 可选：为缺失 AI 总结的内容生成 `ContentItem.aiSummary`（200-500 字，内容级缓存）
...
- `GET /api/v1/console/digests/inbox/items`
  ...
  - 返回建议包含：`runId`、`subscriptionId`、`deliveredAt`、`titleSnapshot`、`aiSummarySnapshot`、`urlSnapshot`、`scoreOverall`、`scoringReason`、`readAt`、`savedAt`、`notInterestedAt`
...
- Inbox 列表：
  - 每条内容展示：**标题 + AI 总结 + 「阅读原文 →」链接**
  - 支持 `unread/saved/notInterested` 快捷筛选，默认按 `deliveredAt` 倒序
  - 点击「阅读原文」跳转原站查看完整内容
...
> 本节定义 aiget.dev（C 端用户界面）和 console.aiget.dev（开发者控制台）的前端路由、布局与交互规范。

#### 6.7.1 界面架构划分

| 平台             | 域名              | 目标用户 | 核心功能                                 |
| ---------------- | ----------------- | -------- | ---------------------------------------- |
| **Aiget 官网**   | aiget.dev         | 终端用户 | Web Inbox、订阅管理、公开主题浏览、收藏  |
| **开发者控制台** | console.aiget.dev | 开发者   | API Key、Digest API 调用、用量统计、计费 |

**技术栈（基于现有实现）**：

| 平台                  | 框架                               | 渲染方式        | 认证                  |
| --------------------- | ---------------------------------- | --------------- | --------------------- |
| **aiget.dev**         | TanStack Start + React 19 + Vite 7 | SSR（SEO 友好） | Better Auth（需新增） |
| **console.aiget.dev** | React 19 + Vite + React Router     | SPA             | Better Auth（已有）   |

**现状与改造**：

当前 `apps/aiget/www` 是纯营销站（Fetchx/Memox 落地页 + Demo Playground），无认证系统。Digest 功能需要：

1. **新增认证系统**：集成 Better Auth（与 console 共享认证服务 `server.aiget.dev/api/auth`）
2. **新增受保护路由**：`/inbox`、`/subscriptions`、`/favorites`、`/settings`
3. **保持公开路由**：`/`、`/fetchx`、`/memox`、`/topics`、`/topics/[slug]`（SEO 友好）

**两个平台的关系**：

- **完全独立的前端项目**：各自有独立的代码库（`apps/aiget/www` vs `apps/aiget/console`）
- **共享认证系统**：同一套 Better Auth，用户账号互通
- **共享 UI 组件**：都使用 `@aiget/ui`（shadcn/ui + Hugeicons）
- **共享后端 API**：都调用 `server.aiget.dev/api/v1`

**登录后跳转策略**：

根据用户来源决定跳转目标：

- 从 `aiget.dev` 登录 → 留在 `aiget.dev/inbox`
- 从 `console.aiget.dev` 登录 → 留在 `console.aiget.dev/`

**订阅管理功能边界**：

| 功能         | aiget.dev/subscriptions | console.aiget.dev/digest/subscriptions |
| ------------ | ----------------------- | -------------------------------------- |
| **目标用户** | 普通 C 端用户           | 开发者                                 |
| **创建方式** | 向导式（3 步骤）        | 表单式（一页完成）                     |
| **UI 风格**  | 友好、引导性强          | 紧凑、API 参数可见                     |
| **侧重点**   | 使用体验                | API 调试、集成                         |
| **代码示例** | 无                      | 有（curl/SDK）                         |

#### 6.7.1a 统一认证方案（跨域 Cookie 共享）

> 用户确认：登录/注册页统一在 aiget.dev 实现，可自由控制跳转到 aiget.dev 或 console.aiget.dev。

**最佳实践方案：Cookie Domain 共享**

由于 `aiget.dev` 和 `console.aiget.dev` 共享相同的顶级域名，可以通过设置 Cookie Domain 实现跨子域名的 Session 共享：
...
│                         (Better Auth 服务)                              │
│                                                                          │
│  Cookie: domain=.aiget.dev  ←─── 关键配置                               │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┴───────────────┐
                    ↓                               ↓
          ┌─────────────────┐             ┌─────────────────┐
          │   aiget.dev     │             │ console.aiget.dev│
          │  (统一登录页)    │             │   (开发者控制台)  │
          │                 │             │                  │
          │ /login          │             │ /                │
          │ /signup         │             │ /digest/*        │
...
**服务端配置（server.aiget.dev Better Auth）**：

```typescript
// apps/aiget/server/src/auth/better-auth.ts
export function createBetterAuth(prisma, sendOTP) {
  return betterAuth({
    // ... 现有配置 ...
    advanced: {
      // 跨子域名共享 Cookie
      cookiePrefix: 'aiget',
      // 注意：在 Better Auth 中可能需要通过 cookies 选项配置
    },
    // 或者通过 cookies 选项（取决于 Better Auth 版本）
    cookies: {
      sessionToken: {
        name: 'aiget.session',
        options: {
          domain: '.aiget.dev', // 关键：允许 aiget.dev 和 *.aiget.dev 共享
          path: '/',
          httpOnly: true,
          secure: true, // 生产环境必须
          sameSite: 'lax',
...
```
1. 用户访问 console.aiget.dev，点击登录
   ↓
2. 跳转到 aiget.dev/login?redirect=https://console.aiget.dev
   ↓
3. 用户在 aiget.dev 完成登录（调用 server.aiget.dev/api/auth）
   ↓
4. Better Auth 设置 Cookie（domain=.aiget.dev）
   ↓
5. 根据 redirect 参数跳转回 console.aiget.dev
   ↓
6. console.aiget.dev 自动读取共享的 Cookie，用户已登录
```
...
```typescript
// aiget.dev/login 页面
const ALLOWED_REDIRECT_HOSTS = ['aiget.dev', 'console.aiget.dev'];
...
aiget.dev/
├── /login                      # 统一登录页
│   └── ?redirect=xxx           # 可选：登录后跳转目标
├── /signup                     # 统一注册页
│   └── ?redirect=xxx           # 可选：注册后跳转目标
├── /auth/callback              # OAuth 回调（Google/Apple 登录）
└── /auth/verify                # 邮箱验证码验证页
```

**Console 登录按钮实现**：

```tsx
// console.aiget.dev 的 Header 组件
function LoginButton() {
  const currentUrl = window.location.href;
  const loginUrl = `https://aiget.dev/login?redirect=${encodeURIComponent(currentUrl)}`;
...
1. **单一登录入口**：UI 只需维护一份，体验一致
2. **无额外跳转**：Cookie 自动共享，用户无感知
3. **安全可控**：redirect 白名单防止 Open Redirect 漏洞
4. **易于维护**：认证逻辑集中在 aiget.dev

#### 6.7.1b SEO 优化（SSR + ISR）

> 用户确认：首页和公开主题页使用 SSR + ISR。

**TanStack Start ISR 配置**：

aiget.dev 使用 TanStack Start（基于 Nitro），支持 ISR 通过 `routeRules` 配置：

```typescript
// apps/aiget/www/app.config.ts
...
export default defineConfig({
  server: {
    routeRules: {
      ...
      // Fetchx/Memox 落地页：1 小时重新生成（内容稳定）
      '/fetchx': { isr: 3600 },
      '/memox': { isr: 3600 },

      // 受保护路由：不缓存（需要实时用户数据）
      '/inbox': { isr: false },
      '/subscriptions/**': { isr: false },
      '/favorites': { isr: false },
      '/settings/**': { isr: false },
    },
  },
...
```
1. 首次请求 /topics/ai-news
   ↓
2. SSR 渲染页面，缓存 HTML（TTL=300s）
   ↓
3. 300s 内的请求直接返回缓存
   ↓
4. 300s 后的首个请求：
...
```typescript
// routes/topics/index.tsx
export const Route = createFileRoute('/topics/')({
  loader: async () => {
    // SSR 时在服务端执行
    const topics = await fetch('https://server.aiget.dev/api/v1/public/digest-topics?sort=trending&limit=20');
    return { topics: await topics.json() };
  },
...
> 用户确认：Inbox Item 等组件在 aiget.dev 和 console 之间共享，抽取到 @aiget/ui。

**共享组件清单**：

| 组件                     | 用途           | 共享位置           |
| ------------------------ | -------------- | ------------------ |
| `DigestInboxItem`        | Inbox 内容卡片 | `@aiget/ui/digest` |
| `DigestSubscriptionCard` | 订阅卡片       | `@aiget/ui/digest` |
| `DigestTopicCard`        | 公开主题卡片   | `@aiget/ui/digest` |
| `DigestRunSummary`       | 运行摘要展示   | `@aiget/ui/digest` |
| `DigestCreateWizard`     | 创建订阅向导   | `@aiget/ui/digest` |
...
│   │   ├── ...existing components...
│   │   └── digest/                    # 🆕 Digest 专用组件
│   │       ├── inbox-item.tsx         # Inbox 内容卡片
│   │       ├── subscription-card.tsx  # 订阅卡片
│   │       ├── topic-card.tsx         # 公开主题卡片
│   │       ├── run-summary.tsx        # 运行摘要
│   │       ├── create-wizard/         # 创建订阅向导
│   │       │   ├── index.tsx
│   │       │   ├── step-basic.tsx
│   │       │   ├── step-sources.tsx
│   │       │   └── step-confirm.tsx
...
```tsx
// aiget.dev 或 console.aiget.dev
import { DigestInboxItem, DigestSubscriptionCard, DigestCreateWizard } from '@aiget/ui/digest';
    ...
    <div>
      {items.map((item) => (
        <DigestInboxItem
          ...
          summary={item.aiSummarySnapshot}
          source={item.source}
...
1. **数据驱动**：组件只接收展示数据，不直接调用 API
2. **回调解耦**：操作（收藏/不感兴趣）通过回调 props 传递
3. **样式一致**：使用 `@aiget/ui` 的设计 token，两端样式统一
4. **响应式内置**：组件内部处理移动端适配

#### 6.7.2 aiget.dev 路由设计（C 端用户界面）

**设计理念（参考 Notion）**：

- 极简克制：黑白灰为主，彩色只用于状态和强调
- 左侧导航：可折叠侧边栏，清晰的功能分区
- 内容优先：大面积留白，让内容呼吸
- 响应式：移动端侧边栏变底部 Tab Bar

**完整路由结构**：

```
aiget.dev/
├── /                           # 首页（Hero Banner + 公开主题列表）
├── /login                      # 登录
├── /signup                     # 注册
│
├── /inbox                      # 📥 Web Inbox（核心入口，登录后默认页）
│   └── ?source=xxx             # 按来源筛选
│   └── ?type=xxx               # 按类型筛选（news/tutorial/paper...）
│   └── ?subscriptionId=xxx     # 按订阅筛选
│   └── ?saved=true             # 仅收藏
│   └── ?unread=true            # 仅未读
│
├── /subscriptions              # 📋 我的订阅
...
│           └── /[runId]        # 单次运行详情
│
├── /favorites                  # ⭐ 收藏的内容（/inbox?saved=true 的快捷入口）
│
├── /topics                     # 🌐 公开主题发现页
│   └── /[slug]                 # 单个公开主题（SEO 友好，slug 全局唯一）
│       └── /editions           # 历史期刊列表
│           └── /[editionId]    # 单期内容
│
└── /settings                   # ⚙️ 设置
...
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│           Your intelligent content subscription                          │
│           ─────────────────────────────────────                         │
│           AI 帮你发现有价值的内容，每天自动送到 Inbox                      │
│                                                                          │
│                    [🔍 搜索主题或关键词...]                              │
│                                                                          │
│                    [开始订阅 →]                                          │
...
│   ────────────                                                           │
│   ┌──────────────────────────────────────────────────────────────────┐  │
│   │ OpenAI 发布 GPT-5                              TechCrunch · 2h   │  │
│   │ AI 总结：OpenAI 今日正式发布 GPT-5 模型...                        │  │
│   └──────────────────────────────────────────────────────────────────┘  │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
> 用户确认：不需要详情页，AI 总结足够，点击「阅读原文」直接跳转原站。
...
│  📥 Inbox │   Inbox                                        [筛选 ▾]     │
│           │   ──────────────────────────────────────────────────────     │
│  📋 订阅   │   [全部] [未读] [今日]      来源: [全部 ▾]  订阅: [全部 ▾]   │
│           │                                                               │
│  ⭐ 收藏   │   ┌────────────────────────────────────────────────────────┐ │
│           │   │ 📰 OpenAI 发布 GPT-5：多模态能力大幅提升                │ │
│  🌐 发现   │   │ TechCrunch · 2 小时前 · via AI 技术动态                │ │
│           │   ├────────────────────────────────────────────────────────┤ │
│  ─────    │   │ OpenAI 今日正式发布 GPT-5 模型，相比 GPT-4 在多模态    │ │
│           │   │ 理解、长上下文处理和推理能力方面均有显著提升。新模型    │ │
│  ⚙️ 设置  │   │ 支持原生图像、音频和视频输入，并能进行复杂的跨模态     │ │
│           │   │ 推理。发布会上展示的 demo 包括...                       │ │
│           │   ├────────────────────────────────────────────────────────┤ │
│           │   │ 阅读原文 →                          [⭐] [👎 不感兴趣]  │ │
│           │   └────────────────────────────────────────────────────────┘ │
│           │                                                               │
│           │   ┌────────────────────────────────────────────────────────┐ │
│           │   │ 📄 React 19 正式发布：并发特性成为默认                  │ │
│           │   │ React Blog · 5 小时前 · via 前端开发周报               │ │
│           │   ├────────────────────────────────────────────────────────┤ │
│           │   │ React 19 正式发布，标志着并发渲染成为默认行为...        │ │
│           │   └────────────────────────────────────────────────────────┘ │
│           │                                                               │
└──────────┴───────────────────────────────────────────────────────────────┘
```
...
│  │ 🤖 AI 技术动态              │  │ 📱 前端开发周报              │       │
│  │                             │  │                             │       │
│  │ 3 个来源 · 每日 8:00        │  │ 5 个来源 · 每周一 9:00      │       │
│  │ 最近运行: 2 小时前          │  │ 最近运行: 3 天前            │       │
│  │ 投递: 12 条 · 未读: 5       │  │ 投递: 8 条 · 未读: 2        │       │
│  │                             │  │                             │       │
│  │ [🌐 公开]  ● 运行中         │  │ [🔒 私密]  ● 运行中         │       │
│  └─────────────────────────────┘  └─────────────────────────────┘       │
│                                                                          │
...
关注的话题（AI 将基于此筛选和排序内容）
┌────────────────────────────────────────────────────────────────┐
│ 大语言模型、Agent、RAG、多模态、AI 产品                        │
└────────────────────────────────────────────────────────────────┘
...
预览订阅配置：
• 名称: AI 技术动态
• 话题: 大语言模型、Agent、RAG、多模态、AI 产品
• 频率: 每日 08:00 (Asia/Shanghai)
• 来源: AI 搜索（候选 60 条，抓取 20 条）
• 可见性: 公开
...
│  🤖 AI 技术动态                                                          │
│  ════════════════                                                        │
│  by @techuser · 1.2k 订阅者 · 每日更新                                  │
│                                                                          │
│  关注 AI 领域最新技术进展，包括大语言模型、Agent、                       │
│  多模态等方向。                                                          │
│                                                                          │
│  [+ 订阅此主题]    [分享]    [RSS]                                       │
│                                                                          │
...
│                                                                          │
│  ┌────────────────────────────────────────────────────────────────────┐  │
│  │ 📰 OpenAI 发布 GPT-5                      TechCrunch · 2h ago      │  │
│  ├────────────────────────────────────────────────────────────────────┤  │
│  │ OpenAI 今日正式发布 GPT-5 模型，相比 GPT-4 在多模态理解...         │  │
│  ├────────────────────────────────────────────────────────────────────┤  │
│  │ 阅读原文 →                                                         │  │
│  └────────────────────────────────────────────────────────────────────┘  │
│                                                                          │
...
#### 6.7.3 console.aiget.dev 路由设计（开发者控制台）

> 基于当前 console 已有设计，将 Digest 作为新模块加入。
...
├── Dashboard (/)
├── Playground (可折叠)
│   ├── Scrape (/playground/scrape)
│   ├── Crawl (/playground/crawl)
│   ├── Map (/playground/map)
│   ├── Extract (/playground/extract)
│   ├── Search (/playground/search)
│   └── Embed (/playground/embed)
└── Memox (可折叠)
    ├── Playground (/memox/playground)
    ├── Memories (/memox/memories)
    ├── Entities (/memox/entities)
    └── Graph (/memox/graph)
...
console.aiget.dev/
├── /digest
│   ├── /playground             # Digest API Playground（测试 API 调用）
│   ├── /subscriptions          # 订阅管理（API 视角）
│   │   ├── /[id]               # 订阅详情
│   │   └── /[id]/runs          # 运行历史
│   │       └── /[runId]        # 单次运行详情
│   └── /usage                  # Digest 用量统计（按 run 计费明细）
```

**Console Digest Playground 页面（/digest/playground）**：
...
│  Code Example                                                            │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │ curl -X POST https://server.aiget.dev/api/v1/digests \             │ │
│  │   -H "Authorization: Bearer ag_xxx" \                              │ │
│  │   -d '{"topic": "AI news", "cron": "0 8 * * *"}'                   │ │
│  └────────────────────────────────────────────────────────────────────┘ │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
**console.aiget.dev 移动端**：

- 侧边栏变为抽屉式（点击汉堡菜单展开）
- 已有实现，无需额外设计

#### 6.7.5 slug 唯一性约束（后端实现要求）

> 用户确认：公开主题 URL 使用 `/topics/[slug]` 格式，slug 全局唯一。
...
model DigestTopic {
  ...
  slug      String   @unique  // 全局唯一，用于 URL
  // ...
}
```
...
1. 用户创建时可自定义（必须校验格式与唯一性）
2. 格式要求：`^[a-z0-9]+(-[a-z0-9]+)*$`（小写字母、数字、连字符）
3. 长度限制：3-50 字符
4. 保留词：`new`, `edit`, `settings`, `admin`, `api` 等
5. 冲突处理：提示用户修改，不自动加后缀

**API 创建/更新时的校验**：

```typescript
// POST /api/v1/console/digest-topics
// PATCH /api/v1/console/digest-topics/:id
{
  slug: z.string()
    .min(3)
    .max(50)
...
1. run 开始时先做一次"预算检查"（基于 `searchLimit` 估算最大抓取成本，最大 100），不足则提前失败（避免跑一半才报额度不足）
2. run 结束后按实际发生的 Fetchx 调用数量与单价汇总 cost（见 16.1），并只扣一次（单 transaction）
3. `retry` 复用同一 `runId` 时：
   - 若上次已扣费：重试不再扣费（但要避免重复抓取/重复写入 Inbox）
   - 若上次未扣费：按重试实际成本扣一次

### 7.2 成本控制（默认公开 + 任意 cron 时，必须风控）

即使不对用户按条计费，也必须在系统层限制成本：

- `searchLimit` 强制上限：100（API 层面硬限制）
- `scrapeLimit` 抓全文上限：默认 20，只对评分最高的前 N 条抓全文，其余用标题+description
- 内容级缓存：`ContentItem.aiSummary` 生成一次后全局复用（不存全文，大幅降低存储成本）
- **全文临时缓存（1 小时）**：抓取的全文写入 Redis（TTL=1 小时），用于 AI 分析和重试；1 小时后自动清理，无长期存储压力
...
- [x] Prisma 模型：ContentItem（global）+ DigestSource + DigestSubscriptionSource + DigestSubscription + DigestRun + DigestRunItem + UserContentState
- [x] 订阅 CRUD + 手动 run + run 历史查询
- [x] Web Inbox API：条目列表（分页/筛选）+ 状态管理（已读/收藏/不感兴趣）
- [x] Console UI：Inbox 列表 + 条目详情 + 订阅管理（可用即可，不追求复杂交互）
- [x] 搜索型 source（ON_RUN）：每次 run 搜索并入池
- [x] canonicalize + canonicalUrlHash 去重
- [x] 二次投递策略：COOLDOWN（7 天）
...
> 所有模板使用默认参数：`searchLimit=60`，`scrapeLimit=20`，`minItems=5`，`minScore=70`。AI 自由判断返回数量。

- `AI Daily`：`0 9 * * *`，topic 示例：`AI agents, LLM, OpenAI, Anthropic, Google DeepMind`
- `Product & Startups (Weekly)`：`0 9 * * 1`，topic 示例：`startup, product strategy, YC, fundraising`
- `Engineering Deep Dives (Weekly)`：`0 9 * * 2`，topic 示例：`distributed systems, databases, system design`
- `Frontend (Weekly)`：`0 9 * * 3`，topic 示例：`React, TypeScript, web performance, Next.js`
- `Security (Daily)`：`0 9 * * *`，topic 示例：`CVE, security advisory, vulnerability`
...
1. **订阅配置**：topic/interests/cron/timezone/searchLimit/scrapeLimit/minItems/minScore/redeliveryPolicy 等
2. **Inbox 状态**：已读/收藏/不感兴趣（`UserContentState`）
3. **内容清单快照**：历史 `DigestRun` 与 `DigestRunItem`（尤其是 `aiSummarySnapshot/scoringReason/narrativeMarkdown`）
...
3. **内容保留策略**：永久保留 `DigestRun/DigestRunItem`；`ContentItem` 只存 AI 总结 + metadata，不存全文（全文临时缓存 1 小时后丢弃）
...
| 参数          | 说明                                            | 范围  | 默认值 |
| ------------- | ----------------------------------------------- | ----- | ------ |
| `searchLimit` | 搜索候选数量上限                                | 1-100 | 60     |
| `scrapeLimit` | 抓取全文数量上限（只对评分最高的前 N 条抓全文） | 0-100 | 20     |
| `minItems`    | 最少返回数量（保底）                            | 1-30  | 5      |
| `minScore`    | 评分阈值                                        | 0-100 | 70     |

工作流程：搜索 `searchLimit` 条 → 对 top `scrapeLimit` 条抓全文 → 筛选 `scoreOverall >= minScore` → 不足则补足到 `minItems` → 返回
...
Digest 在用户视角仍是"按 run 结算一次"，但计价更像"本次 run 实际做了多少抓取工作"。在你要求"缓存也收费"后，成本控制更依赖：限制 `searchLimit`（最大 100）、限制抓全文比例、RSS 优先、域名级并发与频率限制（见 7.2）。

折算示例（默认参数：`searchLimit=60`，`scrapeLimit=20`）：

- 典型费用：1 次 search + 20 次 scrape = **21 credits / run**
- FREE（100 credits/月）：约 **4-5 次 / 月**
- BASIC（5000 credits/月）：约 **238 次 / 月**
- PRO（20000 credits/月）：约 **952 次 / 月**
...
- **临时缓存**：抓取的全文写入 Redis（TTL = 1 小时）
- **分析后丢弃**：AI 生成 `aiSummary`（200-500 字）后，全文自动过期清理
- **用户查看原文**：通过链接跳转原站

优势：

- **存储成本大幅降低**：不需要 DB 或对象存储存放全文
- **版权风险规避**：不复制/存储原文内容
- **实现简化**：无需管理大文本字段、压缩、迁移等
...
1. **界面架构划分**：
   - `aiget.dev`：C 端用户界面（Web Inbox、订阅管理、公开主题浏览）
   - `console.aiget.dev`：开发者控制台（API Key、Digest API 调用、用量统计）
   - 两个平台**完全独立的前端项目**，但共享认证系统（Aiget Dev 业务线）

2. **技术栈**：
   - `aiget.dev`：TanStack Start + React 19 + Vite 7（SSR，SEO 友好）
   - `console.aiget.dev`：React 19 + Vite + React Router（SPA）
   - 共享 `@aiget/ui` 组件库和 Better Auth 认证

3. **首页策略**：两者结合（顶部 Hero Banner + 公开主题列表）
   - 未登录用户可浏览公开主题和最新内容
   - 引导用户注册/登录后进入 Inbox
...
5. **订阅管理功能边界**：
   - `aiget.dev/subscriptions`：面向普通用户，向导式创建，UI 友好
   - `console.aiget.dev/digest/subscriptions`：面向开发者，表单式创建，含代码示例

6. **创建订阅入口**：两边都有，但 UI 风格不同
   - aiget.dev：3 步骤向导（基本信息 → 来源配置 → 确认）
   - console：单页表单 + API 参数可见 + curl/SDK 代码示例

7. **Inbox 详情页策略**：不需要详情页
   - AI 总结（200-500 字）足够用户了解内容
   - 点击「阅读原文 →」直接跳转原站

8. **公开主题 URL 格式**：`/topics/[slug]`
   - slug 全局唯一，后端必须保证不可重复
   - 格式要求：`^[a-z0-9]+(-[a-z0-9]+)*$`，3-50 字符
   - 冲突处理：提示用户修改，不自动加后缀

9. **移动端适配**：需要考虑
   - aiget.dev：侧边栏变底部 Tab Bar（Inbox/订阅/收藏/发现/设置）
   - console.aiget.dev：侧边栏变抽屉式（已有实现）

10. **Console Digest 模块**：作为新模块加入现有导航
    - 路由：`/digest/playground`、`/digest/subscriptions`、`/digest/usage`
    - 与 Playground、Memox 平级

11. **统一登录页**：登录/注册页统一在 aiget.dev 实现
    - aiget.dev/login 和 aiget.dev/signup 是唯一入口
    - 通过 `?redirect=` 参数控制登录后跳转目标
    - 跨域认证通过 Cookie Domain (.aiget.dev) 共享实现
    - redirect 白名单：只允许 aiget.dev 和 console.aiget.dev

12. **SEO 优化**：首页和公开主题页使用 SSR + ISR
    - `/` 首页：ISR 60s
    - `/topics` 列表：ISR 120s
    - `/topics/**` 详情：ISR 300s
    - 受保护路由（/inbox, /subscriptions 等）：不缓存

13. **组件共享**：Digest 组件抽取到 @aiget/ui/digest
    - `DigestInboxItem`、`DigestSubscriptionCard`、`DigestTopicCard` 等
    - 两端共享，数据驱动，回调解耦，样式统一
...
| 评分     | 无                              | 三维评分（全局两维 + 订阅一维）                                         |
| 筛选方式 | 固定条数（Brief/Standard/Deep） | 智能筛选（AI 自由判断 + `minItems` 保底）                               |
...
## 附录：已实现文件清单（2026-01-13）

### Phase 1 MVP 实现文件

**Prisma Schema**：

- `packages/db/prisma/schema/digest.prisma` - Digest 数据模型

**后端服务（apps/aiget/server/src/digest/）**：

- `digest.module.ts` - 模块定义
- `digest.controller.ts` - API 控制器（API Key 鉴权）
- `digest-console.controller.ts` - Console 控制器（Session 鉴权）
- `digest-public.controller.ts` - 公开 API 控制器（无鉴权）
- `digest.service.ts` - 订阅管理服务
- `digest-inbox.service.ts` - 收件箱服务
- `digest-run.service.ts` - 运行执行服务
- `digest-topic.service.ts` - 话题管理服务
- `digest.scheduler.ts` - 定时调度器
- `digest.processor.ts` - BullMQ 队列处理器
- `dto/digest.schema.ts` - Zod Schema + DTO
- `scoring/digest-scorer.service.ts` - 三维评分服务
- `sources/digest-source.interface.ts` - Source 接口
- `sources/search-source.service.ts` - 搜索型 Source

**Console 前端（apps/aiget/console/src/）**：

- `lib/api-paths.ts` - 添加 Digest API 路径常量
- `features/digest/types.ts` - TypeScript 类型定义
- `features/digest/api.ts` - API 调用函数
- `features/digest/hooks.ts` - React Query Hooks
- `features/digest/components/subscription-list.tsx` - 订阅列表组件
- `features/digest/components/inbox-item-card.tsx` - 收件箱卡片组件
- `features/digest/components/create-subscription-dialog.tsx` - 创建订阅对话框
- `features/digest/components/index.ts` - 组件导出
- `features/digest/index.ts` - Feature 模块导出
- `pages/DigestSubscriptionsPage.tsx` - 订阅管理页面
- `pages/DigestInboxPage.tsx` - 收件箱页面
...
- `lib/digest-api.ts` - 公开话题 API 客户端
- `components/digest/topic-list-item.tsx` - 话题列表项组件
