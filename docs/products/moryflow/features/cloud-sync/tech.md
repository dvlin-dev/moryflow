...
title: 云同步系统（技术方案）
date: 2026-01-12
scope: moryflow, pc, server
status: active
---

<!--
[INPUT]: 多端文件同步（上传/下载/删除/冲突/软删除）；R2 存储；设备 lastSyncAt
[OUTPUT]: 可落地的技术方案与代码索引（Monorepo 路径）
[POS]: Moryflow 内部技术文档：云同步

[PROTOCOL]: 本文件涉及核心同步机制；变更时同步更新 `docs/products/moryflow/index.md`。
-->
...
| 模块         | 路径                                                            |
| ------------ | --------------------------------------------------------------- |
| 同步引擎入口 | `apps/moryflow/pc/src/main/cloud-sync/sync-engine/index.ts`     |
| 操作执行器   | `apps/moryflow/pc/src/main/cloud-sync/sync-engine/executor.ts`  |
| 防抖调度器   | `apps/moryflow/pc/src/main/cloud-sync/sync-engine/scheduler.ts` |
| 文件索引管理 | `apps/moryflow/pc/src/main/cloud-sync/file-index/index.ts`      |
| API 客户端   | `apps/moryflow/pc/src/main/cloud-sync/api/client.ts`            |
| 自动绑定     | `apps/moryflow/pc/src/main/cloud-sync/auto-binding.ts`          |
| 同步服务     | `apps/moryflow/server/src/sync/sync.service.ts`                 |
| 存储控制器   | `apps/moryflow/server/src/storage/storage.controller.ts`        |
