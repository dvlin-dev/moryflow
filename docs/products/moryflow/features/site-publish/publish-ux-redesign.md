...
<!--
[INPUT]: 对标 Notion Sites 的发布体验（Popover/渐进披露/先发布后配置）
[OUTPUT]: 设计思路与组件划分（用于追溯实现）
[POS]: 归档：站点发布 UX 方案（是否仍实现以当前产品为准）

[PROTOCOL]: 本文件为归档记录；如需复用，先确认现状再更新。
-->
...
| 模块       | 路径                                                               |
| ---------- | ------------------------------------------------------------------ |
| Share 组件 | `apps/moryflow/pc/src/renderer/components/share/`                  |
| Sites CMS  | `apps/moryflow/pc/src/renderer/workspace/components/sites/`        |
| 编辑器集成 | `apps/moryflow/pc/src/renderer/workspace/components/editor-panel/` |
