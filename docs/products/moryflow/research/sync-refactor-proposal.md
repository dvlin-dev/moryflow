...
<!--
[INPUT]: 现有基于 mtime/lastSyncAt 的同步；离线与多端并发修改
[OUTPUT]: 基于向量时钟的重构方案（用于评审与后续实现）
[POS]: Moryflow 云同步研究文档（与实现保持引用关系）

[PROTOCOL]: 本文件变更时同步更新 `apps/moryflow/server/src/sync/sync.service.ts` 的 `[DOC]` 引用与 `docs/products/moryflow/index.md`。
-->
...
```typescript
// packages/sync/src/vector-clock.ts
...
export class SyncEngine {
  ...
  private async doSync(): Promise<SyncResult> {
    ...
    const { dtos, pendingChanges } = detectLocalChanges(localFiles, this.fileIndex, this.deviceId);
    ...
    const executeResult = await executeActions(actions, this.vaultPath, this.deviceId);

    // 5. 准备 commit 数据
    const completedWithClock = this.mergeVectorClocks(executeResult.completed, pendingChanges);
    ...
    applyChangesToFileIndex(this.fileIndex, pendingChanges, executeResult, completedIds);
...
3. ✅ 类型检查通过 - @moryflow/api、server、pc client cloud-sync 模块均无类型错误

### Phase 5: 共享包集成 ✅ 完成

1. ✅ 统一使用 `@moryflow/sync` 作为向量时钟的单一来源
2. ✅ server 和 pc 添加 `@moryflow/sync` 依赖
3. ✅ `@moryflow/api` 从 `@moryflow/sync` 重导出 `VectorClock` 类型
