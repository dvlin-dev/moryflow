...
- `docs/products/anyhunt-dev/features/homepage-redesign.md`（Discover：Featured/Trending 的入口与 API）
- `docs/products/anyhunt-dev/features/homepage-reader-redesign.md`（Reader 三栏旧的默认路径）
- `docs/products/anyhunt-dev/features/www-reader-and-developer-split.md`（仍有参考价值，但其中的 Discover 导航已不再适用）
