...
结论：对 `apps/anyhunt/www` 的 Reader 模块，采用 **“按视图拆分（SRP） + ViewModel（判别联合）收敛 Props + 细粒度组合”** 的架构最符合最佳实践。
...
| 组件               | 文件                                                                   | 症状                                                                                 |
| ------------------ | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| `ReaderListPane`   | `apps/anyhunt/www/src/features/reader/components/ReaderListPane.tsx`   | `ReaderListPaneProps` 约 25 个字段，混合 3 个域                                      |
| `ReaderDetailPane` | `apps/anyhunt/www/src/features/reader/components/ReaderDetailPane.tsx` | `ReaderDetailPaneProps` 约 21 个字段，混合 4 个域（Welcome/Discover/Topics/Article） |
| `SidePanel`        | `apps/anyhunt/www/src/components/reader/SidePanel.tsx`                 | `SidePanelProps` 约 17 个字段，包含大量 actions                                      |

“可选但推荐”优化目标（不一定是 Props 长，但有明显 SRP/拆分收益）：

- `ReaderPage`：`apps/anyhunt/www/src/features/reader/ReaderPage.tsx`（大组件，编排逻辑仍偏集中）
- `HeroPlayground`：`apps/anyhunt/www/src/components/playground/HeroPlayground.tsx`（大组件，后续若继续压首包可拆 section/chunk）
- `Header`：`apps/anyhunt/www/src/components/layout/Header.tsx`（导航逻辑较多，可拆 Desktop/Mobile/NavItems）
