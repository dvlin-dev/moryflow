...
## PR #100 review 反馈修复（已完成）

### S2

- [done] 修复 vault 切换后 pending intent 残留：`useDocumentState` 在 vault 变化时重置 `pendingSelectionPath/pendingOpenPath`，避免跨 vault 树刷新触发旧意图
  - `apps/moryflow/pc/src/renderer/workspace/hooks/use-document-state.ts:1`
  - `apps/moryflow/pc/src/renderer/workspace/hooks/use-document-state.test.tsx:1`

- [done] 修复 provider-details 重构后 `thinking` 字段丢失：补齐 preset/custom model 在 `view/edit/save` 链路中的 `thinking` 透传
  - `apps/moryflow/pc/src/renderer/components/settings-dialog/components/providers/use-provider-details-controller.ts:1`
  - `apps/moryflow/pc/src/renderer/components/settings-dialog/components/providers/use-provider-details-controller.test.tsx:1`
...
| 2026-02-26 | PR #100 review 反馈                                 | 按评论修复（vault intent + thinking 透传）             | done   | 修复 vault 切换 pending intent 残留与 provider-details `thinking` 丢失；新增回归测试并通过 `pnpm --filter @moryflow/pc typecheck` + `pnpm --filter @moryflow/pc exec vitest run src/renderer/workspace/hooks/use-document-state.test.tsx src/renderer/components/settings-dialog/components/providers/use-provider-details-controller.test.tsx` |
