...
| P3       | packages/agents\*（Agent 平台能力）             | 工具协议/运行时/模型注册/沙盒/测试质量 | `packages/agents-adapter/`, `packages/agents-runtime/`, `packages/agents-tools/`, `packages/agents-mcp/`, `packages/agents-sandbox/`, `packages/agents-model-registry/`, `packages/model-registry-data/` | `docs/code-review/packages-agents.md`           | todo   |
