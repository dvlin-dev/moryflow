...
| -------- | ----------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- | ----------- |
| P0       | 工程基线（可安装性/CI 门禁/生成物）       | 确保“任何人/CI 都能装得起来并跑通 lint/typecheck/test”，同时明确 generated 规则（避免无意义 diff 与线上坑） | `package.json`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, `turbo.json`, `eslint.config.mjs`, `tsconfig.base.json`, `tsc-multi*.json`, `.github/workflows/ci.yml`（当前仅 `workflow_dispatch` 手动触发）, `scripts/` | `docs/code-review/root-tooling.md`                       | in_progress |
