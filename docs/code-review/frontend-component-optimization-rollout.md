...
| 26 | moryflow/admin | PR #99 review follow-up(2) | 修复评审意见（AlertDialogAction 自动关窗） | done | `pnpm --filter @moryflow/admin lint` + `typecheck` + `test:unit`（pass） | 2026-02-26 | `SiteActionConfirmDialog` 确认按钮由 `AlertDialogAction` 改为 `Button`，避免点击后立刻关窗；`SiteDetailPage` 改为 `mutateAsync` 成功后才关闭，失败时保留弹窗并展示错误 |
| Step | Project                | Module                                      | Action                                       | Status | Validation                                                                                                        | Updated At | Notes                                                                                                                                                                                                                                            |
| ---- | ---------------------- | ------------------------------------------- | -------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0    | all                    | 计划建档                                    | 创建执行计划与台账                           | done   | n/a                                                                                                               | 2026-02-25 | 本文档创建                                                                                                                                                                                                                                       |
| 1    | anyhunt/console        | 模块 A（api-keys/settings/webhooks）        | 预扫描（仅问题清单）                         | done   | n/a                                                                                                               | 2026-02-25 | 输出 `S1x3 / S2x2`，详见 `docs/code-review/anyhunt-console.md`                                                                                                                                                                                   |
...
| 12   | anyhunt/docs           | 项目范围调整                                | 忽略本项目（不纳入本轮专项）                 | done   | n/a                                                                                                               | 2026-02-26 | 按用户确认，`apps/anyhunt/docs` 从本轮组件优化专项中移除                                                                                                                                                                                         |
| 13   | moryflow/docs          | 项目范围调整                                | 忽略本项目（不纳入本轮专项）                 | done   | n/a                                                                                                               | 2026-02-26 | 按用户确认，`apps/moryflow/docs` 从本轮组件优化专项中移除                                                                                                                                                                                        |
| 14   | moryflow/site-template | 模块 A（layouts/templates）                 | 预扫描（仅问题清单）                         | done   | n/a                                                                                                               | 2026-02-26 | 输出 `S1x2 / S2x3 / S3x1`，详见 `docs/code-review/moryflow-site-template.md`                                                                                                                                                                     |
| 15   | moryflow/site-template | 模块 A（layouts/templates）                 | 分步重构与修复（A-1~A-5）                    | done   | `pnpm --filter @moryflow/site-template typecheck`（pass） + `build`（pass）+ `sync`（skip，避免跨目录生成物变更） | 2026-02-26 | 完成模板单一真源收敛（移除 `src/layouts`）、`head` 占位契约统一、模板片段化注入、`build` 契约校验接入，并同步 `apps/moryflow/site-template/CLAUDE.md`                                                                                            |
| 16   | moryflow/site-template | 模块 B（components/styles）                 | 预扫描（仅问题清单）                         | done   | n/a                                                                                                               | 2026-02-26 | 输出 `S1x2 / S2x2 / S3x1`，详见 `docs/code-review/moryflow-site-template.md`                                                                                                                                                                     |
| 17   | moryflow/site-template | 模块 B（components/styles）                 | 分步重构与修复（B-1~B-5）                    | done   | `pnpm --filter @moryflow/site-template typecheck`（pass） + `build`（pass）                                       | 2026-02-26 | 完成 `src/components` 孤岛层清理、`app.css` 分片重构（tokens/base/layout）、移动端遮罩 `pointer-events` 修复、`--toc-width` 清理，并同步 `apps/moryflow/site-template/CLAUDE.md`                                                                 |
| 18   | moryflow/site-template | 模块 C（scripts/生成逻辑）                  | 预扫描（仅问题清单）                         | done   | n/a                                                                                                               | 2026-02-26 | 输出 `S1x2 / S2x2 / S3x2`，详见 `docs/code-review/moryflow-site-template.md`                                                                                                                                                                     |
| 19   | moryflow/site-template | 模块 C（scripts/生成逻辑）                  | 分步重构与修复（C-1~C-5）                    | done   | `pnpm --filter @moryflow/site-template typecheck`（pass） + `build`（pass） + `sync`（定向输出，pass）            | 2026-02-26 | 完成主题脚本单一真源、`sync` 稳定排序与去时间戳、`dist` 新鲜度守卫、`build` import 显式失败策略、输出计数修正，并同步 `apps/moryflow/site-template/CLAUDE.md`                                                                                    |
| 20   | moryflow/site-template | 项目复盘（整项目一致性）                    | 全项目 Review（A/B/C）                       | done   | `pnpm --filter @moryflow/site-template typecheck`（pass） + `build`（pass） + `sync`（定向输出，双次哈希一致）    | 2026-02-26 | `site-template` 已完成模块 A/B/C 与项目收口，专项结项                                                                                                                                                                                            |
| 21   | moryflow/site-template | 分支全量 review 回合                        | 问题修复收口（R-2）                          | done   | `pnpm --filter @moryflow/site-template typecheck` + `test:unit` + `build` + `sync`（pass）                        | 2026-02-26 | 修复全量 review 发现项：PC 模板生成物漂移、`sync` mtime 守卫脆弱；补齐 build/sync 回归单测并同步 `site-template/CLAUDE.md`                                                                                                                       |
