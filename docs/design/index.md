...
- 正文只保留当前事实，不保留过程流水。
- `docs/plans/*` 中被采纳的稳定架构/协议/运维事实，必须优先并入已有 design/runbook；只有形成新的稳定边界时，才允许新建设计正文。
- `docs/design/*` 文件名必须表达主题或基线，流程状态统一放在 frontmatter `status`；正式正文不再保留 `*-plan.md` 这类过程态命名。
