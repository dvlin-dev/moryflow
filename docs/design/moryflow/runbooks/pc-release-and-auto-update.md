...
- 支持渠道：`stable`、`beta`。
- 当前发布平台：macOS Apple Silicon、macOS Intel。
- Windows x64 发布暂时下线，待代码签名与发布策略恢复后再重新纳入正式基线。
...
/channels/
  ...
  /beta/
    manifest.json
    /darwin/arm64/beta-mac.yml
    /darwin/x64/beta-mac.yml
```
...
```json
{
  ...
  "notesSummary": ["Faster workspace startup", "Improved cloud sync stability"],
...
- 推送 stable tag：`v*.*.*`
- 推送 beta tag：`v*.*.*-beta.*`
- 保留 `workflow_dispatch` 作为人工补发入口，但必须显式提供既有 tag，并基于该 tag 重放发布流程
- 只有当前 channel 的最新 tag 允许覆盖 `channels/<channel>` feed；历史 tag 补发只允许重传 GitHub Release 和版本归档，不允许回滚 live feed
...
5. 对每个打包后的 `.app` 执行 packaged-app smoke check：
   - 检查 `app.asar` 关键运行时依赖是否存在
   - 直接启动主进程二进制并确认在 smoke window 内未提前退出
6. 生成 GitHub Release（stable=release，beta=prerelease）
   - release notes 必须基于“上一个已发布 stable release”生成，不能基于上一个 tag；中间用于验证的补发 tag 不得截断 changelog
7. 上传全部安装包到 GitHub Release
8. 生成并上传 R2 版本归档目录
9. 生成并上传 channel `manifest.json`
10. 生成并上传各平台 `latest*.yml`
11. 对 `manifest.json` 与 `latest*.yml` 做缓存刷新或旁路缓存策略校验
12. 发布完成
...
1. 打开 GitHub Release 页面，确认本次版本为 prerelease，且同时包含 `arm64.dmg`、`x64.dmg`、对应 `zip` 与 `blockmap`
2. 打开 `https://download.moryflow.com/channels/beta/manifest.json`，确认：
   - `channel=beta`
   - `version` 与 GitHub Release tag 一致
   - `downloads.darwin-arm64` 与 `downloads.darwin-x64` 都存在
3. 打开：
   - `https://download.moryflow.com/channels/beta/darwin/arm64/beta-mac.yml`
   - `https://download.moryflow.com/channels/beta/darwin/x64/beta-mac.yml`
     确认 `version`、`path`、`sha512` 均存在，且 `path` 指向当前 beta 版本的 zip 资产
4. 在 Apple Silicon Mac 上执行全新安装，确认应用可正常打开，未出现签名、公证或 Gatekeeper 阻塞
5. 在 Intel Mac 上执行全新安装，确认应用可正常打开，未出现签名、公证或 Gatekeeper 阻塞
6. 在一台旧版本客户端切换到 `beta` 通道，手动执行 `Check for updates`
7. 确认左下角与 About 页都能看到当前 beta 版本可更新
8. 执行 `Download update`，确认状态按 `checking -> available -> downloading -> downloaded` 推进
9. 执行 `Restart to update`，确认重启后版本号升级到当前 beta 版本
10. 在 About 页验证 `Release notes` 与 `Download page` 都跳转到当前 GitHub Release 页面
...
2. 安装依赖并执行最小发布前校验
3. matrix 构建 `macOS arm64`、`macOS x64`
...
- Windows 构建配置保留在 `electron-builder.yml`，但当前 release workflow 不发布 Windows 包
- Linux 不进入正式发布矩阵
...
- macOS arm64
- macOS x64
- 当前默认只发布 macOS arm64 / x64；Windows 恢复发布时再显式加入 `--targets`
...
3. `latest-mac.yml` 同时覆盖 `darwin/arm64` 与 `darwin/x64`
   - beta 通道对应 `beta-mac.yml`
4. macOS 签名/公证完成
5. 左下角状态能完成 `check -> available -> downloading -> downloaded`
