...
5. 用户取消选中后，右侧 Chat 引用胶囊同步清空；发送后也默认清空该次选区引用（避免下一条误带上下文）。
...
5. 选区清空或变为无效时，同步清空右侧 Chat 引用，避免旧选区残留到后续提问。
...
2. 取消选中后引用胶囊同步消失。
3. 发送后 AI 回答体现对引用段落的处理，且下一条不自动携带旧引用。
4. Chat 折叠时选中触发展开（仅首次）。
5. 编辑器浮动工具栏不再出现 Improve 与其子菜单。
