...
当前 Hero 不再使用静态产品截图，而是桌面端专属的交互式 workspace demo：

- `apps/moryflow/www/src/components/landing/workspace-demo/*`

边界固定如下：

1. 只服务官网转化，不接真实后端、模型或联网搜索
2. 不复用 `apps/moryflow/pc` 的 runtime/store/controller
3. 通过本地 React state 驱动 sidebar、editor、chat 的壳层交互
4. 仅在桌面大屏断点启用交互 demo；非桌面端隐藏 demo
5. 用户可编辑文档与发送消息，但 follow-up 回复为固定官网话术

交互 demo 的目标不是“运行真实桌面端”，而是把 Moryflow 的核心心智在首屏可视化：

- Agent 会使用知识上下文工作
- 结果会沉淀到文档
- Workspace、文档与聊天是一个整体
