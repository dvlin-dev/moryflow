---
title: Moryflow PC Home Tab 独立 Remote Agents 模块方案（Telegram 右侧直出）
date: 2026-03-03
...
- 交互要求：Home 模块区里 `Remote Agents` 位于 `Skills` 上方；点击后右侧主内容区直接显示模块页面。
...
# Moryflow PC Home Tab 独立 Remote Agents 模块方案（Telegram 右侧直出）

## 1. 目标与结论（TL;DR）

目标：将 Telegram 配置从 Settings 内部分区升级为 Home 模块导航中的一级模块，当前名称为 `Remote Agents`，排序在 `Skills` 之前，点击后在右侧主内容区直接展示页面。

推荐结论：

1. 新增独立 destination（内部 key 与现有 `destination='agent'` 解耦），将 `Remote Agents` 作为模块导航项接入。
2. 右侧新增 `RemoteAgentsPage`，承载 Telegram 配置与 Pairing 审批（不再通过 Settings 弹窗承载主路径）。
...
1. Home 模块区顺序改为：`Remote Agents`、`Skills`、`Sites`。
2. 点击 `Remote Agents` 后右侧显示独立模块页面（非弹窗/非跳 settings）。
...
1. 扩展导航 destination，例如内部 key：`'remote-agents'`（用户显示名为 `Remote Agents`）。
2. `ModulesNav` 新增 `Remote Agents` 导航项（navigate 型）。
3. `WorkspaceShellMainContent` 新增页面分支，右侧渲染 `RemoteAgentsPage`。
...
### 方案 C：把 `Remote Agents` 做成打开新窗口/抽屉
...
1. Destination 扩展：
   ...
   - 到：`'agent' | 'remote-agents' | 'skills' | 'sites'`
2. 语义约束：
   - `agent`：原有编辑/会话工作区（Home/Chat）
   - `remote-agents`：Remote Agents 独立模块页（当前承载 Telegram）
...
2. `Sidebar` 组装条目顺序：
   - `Remote Agents` -> `go('remote-agents')`
   - `Skills` -> `go('skills')`
   - `Sites` -> `go('sites')`
...
1. `workspace-shell-main-content.tsx` 增加主视图状态 `remote-agents`。
2. 增加 keep-alive mount 标记（对齐 skills/sites 现有策略），避免频繁切换重建。
3. 新增 `RemoteAgentsPage`：
   - 页面标题：`Remote Agents`
   - 子标题可写明 `Manage remote entry points for your workspace.`
   - 主体渲染 Telegram 共享业务组件。

## 5.4 Telegram 模块页组件化与 Settings 入口下线（根因治理）

1. 将现有 `telegram-section.tsx` 迁移为模块页业务组件（保留表单/状态/pairing 单一实现），由 `RemoteAgentsPage` 直接渲染。
...
4. 保持“Telegram 配置主路径唯一”：Home Modules -> `Remote Agents` -> 右侧模块页。

## 5.5 文档与语义收敛

1. 更新旧导航文档中“Modules 仅非 Agent 模块”的约束，替换为新事实（含 `Remote Agents` 模块）。
2. 不保留“旧语义兼容注释”与双轨描述。
...
7. `apps/moryflow/pc/src/renderer/workspace/components/remote-agents/index.tsx`（新增）
...
1. 导航层：
   - `go('remote-agents')` 状态转换正确。
2. Sidebar：
   - 模块顺序严格为 `Remote Agents > Skills > Sites`。
   - 点击 `Remote Agents` 触发 `go('remote-agents')`。
3. 主内容路由：
   - `destination='remote-agents'` 时渲染 `RemoteAgentsPage`。
4. Telegram 模块页组件：
...
1. 风险：`Remote Agents` 模块与现有 `destination='agent'`（工作区）容易被误认为同层 agent 语义。
   - 规避：内部 key 使用 `remote-agents`，文案与工作区语义显式分层。
...
1. `Remote Agents` 模块当前固定对应 Telegram 独立模块页（非 custom agent 总览页）。
...
1. Navigation destination 扩展为 `remote-agents`，保留 `agent` 作为原工作区语义。
2. Sidebar Modules 顺序改为 `Remote Agents > Skills > Sites`，`Remote Agents` 点击后触发 `go('remote-agents')`。
3. 主内容分发新增 `remote-agents` 视图，右侧渲染 `RemoteAgentsPage`。
4. Telegram 组件迁移到 `workspace/components/remote-agents/telegram-section.tsx`，作为唯一业务实现。
5. Settings Dialog 移除 `telegram` section（`SettingsSection`、`settingsSections`、`SectionContent` 分支均已删除）。

### 10.2 测试回归

1. 新增 `modules-nav.test.tsx`：校验模块顺序与 `Remote Agents -> remote-agents` 点击语义。
2. 新增 `workspace-shell-main-content-model.test.ts`：校验 `remote-agents` 路由分发。
