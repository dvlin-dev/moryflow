...
1. Skills 已经改为“安装包内置基线 + 冷启动触发的在线 revision 检查 + 原子覆盖更新”的单轨模型。
2. 受管技能分成两类：
   - 自动预装：默认安装，离线可用
...
2. 应用冷启动会触发受管 skills 的在线 revision 检查；最近已检查过的 skill 会按成功/失败冷却窗口跳过。
...
2. 每次冷启动都会启动受管 skill 的在线同步流程，并在冷却窗口内跳过重复检查。
