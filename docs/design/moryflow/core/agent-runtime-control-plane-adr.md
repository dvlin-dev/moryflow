...
- 2026-01-28：完成 PC + Mobile 模式切换落地（全局权限模式存储、全权限确认、审计记录与自动批准）。
...
- 受保护的工具输出（默认）：`subagent`、`write`、`edit`、`move`、`delete`。
- **摘要重写**：
...
1. 轻量 task 工具：
   - `task`
   - 只支持 `get / set / clear_done`
   - task 状态真相在 session metadata，compaction 不再依赖也不再保护 task 工具输出
2. 子代理工具：
   - `subagent`
