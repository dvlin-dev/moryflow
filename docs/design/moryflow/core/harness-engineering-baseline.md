...
4. `generate-agent-surface` 已把平台、共享包、运行时能力与长期事实入口转成机读清单
...
4. `runtime-harness.spec.ts` 已把权限、压缩、doom loop、输出截断收敛为统一场景回放
...
2. `packages/ui/test/conversation-harness.test.tsx` 已固定 shared disclosure 与 viewport intent 场景
...
5. PC 已新增 `agent-runtime-harness.spec.ts` 作为独立 Playwright 冒烟入口
