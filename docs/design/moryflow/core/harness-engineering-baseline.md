...
date: 2026-03-13
scope: docs/design/moryflow/core + packages/agents-runtime + packages/ui + apps/moryflow/pc + apps/moryflow/mobile + apps/moryflow/server
status: active
---
...
1. `docs/design/moryflow/core/harness-engineering-baseline.md`
   - 长期事实源
   - 记录当前基线、覆盖边界、目标闭环与实施边界
2. 具体实现细节、验证结果与历史执行过程
   - 统一依赖相关 `design/runbooks`、验证文档与 git / PR 历史

约束：

1. 长期事实写入 `core`，不得长期滞留在临时执行文档
2. Harness 的实现事实生效后，必须回写本文件或相关 runbook/reference，而不是再维护第二套计划正文
...
6. PC 已新增 `core-flow.spec.ts`、`chat-chips.spec.ts`、`agent-runtime-harness.spec.ts`、`automations-harness.spec.ts`、`memory-harness.spec.ts` 五类 feature harness
7. `memory-harness.spec.ts` 固定覆盖 desktop membership token 注入、workspace profile 解析、`desktopAPI.memory.search` 与 `desktopAPI.memory.queryGraph` 的产品壳层读链
