...
5. PC 已新增 `agent-runtime-harness.spec.ts` 作为独立 Playwright 冒烟入口，并断言真实 runtime 请求、失败反馈与当前文件引用保持
...
5. PC 壳层语义改动：先执行 `pnpm build:packages && pnpm --filter @moryflow/pc build`，再执行相关 `renderer/main` 测试与 `pnpm --filter @moryflow/pc exec playwright test tests/agent-runtime-harness.spec.ts`
