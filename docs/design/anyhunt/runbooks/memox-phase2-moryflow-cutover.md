...
- `apps/moryflow/server/src/memox/memox-workspace-content-control.service.ts`
  - `redriveDeadLetters(limit)`
  - `replayOutbox({ batchSize, maxBatches, leaseMs, consumerId? })`
- `apps/moryflow/server/src/memox/memox-workspace-content-control.controller.ts`
  - `POST /internal/sync/memox/workspace-content/replay`
- `apps/moryflow/server/src/memox/memox-internal-metrics.controller.ts`
  - `GET /internal/metrics/memox`
- 历史补偿固定复用 `WorkspaceContentOutbox -> MemoxWorkspaceContentConsumerService` 主链，不允许另起私有 replay 写链
...
- 二期冻结合同已生效：服务 API Key、scope/source identity 映射、平台稳定文件身份返回、gateway -> PC 最小搜索合同、graph scope 合同。
...
- `workspace-content upsert` 固定先 `GET source identity`，再 resolve stable identity；只有在 lookup 证明文件代际已变化时，才允许执行 revision create/finalize，并在 finalize 后 materialize `metadata(content_hash/storage_revision)`
- “是否需要 revision create/finalize” 的判定固定读取 lookup 结果中的既有 lifecycle metadata；禁止读取本次 resolve 刚写回的 metadata 做跳过判断
...
- dead-letter redrive 固定通过 `redriveDeadLetters(limit)` 或 `POST /internal/sync/memox/workspace-content/replay` 的 `redriveDeadLetterLimit` 参数完成；禁止手写 SQL 改 lease / dead-letter 状态
...
2. mutation 回放已通过：第二轮 sync 只产生 `beta rename + gamma delete + delta add` 3 个动作；`replayOutbox()` 返回 `claimed=3 / acknowledged=3 / failedIds=[] / deadLetteredIds=[]`。若 `drained=false`，含义固定为“全局 pending backlog 或 DLQ 未清零”，不代表当前 vault 演练失败。当前运行时代码也已把 drain / replay 吞吐固定为“每 5 秒调度一次、单个 job 最多连续处理 10 个 batch \* 20 条事件”。
