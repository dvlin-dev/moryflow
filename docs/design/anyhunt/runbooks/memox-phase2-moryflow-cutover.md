...
本 runbook 是 Moryflow 二期从旧 `vectorize/search` 切到 Memox 的唯一切换事实源。
...
- `apps/moryflow/server/src/memox/memox-cutover.service.ts`
  ...
  - `shadowCompare({ userId, topK, queries })`
...
- 回放、重试、断点续跑与 cutover rehearsal 不允许改写幂等根规则。
...
- 若 Memox 当前 `current_revision_id + metadata(content_hash/storage_revision)` 已与文件代际一致，则跳过 Memox 侧 revision create/finalize；legacy baseline 只在显式 `legacy_vector_baseline` backend 下跟随写链刷新，不再由默认热路径常驻镜像
...
- `MemoxRuntimeConfigService` 在模块启动期固定 fail-fast 校验 `MEMOX_API_BASE_URL / MEMOX_API_KEY / MEMOX_REQUEST_TIMEOUT_MS`，并冻结 `MORYFLOW_SEARCH_BACKEND`；只有显式启用 `legacy_vector_baseline` 时才要求 `VECTORIZE_API_URL`，且 `MEMOX_API_BASE_URL / VECTORIZE_API_URL` 都固定要求 origin-only，不允许把缺配或明显误配拖到首个用户请求或首轮 drain job
...
drift check 固定按“用户感知是否一致”判定，不按 score 完全一致判定。
...
- `shadowCompare()` 报告固定输出 `expectedHitRate / deletedLeakCount / pathMismatchCount`
- legacy 基线固定为 `LegacyVectorSearchClient.query() -> SyncFile 活跃集过滤`
- Memox 基线固定为 `MemoxSearchAdapterService.searchFiles() -> SyncFile 活跃集校验`
...
2. 若 legacy baseline 数据集与 rollback backend 健康，允许把 `POST /api/v1/search` 通过 `MORYFLOW_SEARCH_BACKEND=legacy_vector_baseline` 临时切回 external legacy baseline 止血；若 baseline 或写入合同本身不健康，则禁止切回，必须先停 ack / 停 consumer 保留 backlog。
3. 若故障根因位于写入合同，写侧必须暂停 ack / 修复合同，必要时按当前环境 scope 定向清理或直接重置 Memox 数据集，再重新执行 backfill + replay。
4. 修复后重新执行 drift check，再次进入 cutover。
5. 整个恢复过程不删除 outbox 事实源；Moryflow `sync` 仍是唯一文件生命周期真相源。
