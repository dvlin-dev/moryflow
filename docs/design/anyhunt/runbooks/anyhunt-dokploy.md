...
| --------------- | ----------------------------------- | ---- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| anyhunt-server  | `apps/anyhunt/server/Dockerfile`    | 3000 | `server.anyhunt.app/api/v1/*` | `DATABASE_URL`<br>`VECTOR_DATABASE_URL`<br>`REDIS_URL`<br>`BETTER_AUTH_SECRET`<br>`BETTER_AUTH_URL`<br>`ADMIN_EMAILS`<br>`ALLOWED_ORIGINS`<br>`TRUSTED_ORIGINS`<br>`SERVER_URL` | `apps/anyhunt/server/**`<br>`pnpm-lock.yaml`<br>`package.json`<br>`pnpm-workspace.yaml`<br>`tsconfig.base.json`                                                                                                                                         |
...
`anyhunt-server` 使用 `ADMIN_EMAILS` 作为管理员邮箱白名单，用户完成注册后自动标记为 `isAdmin=true`（不再在启动期注入管理员密码）。
