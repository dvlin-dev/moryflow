...
原因：这一块负责 `SyncFile` 真相源、`file lifecycle outbox`、Memox bridge、delete/rollback/replay/backfill、quota 与运行时门禁，是 Moryflow 接入 Memox 的核心风险面。
...
原因：这一块决定用户实际搜索结果、active file 过滤、legacy baseline compare/rollback 与跨端 DTO 契约，是用户感知一致性的主链。
...
- `delete / replay / backfill / rollback` 是否幂等且不破坏当前真相源
...
- Memox 与 legacy baseline compare/rollback 边界是否清晰
...
Block C 已收口为 ready。Anyhunt retrieval 聚合、Moryflow `SearchService -> SearchBackendService -> SearchLiveFileProjectorService`、legacy baseline DTO 映射和共享 API 类型上的 review 结果现在与冻结的 `note_markdown` 文件搜索域保持一致。
...
- 是否还有 delete / replay / rollback 的边界缺口
...
3. 本轮没有发现新的未收口系统性问题。
   ...
   - 旧 `vectorize` 栈仍只保留显式 `legacy_vector_baseline` 冷恢复用途，没有发现隐藏运行时依赖复活。
