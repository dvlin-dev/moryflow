 ...
 * 支持基本类型和嵌套对象/数组
 */
function jsonSchemaToZod(jsonSchema: Record<string, unknown>): z.ZodType<unknown> {
...
export class ExtractService {
   ...
   */
  async extract(userId: string, options: ExtractOptions): Promise<ExtractResponse> {
    ...
    this.logger.debug(`Extracting from ${urls.length} URLs with concurrency ${this.concurrency}${model ? `, model=${model}` : ''}`);

    // 并发处理（带限流）
    const results = await this.processWithConcurrency(
      urls,
      (url) => this.extractSingle(userId, url, prompt, systemPrompt, zodSchema, model),
      this.concurrency,
    );
