...
export class MapService {
  ...
  constructor(
    ...
    this.maxCrawlPages = config.get('MAP_MAX_CRAWL_PAGES') || DEFAULT_MAX_CRAWL_PAGES;
  }
  ...
  async map(options: MapOptions): Promise<MapResult> {
    const { url, search, limit = 5000, ignoreSitemap = false, includeSubdomains = false } = options;
    ...
    if (!ignoreSitemap) {
      try {
        const sitemapUrls = await this.sitemapParser.fetchAndParse(baseUrl.origin);
        this.logger.debug(`Found ${sitemapUrls.length} URLs from sitemap`);

        for (const entry of sitemapUrls) {
          if (results.length >= limit) break;
          if (this.isValidUrl(entry.url, baseHost, includeSubdomains) && !seen.has(entry.url)) {
            if (!search || entry.url.toLowerCase().includes(search.toLowerCase())) {
              seen.add(entry.url);
  ...
  private async crawlForUrls(
    ...
    try {
      ...
      while (queue.length > 0 && results.length < limit && pagesVisited < maxPages) {
        ...
        try {
          ...
          for (const link of links) {
            if (results.length >= limit) break;
            if (this.isValidUrl(link, baseHost, includeSubdomains) && !seen.has(link)) {
   ...
   * 验证 URL 是否有效
   */
  private isValidUrl(url: string, baseHost: string, includeSubdomains: boolean): boolean {
    try {
      ...
      // 排除资源文件
      const excludedExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.svg', '.webp', '.ico', '.css', '.js', '.woff', '.woff2', '.ttf', '.eot', '.pdf', '.zip', '.tar', '.gz'];
