...
export class SearchService {
  ...
  private async performSearch(options: {
    ...
    const { query, limit, categories, engines, language, timeRange, safeSearch } =
      options;
    ...
    return this.fetchWithRetry(
      ...
      async (response) => {
        ...
        // 转换并限制结果数量
        const results: SearchResult[] = data.results.slice(0, limit).map((result) => ({
          title: result.title,
          url: result.url,
  ...
  private async fetchWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= this.retryCount; attempt++) {
      try {
        ...
        this.logger.warn(`SearXNG request failed (attempt ${attempt}/${this.retryCount}): ${lastError.message}`);
    ...
    this.logger.error(`SearXNG search failed after ${this.retryCount} attempts`);
    throw lastError;
  }
  ...
  async getAutocomplete(query: string): Promise<string[]> {
    ...
    try {
      const response = await fetch(`${this.searxngUrl}/autocompleter?${params}`, {
        headers: {
