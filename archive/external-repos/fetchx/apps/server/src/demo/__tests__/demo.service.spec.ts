...
describe('DemoService', () => {
  ...
  beforeEach(() => {
    ...
    mockSearchService = {
      search: vi.fn().mockResolvedValue({
        query: 'test query',
        results: [
          { title: 'Result 1', url: 'https://result1.com', content: 'Content 1' },
        ],
      }),
    };
  ...
  describe('scrape', () => {
    ...
    it('should throw BadRequestException on failure', async () => {
      mockScraperService.scrapeSync.mockRejectedValue(new Error('Scrape failed'));
    ...
    it('should pass format options', async () => {
      await service.scrape('https://example.com', ['markdown', 'html', 'links'], true);
  ...
  describe('crawl', () => {
    ...
    it('should throw BadRequestException on failure', async () => {
      ...
      await expect(
        service.crawl('https://example.com', 2, 5),
