...
describe('ApiKeyService', () => {
  ...
  describe('create', () => {
    it('should generate key with correct prefix', async () => {
      ...
      const result = await service.create('user_1', { name: 'Test Key', expiresAt: undefined });
    ...
    it('should store hashed key in database', async () => {
      ...
      await service.create('user_1', { name: 'Test Key', expiresAt: undefined });
  ...
  describe('update', () => {
    it('should update key name', async () => {
      mockPrisma.apiKey.findFirst.mockResolvedValue({ id: 'key_1', keyHash: 'hash_1' });
      mockPrisma.apiKey.update.mockResolvedValue({ id: 'key_1', name: 'Updated Name' });

      const result = await service.update('user_1', 'key_1', { name: 'Updated Name' });
    ...
    it('should invalidate cache when key is deactivated', async () => {
      mockPrisma.apiKey.findFirst.mockResolvedValue({ id: 'key_1', keyHash: 'hash_1' });
      mockPrisma.apiKey.update.mockResolvedValue({ id: 'key_1', isActive: false });
    ...
    it('should not invalidate cache when key is activated', async () => {
      mockPrisma.apiKey.findFirst.mockResolvedValue({ id: 'key_1', keyHash: 'hash_1' });
      mockPrisma.apiKey.update.mockResolvedValue({ id: 'key_1', isActive: true });
  ...
  describe('delete', () => {
    it('should delete key and invalidate cache', async () => {
      ...
      expect(mockPrisma.apiKey.delete).toHaveBeenCalledWith({ where: { id: 'key_1' } });
      expect(mockRedis.del).toHaveBeenCalledWith('apikey:hash_1');
    });
    ...
    it('should throw NotFoundException when key not found', async () => {
      ...
      await expect(
        service.delete('user_1', 'nonexistent'),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('validateKey', () => {
    it('should throw ForbiddenException for invalid format', async () => {
      ...
      await expect(service.validateKey('sk_' + 'a'.repeat(64))).rejects.toThrow(ForbiddenException);
    });

    it('should throw ForbiddenException for non-existent key', async () => {
      mockPrisma.apiKey.findUnique.mockResolvedValue(null);

      await expect(service.validateKey(validApiKey)).rejects.toThrow('Invalid API key');
    });
    ...
    it('should throw ForbiddenException for inactive key', async () => {
      ...
      await expect(service.validateKey(validApiKey)).rejects.toThrow('inactive');
    });
    ...
    it('should use cache when available', async () => {
      const cached = {
        ...
        userId: 'user_1',
        name: 'Cached Key',
        user: { id: 'user_1', email: 'cached@example.com', tier: 'FREE', isAdmin: false },
      };
