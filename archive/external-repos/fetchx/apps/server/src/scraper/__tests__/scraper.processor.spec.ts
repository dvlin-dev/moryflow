...
type MockUpdateCall = [{ where: { id: string }; data: Record<string, unknown> }];
...
describe('ScraperProcessor', () => {
  ...
  describe('error code resolution', () => {
    ...
    it('should set PAGE_NOT_FOUND for 404 errors', async () => {
      ...
      expect(failedCall).toBeDefined();
      expect(failedCall![0].data.errorCode).toBe(ScrapeErrorCode.PAGE_NOT_FOUND);
    });
    ...
    it('should use error code from error object if present', async () => {
      const errorWithCode = new Error('Custom error') as Error & { code: ScrapeErrorCode };
      ...
      expect(failedCall).toBeDefined();
      expect(failedCall![0].data.errorCode).toBe(ScrapeErrorCode.SELECTOR_NOT_FOUND);
    });
  });
