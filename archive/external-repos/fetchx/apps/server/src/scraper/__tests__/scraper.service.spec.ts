...
describe('ScraperService', () => {
  ...
  describe('scrape', () => {
    it('should throw error for SSRF attempt', async () => {
      ...
      await expect(
        service.scrape('user_1', { ...validOptions, url: 'http://169.254.169.254' }),
  ...
  describe('URL validation', () => {
    it('should validate URL before processing', async () => {
      ...
      await service.scrape('user_1', { ...validOptions, url: 'https://valid.example.com' });

      expect(mockUrlValidator.isAllowed).toHaveBeenCalledWith('https://valid.example.com');
    });
    ...
    it('should reject various SSRF attack vectors', async () => {
      ...
      for (const url of ssrfUrls) {
        await expect(service.scrape('user_1', { ...validOptions, url })).rejects.toThrow('SSRF');
      }
    });
  });
  ...
  describe('cache behavior', () => {
    it('should only use cache within TTL', async () => {
      ...
      await service.scrape('user_1', { ...validOptions, url: 'https://example.com' });
    ...
    it('should not cache non-COMPLETED jobs', async () => {
      ...
      const result = await service.scrape('user_1', { ...validOptions, url: 'https://example.com' });
