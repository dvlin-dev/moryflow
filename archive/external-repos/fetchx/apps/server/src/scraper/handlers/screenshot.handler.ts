...
export class ScreenshotHandler {
  ...
  async process(
    ...
    const screenshotOptions = { ...DEFAULT_SCREENSHOT_OPTIONS, ...options.screenshotOptions };
    ...
    // 1. 执行截图
    const { buffer, width, height } = await this.captureScreenshot(page, screenshotOptions);
