...
export class CrawlerProcessor extends WorkerHost {
  ...
  private async handleCrawlBatch(data: CrawlJobData): Promise<void> {
    ...
    const results = await Promise.allSettled(
      batch.map((item) =>
        this.processUrl(crawlJobId, crawlJob.userId, item, options, frontierOptions),
      ),
    );
  ...
  private async processUrl(
    ...
    try {
      ...
      const scrapeOptions = (options.scrapeOptions as Record<string, unknown>) || {};
      ...
      const addedCount = await this.urlFrontier.addUrls(newUrls, frontierOptions);
