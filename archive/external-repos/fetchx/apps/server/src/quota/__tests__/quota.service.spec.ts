...
describe('QuotaService', () => {
  ...
  // 创建一个未过期的配额记录
  const createValidQuota = (overrides: Partial<{
    monthlyLimit: number;
    monthlyUsed: number;
  ...
  describe('getStatus', () => {
    ...
    it('should throw QuotaNotFoundError when quota not exists', async () => {
      ...
      await expect(service.getStatus('nonexistent')).rejects.toThrow(QuotaNotFoundError);
    });
    ...
    it('should reset period when expired', async () => {
      ...
      expect(mockRepository.resetPeriodInTransaction).toHaveBeenCalledWith('user_1');
  ...
  describe('deduct', () => {
    it('should deduct from monthly quota when sufficient', async () => {
      ...
      expect(mockRepository.deductMonthlyInTransaction).toHaveBeenCalledWith('user_1', 1, undefined);
    });
    ...
    it('should throw QuotaNotFoundError when user has no quota', async () => {
      ...
      await expect(service.deduct('nonexistent', 1)).rejects.toThrow(QuotaNotFoundError);
    });
    ...
    it('should pass reason to repository', async () => {
      ...
      expect(mockRepository.deductMonthlyInTransaction).toHaveBeenCalledWith('user_1', 1, 'screenshot_123');
    });
  });
  ...
  describe('deductOrThrow', () => {
    ...
    it('should throw QuotaExceededError when insufficient', async () => {
      ...
      await expect(service.deductOrThrow('user_1', 1)).rejects.toThrow(QuotaExceededError);
    });
  });
  ...
  describe('refund', () => {
    ...
    it('should return failure when refund transaction fails', async () => {
      mockRepository.hasRefundForScreenshot.mockResolvedValue(false);
      mockRepository.refundInTransaction.mockRejectedValue(new Error('DB error'));
  ...
  describe('initialize', () => {
    it('should create quota for new user', async () => {
      ...
      mockRepository.create.mockResolvedValue(createValidQuota({ monthlyLimit: 100 }));
  ...
  describe('updateMonthlyLimit', () => {
    it('should update monthly limit based on tier', async () => {
      ...
      expect(mockRepository.updateMonthlyLimit).toHaveBeenCalledWith('user_1', 60000);
    });
  });
