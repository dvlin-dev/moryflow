...
import type { BrowserInstance, WaitingRequest, BrowserPoolStatus, BrowserPoolDetailedStatus } from './browser.types';
import {
  BROWSER_POOL_SIZE,
...
export class BrowserPool implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.logger.log(`System: ${resources.cpuCount} CPU cores, ${resources.totalMemoryGB}GB RAM (${resources.freeMemoryGB}GB free)`);
    this.logger.log(`Pool size: ${BROWSER_POOL_SIZE} (recommended: ${resources.recommendedPoolSize})`);
    this.logger.log(`Warmup count: ${BROWSER_WARMUP_COUNT} (recommended: ${resources.recommendedWarmupCount})`);
    this.logger.log(`Max pages per browser: ${MAX_PAGES_PER_BROWSER} (recommended: ${resources.recommendedMaxPages})`);
    ...
    const warmupPromises = Array.from({ length: warmupCount }, () => this.createInstance());
    ...
    if (failed > 0) {
      const errors = results
        ...
        .map((r) => r.reason);
      this.logger.warn(`Browser warmup: ${succeeded} succeeded, ${failed} failed`, errors);
    }

    this.logger.log(`Browser pool warmed up with ${succeeded}/${warmupCount} instance(s)`);
  ...
  getDetailedStatus(): BrowserPoolDetailedStatus {
    ...
    return {
      ...
      utilization: {
        poolUtilization: Math.round((basicStatus.total / BROWSER_POOL_SIZE) * 100),
        pageUtilization: Math.round((basicStatus.totalPages / MAX_CONCURRENT_PAGES) * 100),
      },
  ...
  private async cleanupIdleInstances(): Promise<void> {
    ...
    const toClose = idleInstances.slice(0, Math.max(0, this.instances.length - minInstances));
