 ...
 */
function parseEnvInt(value: string | undefined, min: number, max: number): number | undefined {
...
export function getSystemResources(): SystemResources {
  ...
  const recommendedMaxPages = totalMemoryGB >= 16 ? 15 : totalMemoryGB >= 8 ? 10 : 5;
...
export const BROWSER_POOL_SIZE =
  parseEnvInt(process.env.BROWSER_POOL_SIZE, LIMITS.POOL_SIZE.min, LIMITS.POOL_SIZE.max) ?? resources.recommendedPoolSize;

/** 预热浏览器数量 */
export const BROWSER_WARMUP_COUNT =
  parseEnvInt(process.env.BROWSER_WARMUP_COUNT, LIMITS.WARMUP_COUNT.min, LIMITS.WARMUP_COUNT.max) ??
  resources.recommendedWarmupCount;

/** 浏览器实例空闲回收时间（ms） */
export const BROWSER_IDLE_TIMEOUT =
  (parseEnvInt(process.env.BROWSER_IDLE_TIMEOUT, LIMITS.IDLE_TIMEOUT.min, LIMITS.IDLE_TIMEOUT.max) ?? 5 * 60) * 1000;
...
export const MAX_PAGES_PER_BROWSER =
  parseEnvInt(process.env.MAX_PAGES_PER_BROWSER, LIMITS.MAX_PAGES.min, LIMITS.MAX_PAGES.max) ??
