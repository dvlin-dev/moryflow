...
export async function getQueueJobs(
  ...
  const url = qs
    ? `${ADMIN_API.QUEUES}/${name}/jobs?${qs}`
    : `${ADMIN_API.QUEUES}/${name}/jobs`;
...
export async function retryJob(name: string, jobId: string): Promise<QueueOperationResult> {
  return apiClient.post<QueueOperationResult>(
    `${ADMIN_API.QUEUES}/${name}/jobs/${jobId}/retry`,
  );
}
...
export async function cleanQueue(
  ...
  return apiClient.post<QueueOperationResult>(
    `${ADMIN_API.QUEUES}/${name}/clean?status=${status}`,
  );
}
