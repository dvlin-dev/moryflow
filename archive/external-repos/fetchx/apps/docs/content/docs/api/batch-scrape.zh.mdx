...
```bash
curl -X POST https://api.aiget.dev/api/v1/batch/scrape \
...
```javascript
// 创建批量抓取
const response = await fetch('https://api.aiget.dev/api/v1/batch/scrape', {
  method: 'POST',
  headers: {
...
const checkStatus = async (id) => {
  const res = await fetch(`https://api.aiget.dev/api/v1/batch/scrape/${id}`, {
...
response = requests.post(
    'https://api.aiget.dev/api/v1/batch/scrape',
    headers={
        'Authorization': 'Bearer lk_your_api_key',
        'Content-Type': 'application/json',
    },
...
while True:
    status = requests.get(
        f'https://api.aiget.dev/api/v1/batch/scrape/{batch_id}',
        headers={'Authorization': 'Bearer lk_your_api_key'},
    ).json()
