...
import { ArrowRight, Loader2, Globe, Map, Layers, Sparkles, Search, ChevronDown } from 'lucide-react'
...
function ScrapeParams({ formats, onToggleFormat, onlyMainContent, onOnlyMainContentChange }: ScrapeParamsProps) {
  return (
...
function MapParams({ search, onSearchChange, includeSubdomains, onIncludeSubdomainsChange }: MapParamsProps) {
  return (
    <div className="flex flex-wrap items-center gap-4">
      <div className="flex items-center gap-2">
...
function ScreenshotRenderer({ screenshot }: { screenshot: NonNullable<ScrapeResult['screenshot']> }) {
  const src = screenshot.base64
...
function FormatSummary({ format, data }: { format: FormatType; data: ScrapeResult }) {
  switch (format) {
    ...
    case 'html':
      return <span className="text-xs text-muted-foreground">{Math.round((data.html?.length || 0) / 1024)}KB</span>
    case 'rawHtml':
      return <span className="text-xs text-muted-foreground">{Math.round((data.rawHtml?.length || 0) / 1024)}KB</span>
    case 'links':
    ...
    case 'screenshot':
      return data.screenshot?.width && data.screenshot?.height
        ? <span className="text-xs text-muted-foreground">{data.screenshot.width}×{data.screenshot.height}</span>
        : null
    case 'pdf':
      return data.pdf
...
function ResultDisplay({ result, scrapeFormats = ['markdown'] }: ResultDisplayProps) {
  ...
  return (
    <div className="mt-4 border border-border bg-card p-4">
      ...
      {result.type === 'scrape' && <ScrapeResultContent data={result.data} formats={scrapeFormats} />}
    ...
    <div className="space-y-1">
      ...
      <ul className="max-h-48 space-y-1 overflow-auto">
        {data.links.slice(0, MAX_MAP_URLS_DISPLAY).map((link, i) => (
          <li key={i} className="truncate font-mono text-xs text-foreground">{link}</li>
        ))}
    ...
    <div className="mx-auto w-full max-w-2xl">
      ...
      <div className="border border-t-0 border-border bg-card">
        ...
        <div className="flex items-center gap-2 p-3">
          ...
          />
          <Button onClick={handleSubmit} disabled={isSubmitDisabled} size="sm" className="font-mono">
