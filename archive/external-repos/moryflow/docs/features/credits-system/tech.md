...
> 相关代码：
> - `apps/server/src/payment/` - 支付与积分服务
> - `apps/server/src/config/pricing.config.ts` - 价格配置
> - `apps/pc/src/renderer/components/SubscriptionDialog/` - 订阅弹窗
> - `apps/mobile/components/settings/` - 移动端设置
>
...
const creditsUsed = Math.ceil(
  ...
  CREDITS_PER_DOLLAR * PROFIT_MULTIPLIER
);
```
