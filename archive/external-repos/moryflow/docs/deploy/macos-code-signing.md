...
本文档提供 MoryFlow 桌面端（apps/pc）在 macOS 上的代码签名和公证流程，使打包后的应用不会显示"无法验证开发者"警告。
...
生成的 `apps/pc/fastlane/Matchfile`：
...
创建 `apps/pc/build/entitlements.mac.plist`：
...
修改 `apps/pc/electron-builder.yml`：
...
创建 `apps/pc/scripts/notarize.js`：
...
输出位置：`apps/pc/release/{version}/MoryFlow-{version}-arm64.dmg`
...
修改 `apps/pc/.github/workflows/release.yml`：
