...
// 强制所有包使用同一个 React 实例（解决 monorepo 中 Zeego Invalid hook call 问题）
...
// 'react-native' 优先级最高，使 @moryflow/agents-core 使用 shims-react-native.ts
config.resolver.resolverMainFields = ['react-native', 'browser', 'main', 'module']

// 配置条件导出（package.json exports 的条件解析）
// 这使得 Metro 能正确解析 "@moryflow/agents-core/_shims" 的 react-native 条件
...
// 自定义解析器处理 @moryflow/agents-core/_shims
...
const shimsMap = {
  '@moryflow/agents-core/_shims': 'packages/agents-core/dist/shims/shims-react-native.js',
  '@moryflow/agents-realtime/_shims': 'packages/agents-realtime/dist/shims/shims-browser.js',
}
