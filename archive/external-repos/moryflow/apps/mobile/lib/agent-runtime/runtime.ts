 ...
 * 与 PC 端 apps/pc/src/main/agent-runtime/index.ts 对应。
 */

import { run, setTracingDisabled } from '@moryflow/agents'
import {
  createAgentFactory,
  createModelFactory,
  createVaultUtils,
...
import { providerRegistry, toApiModelId } from '@moryflow/agents-model-registry'
...
function createRuntimeInstance(): MobileAgentRuntime {
  return {
    async runChatTurn(options: MobileAgentRuntimeOptions): Promise<MobileChatTurnResult> {
      ...
      if (__DEV__) {
        logger.debug('runChatTurn agent.tools:', agent.tools.map((t) => t.name))
      }
