 ...
 * 参考 PC 端 apps/pc/src/main/chat/messages.ts 的 streamAgentRun 实现。
 */

import type { ChatTransport, UIMessage, UIMessageChunk } from 'ai'
import type { RunItemStreamEvent } from '@moryflow/agents'
import { createSessionAdapter } from '@moryflow/agents-runtime'
import type { AgentChatContext, AgentAttachmentContext } from '@moryflow/agents-runtime'
      ...
      async start(controller) {
        try {
          ...
          for await (const event of result) {
            ...
            const isToolEvent = isEventType(event, eventType, 'run_item_stream_event', 'RunItemStreamEvent')
            const isModelEvent = isEventType(event, eventType, 'raw_model_stream_event', 'RunRawModelStreamEvent')
            ...
            if (isModelEvent) {
              ...
              if (extracted.reasoningDelta) {
                ...
                controller.enqueue({ type: 'reasoning-delta', id: reasoningMessageId, delta: extracted.reasoningDelta })
              }
              ...
              if (extracted.deltaText) {
                ...
                controller.enqueue({ type: 'text-delta', id: textMessageId, delta: extracted.deltaText })
              }
