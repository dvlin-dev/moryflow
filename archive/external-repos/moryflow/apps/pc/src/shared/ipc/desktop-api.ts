...
import type { VaultCreateOptions, VaultFsEvent, VaultInfo, VaultItem, VaultOpenOptions, VaultTreeNode } from './vault'
import type {
...
export type DesktopApi = {
  ...
  vault: {
    ...
    getTreeCache: (vaultPath: string) => Promise<{ capturedAt: number; nodes: VaultTreeNode[] } | null>
  ...
  workspace: {
    ...
    getOpenTabs: (vaultPath: string) => Promise<{ id: string; name: string; path: string; pinned?: boolean }[]>
    setOpenTabs: (vaultPath: string, tabs: { id: string; name: string; path: string; pinned?: boolean }[]) => Promise<void>
  }
  preload: {
    getCache: () => Promise<Record<string, { key: string; loadedAt: number; hash?: string; appVersion?: string }>>
    setCache: (input: { key: string; loadedAt: number; hash?: string; appVersion?: string }) => Promise<void>
  ...
  files: {
    ...
    write: (input: { path: string; content: string; clientMtime?: number }) => Promise<{ mtime: number }>
    createFile: (input: { parentPath: string; name: string; template?: string }) => Promise<{ path: string }>
  ...
  chat: {
    ...
    /** AI 生成会话标题 */
    generateSessionTitle: (input: { sessionId: string; userMessage: string; preferredModelId?: string }) => Promise<ChatSessionSummary | null>
    ...
    /** 替换指定索引的消息内容（用于编辑重发） */
    replaceMessage: (input: { sessionId: string; index: number; content: string }) => Promise<{ ok: boolean }>
    /** 从指定位置分支出新会话 */
  ...
  sitePublish: {
    ...
    /** 检测文件变更 */
    detectChanges: (sourcePaths: string[], lastHashes: Record<string, string>) => Promise<{
      hasChanges: boolean
      changedFiles: string[]
    }>
    /** 更新站点内容（重新构建上传） */
