...
`template/` 目录由 `apps/site-template/` 构建后自动生成：
...
**模板源文件位置**: `apps/site-template/src/templates/`
...
| 场景 | 涉及文件 | 注意事项 |
|------|----------|----------|
| 修改页面样式 | `apps/site-template/src/` | 修改后运行 `pnpm build && pnpm sync` |
| 修改 HTML 模板 | `apps/site-template/src/templates/` | 修改后运行 `pnpm sync` |
...
```
site-publish/
├── 依赖 → apps/site-template/（模板资源）
├── 依赖 → src/shared/ipc/site-publish.ts（IPC 类型）
├── 依赖 → membership-bridge.ts（获取认证 token）
├── 功能文档 → docs/features/site-publish/
└── 服务端 → apps/server/src/site/
```
