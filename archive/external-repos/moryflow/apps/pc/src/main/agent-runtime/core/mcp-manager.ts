import { setTracingDisabled, type MCPServer, type Tool } from '@moryflow/agents'
...
import { MCPServerStdio, MCPServerStreamableHttp } from '@moryflow/agents'
...
export const createMcpManager = (): McpManager<AgentContext> => {
  ...
  const testServer = async (input: McpTestInput): Promise<McpTestResult> => {
    ...
    const safeInput = {
      ...
      config: {
        ...input.config,
        env: input.type === 'stdio' && input.config.env
          ? Object.keys(input.config.env).reduce((acc, key) => {
              acc[key] = '***'
              return acc
            }, {} as Record<string, string>)
          : undefined,
        authorizationHeader: input.type === 'http' && input.config.authorizationHeader ? '***' : undefined,
      },
    }
