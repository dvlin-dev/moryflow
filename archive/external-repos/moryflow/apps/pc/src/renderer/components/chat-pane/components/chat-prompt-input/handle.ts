...
export const enrichFileMetadata = (file: FileUIPart): FileUIPart => {
  const metadata = { ...(file.providerMetadata as Record<string, any> | undefined) }
  const moryflow = { ...(metadata.moryflow as Record<string, any> | undefined) }
  moryflow.usedAsContext = true

  if (isTextualMediaType(file.mediaType)) {
    const preview = extractPreviewFromDataUrl(file.url)
    if (preview) {
      moryflow.preview = preview.text
      moryflow.previewTruncated = preview.truncated
    }
  }

  metadata.moryflow = moryflow
  return {
    ...file,
