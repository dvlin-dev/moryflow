      ...
      try {
        ...
        if (expandedPathsRef.current.size > 0) {
          for (const expandedPath of expandedPathsRef.current) {
            try {
              const children = await window.desktopAPI.vault.readTreeChildren(expandedPath)
              merged = mergeChildrenIntoTree(merged, expandedPath, children)
            } catch (error) {
              console.warn('[workspace] readTreeChildren failed, removing invalid path:', expandedPath)
        ...
        if (invalidPaths.length > 0) {
          updateExpandedPaths((prev) =>
            prev.filter((p) => !invalidPaths.some((invalid) => p === invalid || p.startsWith(invalid + '/')))
          )
        }
        setTree(merged)
        setTreeState('idle')
    ...
    const dispose = window.desktopAPI.events.onVaultFsEvent((event: VaultFsEvent) => {
      ...
      if (dirEvent) {
        ...
        const target =
          event.type === 'dir-added'
            ? event.path
            : getParentDirectoryPath(event.path)
