...
> 最近更新：2026-02-26（CI 安装阶段构建防护：`build:packages` 前新增 `prepare:model-registry-data`，确保 `packages/model-registry-data/src/data/*.json` 在干净环境存在，避免 postinstall `TS2307`）
