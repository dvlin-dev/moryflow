...
1. **`CLAUDE.md` 只承载稳定上下文**：仅记录目录职责、结构边界、公共契约、关键约束、核心入口；禁止写“近期变更 / 最近更新 / PR 收口 / 验证通过”这类时间线。
2. **按事实失真触发更新**：只有当目录职责、结构、跨模块契约、关键约束发生变化时，才更新对应 `CLAUDE.md`；日常实现细节、小修复、局部重构默认不更新。
3. **文件 Header 按语义变化更新**：仅当 `[INPUT] / [OUTPUT] / [POS]`、`[PROPS] / [EMITS]`、`[PROVIDES] / [DEPENDS]` 等头部事实变化时，才更新文件 Header；禁止“改了代码就顺手刷新 Header”。
4. **代码注释只保留当前事实**：业务代码、共享包、脚本与局部 `CLAUDE.md` 禁止记录 `[UPDATE]`、日期、PR 编号、Step 进度、review 闭环、验证通过等时间线式变更日志；需要追历史时直接查看 git / PR / changelog。
5. **`index.md` 只做导航**：索引页只保留文档入口、状态与一行摘要；仅在新增/删除/重命名文档，或导航摘要失真时更新，禁止承担 changelog。
6. **设计正文是唯一事实源**：协议、架构、运维、验收结论写入对应 design/runbook 正文；历史过程依赖 git / PR，不在 `CLAUDE.md` 与 `index.md` 里重复维护。
7. **完成态需求文档合并前必须冻结为事实**：凡 `completed / implemented / confirmed` 的需求/设计/runbook 文档，在合并 PR 前必须把 Step、阶段进度、review/评论回收、验证播报等过程态收敛为“当前状态 / 当前实现 / 当前验证基线”；在不丢失有效事实的前提下删除冗余过程日志，历史追溯统一依赖 git / PR。
8. **禁止历史包袱**：不做向后兼容，无用代码直接删除/重构，不保留废弃注释。
9. **零兼容原则**：除非直接影响用户已有数据，否则一律不兼容旧代码/旧数据结构，按最佳实践重构。
10. **CLAUDE.md 创建门槛**：仅当“当前目录（包含所有子目录）”文件数 **> 10** 时，才允许新增 `CLAUDE.md` 及其 `AGENTS.md` 软链接；否则禁止创建，避免文档层级过度碎片化。
...
- Prisma 相关依赖（`prisma`/`@prisma/client`/`@prisma/adapter-pg`）必须使用精确版本并保持一致；Server Docker 在 deploy 后必须执行版本一致性断言（`generated clientVersion === runtime @prisma/client === prisma`）
...
4. **校验（风险分级）**：按变更风险执行，避免低风险改动重复跑全量流水线：
   - **本地校验最小化原则（强制）**：
     默认不要在每次需求完成后机械性执行根级全量 `pnpm lint`、`pnpm typecheck`、`pnpm test:unit`；本地验证必须优先选择“受影响模块最小闭环”。
   - **受影响范围优先（强制）**：
   ...
   - **L1（中风险）**：组件交互、状态管理、数据映射、非核心逻辑重构  
     至少运行受影响包的 `typecheck` 与 `test:unit`；若有对应测试文件，优先只跑相关用例
   - **L2（高风险）**：核心业务逻辑、跨包接口、后端模块、构建/基础设施改动  
     默认先运行受影响包/链路的最小必要校验，并补齐对应回归测试；仅在以下任一场景成立时，才升级为根级全量：
     1. 改动触及根配置、workspace 依赖、Turbo / TypeScript / ESLint / Prisma / Docker / CI 等基础设施
     2. 改动触及共享包，且影响无法被单个消费者包充分覆盖
     3. 本地已出现跨包集成异常，需要用全量校验定位根因
     4. 用户明确要求执行根级全量校验

     升级到根级全量后，必须运行以下命令并全部通过：
...
5. **同步**：仅在事实失真时更新事实源正文、必要索引与相关 `CLAUDE.md`

### PR 处理流程（强制）

1. **先同步分支基线**：处理 PR 评论前，先同步最新 `main`（合并或 rebase），确保评论定位与代码行号有效。
2. **收敛评论事实源**：统一使用 GitHub PR review threads 作为事实源；每次重新查看 PR 时都要重新拉取 threads，逐条确认 `isResolved/isOutdated`，禁止凭记忆口头状态推进。
3. **先判定是否成立**：每条评论必须先复现或代码走读确认根因，先评估“这条评论是否技术上成立、是否适用于当前代码”；若不成立，回复反证依据；若成立，按“根因治理优先”一次性收口。禁止未评估就按评论字面直接改代码，也禁止跳过当前架构决策与既有产品约束。
4. **禁止补丁式修复**：不加临时分支判断、不叠加兼容映射；直接在事实源模块、协议边界和职责分层处重构到位。
5. **文档与实现同源**：代码改动后，只更新已经失真的事实源文档；禁止把 PR 轮次、过程流水、验证播报继续写进 `CLAUDE.md` 与 `index.md`。
6. **按风险分级验证**：L0/L1/L2 按本章规则执行校验；默认优先受影响模块的最小必要验证，不得把根级全量当作每次改动后的固定动作；未通过不得推进到提交与推送阶段。
7. **提交与推送需用户授权**：遵循“AI 提交约束”，仅在用户明确批准后执行 `git commit` / `git push` / 发布相关操作。
8. **回写 PR 状态闭环**：每次推送后，必须把“当前这次提交已经解决的问题”在线程内逐条回复并 resolve，不得把已闭环问题继续留在未解决状态；再次核对未解决线程数量为 0（或明确列出剩余项与处理计划）。
...
- Header 只描述当前职责、输入输出、依赖、契约位置。
- 禁止在 Header 或普通代码注释中写 `[UPDATE]`、日期、PR/Issue 编号、Step 进度、review 结论、测试通过记录。
- 历史演进放到 git / PR / `CHANGELOG.md`，不写进业务代码。
