...
export class ReadabilityTransformer {
  ...
  private extractByReadability(
    ...
    return article?.content &&
      article.content.trim().length > MIN_CONTENT_LENGTH
      ? article.content
      : null;
  }
  ...
  private finalizeContent(html: string, url: string, baseUrl: string): string {
  ...
  private parseSrcset(srcset: string): Array<{ url: string; size: number }> {
    return srcset
      ...
      .filter(
        (entry): entry is { url: string; size: number } => entry !== null,
      );
  }
}
