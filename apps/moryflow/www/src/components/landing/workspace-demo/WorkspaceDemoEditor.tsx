import type { WorkspaceDemoContent } from './mock-data';
...
export function WorkspaceDemoEditor({
  content,
  title,
  body,
  onBodyChange,
}: WorkspaceDemoEditorProps) {
  return (
    <section className="flex h-full min-h-0 flex-col bg-mory-paper">
      <div className="flex items-center justify-between gap-3 border-b border-mory-border px-6 py-4">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-mory-text-tertiary">{content.editorDescription}</div>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto px-6 py-6">
        <div className="mx-auto max-w-2xl rounded-[28px] border border-mory-border bg-[#fcfbf8] p-6 shadow-[0_18px_44px_rgba(0,0,0,0.05)]">
          ...
          <textarea
            ...
            spellCheck={false}
            aria-label={content.editorOpenDocumentLabel}
          />
        </div>
      </div>
    </section>
