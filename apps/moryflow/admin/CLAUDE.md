...
- 2026-03-07：Memox 二期 review follow-up 已同步到 Admin：storage 相关 UI 现只承认“云存储 / Vault”语义，不再保留向量化历史口径；相关事实源以 server `admin-storage` 返回的 live `SyncFile` 统计为准。
...
- 2026-03-07：Admin storage DTO 事实源已收口到 `@moryflow/api/admin-storage`；`src/types/storage.ts` 仅保留应用内 re-export，禁止继续手写平行接口。
