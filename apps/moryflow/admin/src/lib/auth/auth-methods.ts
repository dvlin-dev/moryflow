...
import { fetchCurrentAdmin, logoutByToken, refreshByToken, signInWithEmail } from './auth-api';
...
export async function bootstrapAuth(): Promise<void> {
  ...
  try {
    let user = await fetchCurrentAdmin(token);
    if (!user) {
      const refreshed = await refreshAccessToken();
      if (refreshed) {
        const retryToken = getAccessToken();
        if (retryToken) {
          user = await fetchCurrentAdmin(retryToken);
        }
      }
    }
...
export async function signIn(email: string, password: string): Promise<void> {
  ...
  const user = await fetchCurrentAdmin(bundle.accessToken);
  if (!user || !user.isAdmin) {
    clearSession();
