...
export function ModelFormDialog({
  ...
  useEffect(() => {
    if (open) {
      const newCaps = model ? parseCapabilities(model.capabilitiesJson) : null
      form.reset({
        providerId: model?.providerId || providers[0]?.id || '',
        modelId: model?.modelId || '',
        upstreamId: model?.upstreamId || '',
        ...
        maxOutputTokens: model?.maxOutputTokens ?? 4096,
        capabilities: {
          vision: newCaps?.vision ?? false,
          tools: newCaps?.tools ?? false,
          json: newCaps?.json ?? false,
        },
        reasoning: newCaps?.reasoning ?? { enabled: false, effort: 'medium' },
        sortOrder: model?.sortOrder ?? 0,
      })
      setSearchQuery('')
      // 初始化 rawConfig 文本状态
      const rawConfig = newCaps?.reasoning?.rawConfig
      setRawConfigText(rawConfig ? JSON.stringify(rawConfig, null, 2) : '')
      setRawConfigError(false)
    }
  }, [open, model, providers, form])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing ? '修改 Model 配置' : '添加新的 AI 模型'}
          </DialogDescription>
        </DialogHeader>
                            ...
                            <div className="flex justify-between w-full">
                              ...
                              <div className="text-right text-xs ml-2 shrink-0">
                                ...
                                <div className="text-muted-foreground">
                                  ${modelInfo.inputPricePerMillion.toFixed(2)} / ${modelInfo.outputPricePerMillion.toFixed(2)}
                                </div>
                              </div>
                            </div>
                          </CommandItem>
