...
- Chat 可见性状态机纯函数化（2026-03-02）：新增 `lib/chat/visibility-transitions.ts` 统一 Tool/Reasoning 开合迁移判定，并新增 `lib/chat/__tests__/visibility-transitions.spec.ts` 覆盖运行态展开、Tool 完成即折叠、Reasoning 完成即折叠与手动展开优先规则。
