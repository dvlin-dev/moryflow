...
export const cloudSyncEngine = {
  ...
  async init(vaultPath: string): Promise<void> {
    ...
    if (created > 0) {
      console.log(`[CloudSync] fileIndex created ${created} new entries`)
    }

    // 自动绑定
    let binding = await readBinding(vaultPath)
    if (!binding) {
      console.log('[CloudSync] No binding found, trying auto binding...')
      binding = await tryAutoBinding(vaultPath)
      if (!binding) {
        console.warn('[CloudSync] Auto binding failed, entering degraded mode')
        store.setStatus('offline')
        store.setVault(vaultPath, null, null)
