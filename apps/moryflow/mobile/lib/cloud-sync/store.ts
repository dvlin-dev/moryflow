  ...
  try {
    ...
    if (stored) {
      ...
      if (!UUID_REGEX.test(settings.deviceId)) {
        console.log('[CloudSync] Migrating invalid deviceId to UUID format')
    ...
    console.error('[CloudSync] Failed to read settings:', error)
  }
...
export const writeSettings = async (settings: CloudSyncSettings): Promise<boolean> => {
  try {
    ...
    console.error('[CloudSync] Failed to write settings:', error)
    return false
  }
}
...
export const readBindings = async (): Promise<Record<string, VaultBinding>> => {
  try {
    ...
    console.error('[CloudSync] Failed to read bindings:', error)
  }
  return {}
}
...
export const writeBinding = async (binding: VaultBinding): Promise<boolean> => {
  try {
    ...
    console.error('[CloudSync] Failed to write binding:', error)
    return false
  }
}
...
export const deleteBinding = async (localPath: string): Promise<boolean> => {
  try {
    ...
    console.error('[CloudSync] Failed to delete binding:', error)
    return false
  }
}
