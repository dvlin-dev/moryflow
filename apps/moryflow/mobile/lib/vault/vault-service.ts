...
export async function getVault(): Promise<VaultInfo | null> {
  ...
  if (stored) {
    try {
      ...
      console.error('[VaultService] Failed to parse vault info:', error)
    }
  }
...
export async function setVault(info: VaultInfo): Promise<void> {
  ...
  if (!dir.exists) {
    ...
    console.log('[VaultService] Created vault directory:', info.path)
  }

  currentVault = info
  await AsyncStorage.setItem(VAULT_INFO_KEY, JSON.stringify(info))
  console.log('[VaultService] Vault set to:', info.path)

  // 初始化 fileIndex 并扫描
  await fileIndexManager.load(info.path)
  const created = await fileIndexManager.scanAndCreateIds(info.path)
  if (created > 0) {
    console.log(`[VaultService] fileIndex created ${created} new entries`)
  }
}
...
export async function readTree(relativePath: string = ''): Promise<VaultTreeNode[]> {
  ...
  try {
    ...
    console.error('[VaultService] Failed to read tree:', error)
    return []
  }
}
 ...
 * 用于支持文件夹展开/收起功能
 */
export async function readTreeRecursive(relativePath: string = '', maxDepth: number = 10): Promise<VaultTreeNode[]> {
  const vault = await getVault()
  if (!vault) return []
