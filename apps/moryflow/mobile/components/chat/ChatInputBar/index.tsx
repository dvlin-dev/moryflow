...
export function ChatInputBar({
  ...
  // 已选中的上下文文件
  const { contextFiles, addFile, removeFile, clearFiles, isSelected } =
    useContextFiles()

  // 文件搜索
  const { query, setQuery, filteredFiles, clearQuery } =
    useFileSearch(workspaceFiles)
  ...
  const handleSend = useCallback(() => {
    ...
    onSend({
      ...
      metadata: contextFiles.length > 0
        ? createMessageMetadata({ attachments: contextFiles })
        : undefined,
    })
