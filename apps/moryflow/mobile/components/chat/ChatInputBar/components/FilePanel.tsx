...
import { FileTextIcon, SearchIcon, CheckIcon } from 'lucide-react-native'
...
function FileItem({
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
    >
      <FileTextIcon size={18} color={colors.iconMuted} />
      <View style={{ flex: 1 }}>
        <Text
...
export function FilePanel({
  ...
  const panelContent = (
    <View>
      ...
      >
        <SearchIcon size={16} color={colors.iconMuted} />
        <TextInput
          ref={inputRef}
          style={{
            flex: 1,
  ...
  // Fallback: BlurView
  const panelOverlayColor = isDark
    ? 'rgba(20, 20, 22, 0.92)'
  ...
  return (
    <View style={containerStyle}>
      <BlurView
        ...
        style={{ borderRadius: 16 }}
      >
        <View style={{ borderRadius: 16, backgroundColor: panelOverlayColor }}>
          {panelContent}
        </View>
      </BlurView>
    </View>
