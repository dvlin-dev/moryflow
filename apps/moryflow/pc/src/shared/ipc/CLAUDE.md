...
- 新增/修改 IPC 必须三处同步（无历史兼容，旧通道直接删除）：
  - Main：`apps/moryflow/pc/src/main/app/ipc/register-handlers.ts`
...
- `desktop-api.ts`
  ...
  - `membership` 命名空间约束：token-bearing auth 流程（sign-in / verify otp / google exchange / refresh / logout）由 main 独占；renderer-facing payload 只允许 access session，不暴露 refresh token
- `skills.ts`
