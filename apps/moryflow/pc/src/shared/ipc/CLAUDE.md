...
- 按 domain 拆分类型（vault/chat/workspace/site-publish/sandbox 等）
...
- `vault.ts` / `chat.ts` / `site-publish.ts` / `sandbox.ts`
...
- 2026-03-08：cloud-sync IPC 契约新增 `SyncNotice`（当前用于 `conflict_copy_created` 提示），`SyncStatusSnapshot` / `SyncStatusDetail` 现可随状态一起透传 `notice`，供 renderer 在不暴露协议术语的前提下提示冲突副本入口。
- 2026-03-07：task IPC 契约收口：删除 `tasks.ts` 与 `DesktopApi.tasks` 命名空间，`ChatSessionSummary` 直接携带 `taskState`，renderer 通过 `chat:session-event` 读取任务快照，不再保留 `tasks:list/get/changed`。
- 2026-03-05：Quick Chat IPC 契约新增 `quick-chat:setSessionId`：`quick-chat.ts` 增加 `QuickChatSetSessionInput`，`desktop-api.ts` 的 `desktopAPI.quickChat` 增加 `setSessionId(input)`，用于 Quick Chat 当前会话绑定持久化。
- 2026-03-05：新增菜单栏运行时契约：`app-runtime.ts`（`AppCloseBehavior/LaunchAtLoginState/AppRuntimeResult`）与 `quick-chat.ts`（`QuickChatWindowState`）；`desktop-api.ts` 同步新增 `desktopAPI.appRuntime.*` 与 `desktopAPI.quickChat.*`。其中 `appRuntime` 明确错误码边界仅允许 `UNSUPPORTED_PLATFORM | SYSTEM_API_ERROR`。
- 2026-03-05：chat 权限模式契约最终收口：删除会话级 mode 事实源，新增全局模式通道 `chat:permission:getGlobalMode` / `chat:permission:setGlobalMode` / `chat:permission:global-mode-changed`，`ChatSessionSummary` 不再包含 `mode` 字段。
- 2026-03-05：chat 审批契约收口：`chat.ts` 新增 `ChatToolApprovalAction`（`once/allow_type/deny`），`desktop-api.ts` 的 `chat.approveTool` 入参改为必填 `action`，移除 `remember` 兼容。
- 2026-03-05：Telegram IPC 新增自动代理探测契约：`telegram.ts` 增加 `TelegramProxySuggestionInput/Reason/Result`，`desktop-api.ts` 增加 `desktopAPI.telegram.detectProxySuggestion`，并与 main/preload `telegram:detectProxySuggestion` 通道对齐，用于 Agent 页面进入后自动给出 proxy 建议。
- 2026-03-05：chat 正文契约新增 `revision` 防回滚机制：`chat.ts` 新增 `ChatSessionMessagesSnapshot`（`sessionId/messages/revision`），`ChatMessageEvent` 的 `snapshot/deleted` 统一携带 `revision`；`desktop-api.ts` 的 `chat.getSessionMessages` 返回值从 `UIMessage[]` 收口为快照对象。该变更用于根治“初始拉取晚到覆盖实时事件导致 UI 闪回”竞态。
- 2026-03-04：`chat.ChatSessionSummary` 新增 `thinking/thinkingProfile`（会话级 Agent 参数事实源），用于 TG/PC 统一复用模型与思考配置；相关主进程回写路径在 `chat-request`，TG 入站读取路径在 `channels/telegram/runtime-orchestrator`。
- 2026-03-04：chat IPC 新增正文事件契约：`chat.ts` 增加 `ChatMessageEvent`（`snapshot/deleted` + `persisted`），`desktop-api.ts` 增加 `chat.onMessageEvent(handler)`，并由 preload 对齐 `chat:message-event` 订阅桥接，实现“会话摘要事件”与“会话正文事件”职责解耦。
- 2026-03-04：Telegram settings snapshot 契约新增 `botTokenEcho/proxyUrl`（`telegram.ts`），用于重启后回填 Bot Token 密文 echo（password 输入）与 Proxy URL（text 输入）；`desktop-api` 类型面保持兼容。
- 2026-03-04：Telegram IPC 契约新增 Proxy 连通测试能力：`telegram.ts` 扩展 `proxyEnabled/hasProxyUrl/proxyUrl` 字段，并新增 `TelegramProxyTestInput/TelegramProxyTestResult`；`desktop-api.ts` 新增 `desktopAPI.telegram.testProxyConnection`，与 main/preload `telegram:testProxyConnection` 通道对齐。
...
- 2026-03-02：权限 IPC 契约重写：`chat` 权限模式改为全局 `ask | full_access`；`sandbox` 删除 `mode/setMode`，仅保留 `authorizedPaths` 并新增 `addAuthorizedPath`；`agent-settings` 删除 MCP `autoApprove` 字段。
