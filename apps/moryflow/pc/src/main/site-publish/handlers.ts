...
async function buildAndPublishSite(
  ...
  const buildResult = await buildSite(sourcePaths, { siteTitle, onProgress })
...
export function registerSitePublishHandlers() {
  ...
  // 推荐子域名
  ipcMain.handle(
    'site-publish:suggestSubdomain',
    async (_event, { base }: { base: string }) => {
  ...
  ipcMain.handle(
    ...
    async (event, input: BuildSiteInput): Promise<BuildSiteResult> => {
      ...
      try {
        ...
        // 构建并发布
        const site = await buildAndPublishSite(siteId, input.sourcePaths, input.title, onProgress)
        ...
        onProgress({ phase: 'error', current: 0, total: 0, message: errorMessage, error: errorMessage })
    ...
    async (event, { siteId }: { siteId: string }): Promise<BuildSiteResult> => {
      ...
      try {
        ...
        const [site, pages] = await Promise.all([
          ...
          apiRequest<{ path: string; localFilePath: string | null }[]>(`/api/sites/${siteId}/pages`),
        ])

        const sourcePaths = pages.map((p) => p.localFilePath).filter((p): p is string => p !== null)
        ...
        // 构建并发布
        const updatedSite = await buildAndPublishSite(siteId, existingPaths, site.title || undefined, onProgress)
