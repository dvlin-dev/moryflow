...
import { fileCreateSchema, fileReadSchema, fileWriteSchema, folderCreateSchema } from './const.js';
...
const createSequentialPath = async ({
  parentDir,
  baseName,
  extension,
  create,
  ...
  baseName: string;
  extension?: string;
  create: (targetPath: string) => Promise<void>;
}): Promise<string> => {
  let suffix = 1;

  while (true) {
    const candidateBaseName = suffix === 1 ? baseName : `${baseName}${suffix}`;
    const candidateName = extension ? `${candidateBaseName}${extension}` : candidateBaseName;
    const targetPath = path.join(parentDir, candidateName);
...
export const writeVaultFile = async (rawPayload: {
  path: string;
  content: string;
  clientMtime?: number;
}) => {
...
export const createVaultFile = async (rawPayload: {
  parentPath: string;
  name: string;
  template?: string;
}) => {
  ...
  const logicalBaseName = ensureMarkdownName(sanitizedName).replace(/\.md$/i, '');
  const targetPath = await createSequentialPath({
    parentDir,
    baseName: logicalBaseName,
    extension: '.md',
    create: async (candidatePath) => {
      await writeFile(candidatePath, payload.template ?? '', { encoding: 'utf-8', flag: 'wx' });
    },
  });
