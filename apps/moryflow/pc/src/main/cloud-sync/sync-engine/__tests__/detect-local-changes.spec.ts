...
describe('detectLocalChanges (mtime/size prefilter)', () => {
  ...
  afterEach(async () => {
    ...
    await workspaceDocRegistry.clear(vaultPath);
  ...
  it('skips hashing when mtime/size unchanged', async () => {
    ...
    const entry = await workspaceDocRegistry.getByPath(vaultPath, relativePath);
    ...
    await updateSyncMirrorEntry(vaultPath, profileKey, entry!.documentId, {
      vectorClock: {},
      lastSyncedHash: hash,
      lastSyncedClock: {},
    ...
    const result = await detectLocalChanges(vaultPath, profileKey, 'device-1');
  ...
  it('rehashes when mtime/size mismatch', async () => {
    ...
    expect(result.pendingChanges.get(entry!.documentId)?.expectedHash).toBe(
      'old-hash',
    );
  });
  ...
  it('reuses cached hash when file unchanged but not yet synced', async () => {
    ...
    await detectLocalChanges(vaultPath, profileKey, 'device-1');

    readFileMock.mockClear();
    const second = await detectLocalChanges(vaultPath, profileKey, 'device-1');
