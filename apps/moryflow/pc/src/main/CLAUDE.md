...
| ------------------------- | ---- | --------------------------------------------------------------- |
| `index.ts`                | 入口 | 主进程入口                                                      |
| `app/`                    | 目录 | Electron 壳层模块（bootstrap / IPC / 窗口 / runtime / menubar） |
...
- `bootstrap/`
  ...
  - `main-window-runtime.ts` 负责主窗口状态机，`index.ts` 不再直接持有窗口生命周期细节。
  - `membership-reconcile.ts` 负责 membership 登录态变化后的串行协调。
...
- `runtime/`
  - 应用运行时 store、Launch at Login、fs 事件桥接等。
- `menubar/`
...
- 窗口导航安全边界统一由 `app/windows/shared/external-navigation-guards.ts` 绑定，禁止在各窗口内重复实现。
- 运行时持久化按子域拆分：窗口偏好在 `app/runtime/preferences-store.ts`，Quick Chat 会话在 `app/windows/quick-chat/quick-chat-store.ts`，更新设置在 `app/updates/update-settings-store.ts`。
