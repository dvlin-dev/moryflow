...
- Agent Runtime 工具装配 API 命名对齐（2026-03-05）：`agent-runtime/index.ts` 将 `createPcLeanToolsWithoutSubagent` 调整为 `createPcToolsWithoutSubagent`，与 `agents-tools` 的 PC/Mobile 装配命名统一（`createPcTools*` / `createMobileTools*`）。
...
- Chat 面板实时同步重构（2026-03-04）：主进程新增 `chat:message-event` 正文广播通道并与 `chat:session-event` 解耦；`chat-request`、`chat/handlers`、`channels/telegram/runtime-orchestrator` 在正文写路径统一广播 snapshot（持久化/预览分层），`channels/telegram/inbound-reply-service.ts` 新增节流 `publishConversationPreview` 回调，支持 TG 入站过程中用户输入与助手草稿实时回显到 PC Chat 面板。
- Telegram 流式性能收口（2026-03-04）：`channels/telegram/inbound-reply-service.ts` 的 delta 处理从串行 await 改为“非阻塞合并发送”（`queuedDraftText + previewUpdateTask + 节流调度`），并在流结束前统一 `drain in-flight preview`，根治 TG 侧逐 token 慢速输出；新增回归用例 `preview update 在网络慢时应合并后续增量，避免逐字阻塞`。
- Telegram 统一收口重构（2026-03-04）：`channels/telegram/inbound-reply-service.ts` 新增 `syncConversationUiState` 回调并在 `/start`、`/new`、普通消息链路统一触发；`channels/telegram/runtime-orchestrator.ts` 将回调绑定到 `chatSessionStore.getHistory -> agentHistoryToUiMessages -> sanitizePersistedUiMessages -> updateSessionMeta -> broadcastSessionEvent`，解决 TG 对话不进入 Chat 面板的问题；同时 `conversation-service.ts` 与 `runtime-orchestrator.ts` 增加 workspace 绝对路径校验，防止相对路径导致运行上下文漂移。对应回归：`inbound-reply-service.test.ts`、`runtime-orchestrator.test.ts`、`conversation-service.test.ts`。
- Telegram C+ 会话路由收口（2026-03-04）：`channels/telegram` 新增 `conversation-service`（`ensureConversationId/createNewConversationId`），`sqlite-store` 会话映射事实源从 `session_key` 收敛为 `conversation_id`（表 `channel_conversation_bindings`）；`inbound-reply-service` 增加 `/start`（幂等建连）与 `/new`（强制新会话）命令分流，普通消息执行前强制解析真实 `conversationId`，不再使用渠道 `sessionKey` 作为 chatId；`/new` 增加按 `eventId` 的重试缓存，避免同一 update 重试时重复创建会话。
- Telegram settings 回显补齐（2026-03-04）：`channels/telegram/settings-application-service.ts` 的 `getSettings` 快照新增回填 `botTokenEcho/proxyUrl`（bot token 为主进程可解密密文回显，proxy URL 明文）；`updateSettings` 新增 `botTokenEcho` 识别，未改动 token 时不重复写 keytar，并继续保留 `hasBotToken/hasProxyUrl` 布尔状态。
...
- Agent Runtime 工具装配收敛（2026-03-03）：`agent-runtime/index.ts` 改为 PC Bash-First 链路，默认不再注入 `read/write/edit/delete/move/ls/glob/grep/search_in_file`；基础工具收敛为 web/task/image + 沙盒 `bash` + `subagent` + `skill` + MCP/external，并将 `subagent` 子代理改为“复用主 agent 工具事实源的单一全能力面”（含 MCP/external，且随主链路自动同步）。
...
- Chat 调试日志升级（2026-02-28）：`thinking-debug` 已升级为 `chat-debug-log`，日志文件统一为 `chat-stream.log`，全环境默认开启并按大小阈值裁剪（不再每次启动清空）；初始化失败自动降级 console-only（不阻断 app 启动）。
...
- Chat 权限模式切换：全局 `mode` 事实源、IPC 更新入口与运行时注入，全权限自动放行并审计
