...
import { readVaultTree } from '../vault/index.js';
import { getTreeCache, setTreeCache } from '../vault/tree-cache.js';
import { WATCHER_READY_DEPTH, WATCHER_SCHEDULE_DELAY, type EmitFsEvent } from './const.js';
  ...
  const startWatcher = async (resolved: string) => {
    try {
      ...
      registerFsEvents(vaultWatcher, emitFsEvent).on('ready', () =>
        hydrateInitialDiff(resolved).catch((error) =>
          console.warn('[vault] watcher ready diff failed', error)
        )
      );
