...
import type { AutomationContextRecord, AutomationJob } from '@moryflow/automations-core';
...
describe('automation service', () => {
  it(
    'wires scheduler to the full run pipeline instead of the raw runner',
    { timeout: 30_000 },
    async () => {
  ...
  it('canonicalizes conversation-session source when updating an automation', async () => {
    ...
    const service = createAutomationService({
      ...
      resolveApprovedVaultPath: (vaultPath) => vaultPath,
      now: () => 50,
      generateAutomationId: () => 'job-1',
    });
  ...
  it('canonicalizes persisted automation-context source before runAutomationNow execution', async () => {
    ...
    const service = createAutomationService({
      ...
      resolveApprovedVaultPath: (vaultPath) =>
        vaultPath.startsWith('/vaults/') ? vaultPath : null,
      now: () => 20,
      generateAutomationId: () => 'job-1',
    });
