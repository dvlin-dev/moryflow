...
export async function readWorkspaceFileIpc(
  ...
  const TEXT_MIME_MAP: Record<string, string> = {
    ...
    '.sql': 'text/x-sql', '.r': 'text/plain', '.m': 'text/plain',
  ...
  const slice = fullContent.slice(offsetChars, offsetChars + maxChars);
  const endOffset = offsetChars + slice.length;
  const truncated = endOffset < fullContent.length;
