...
import { applyWriteOperation, writeOperationSchema } from '@aiget/agents-tools'
import { createVaultUtils } from '@aiget/agents-runtime'
import { createDesktopCapabilities, createDesktopCrypto } from '../agent-runtime/desktop-adapter.js'
...
const sessions = new Map<string, { stream: ReadableStream<UIMessageChunk>; cancel: () => Promise<void> | void }>()
...
export const registerChatHandlers = () => {
  ...
  ipcMain.handle('chat:sessions:rename', (_event, payload: { sessionId: string; title: string }) => {
  ...
  // AI 生成会话标题
  ipcMain.handle('chat:sessions:generateTitle', async (_event, payload: { sessionId: string; userMessage: string; preferredModelId?: string }) => {
  ...
  ipcMain.handle('chat:sessions:truncate', (_event, payload: { sessionId: string; index: number }) => {
  ...
  ipcMain.handle('chat:sessions:fork', (_event, payload: { sessionId: string; atIndex: number }) => {
