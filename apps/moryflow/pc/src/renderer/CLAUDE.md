...
- Chat 消息 loading 判定修复（2026-03-01）：assistant 空消息仅在“最后一条且状态为 submitted/streaming”时显示 loading；非运行态空 assistant 直接跳过渲染，且 file-only assistant 保持可见，避免假 loading 与误隐藏。
