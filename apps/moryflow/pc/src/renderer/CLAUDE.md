...
- 2026-03-07：Auth Session 刷新/注销改为调用主进程代理（`desktopAPI.membership.refreshSession/logout`），renderer 仅管理 access token 与 refresh token 存在性判断。
...
- Workspace Shell：导航语义重构为 `destination(agent/skills/sites) + sidebarMode(home/chat)`；`destination !== 'agent'` 时强制回落 Home 侧栏；支持 `Cmd/Ctrl+1/2/3/4` 快捷键（1/2 切 Home/Chat，3/4 打开 Skills/Sites 并保持 Home 侧栏）
...
- 设置弹窗 Personalization 基线（2026-03-02）：`system-prompt` Tab 下线，替换为 `personalization` Tab；仅保留 `customInstructions` 输入并注入运行时 system prompt
