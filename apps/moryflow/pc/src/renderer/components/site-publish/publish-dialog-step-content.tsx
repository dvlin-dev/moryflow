    ...
    <div className="space-y-4 py-4">
      <div className="rounded-lg bg-muted/50 p-3">
        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{t('publishDialogContentToPublish')}</span>
        </div>
        <div className="text-sm">
          {model.sourcePaths.length === 1 ? (
            ...
            <span className="font-medium">
              {t('publishDialogFilesOrFolders', { count: model.sourcePaths.length })}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <Label htmlFor="subdomain">{t('publishDialogSiteAddress')}</Label>
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <Input
              ...
              placeholder={t('publishDialogSiteAddressPlaceholder')}
              className={cn(
                'pr-8',
      ...
      <div className="space-y-2">
        <Label htmlFor="title">{t('publishDialogSiteTitle')}</Label>
        <Input
          id="title"
          value={model.title}
          onChange={(event) => actions.onTitleChange(event.target.value)}
          placeholder={t('publishDialogSiteTitlePlaceholder')}
        />
      </div>

      <div className="space-y-2">
        <Label htmlFor="description">{t('publishDialogSiteDescription')}</Label>
        <Input
          id="description"
          value={model.description}
          onChange={(event) => actions.onDescriptionChange(event.target.value)}
          placeholder={t('publishDialogSiteDescriptionPlaceholder')}
        />
      </div>
    </div>
  );
    ...
    <div className="space-y-4 py-8">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="text-center">
          <p className="font-medium">{progress?.message || t('publishDialogPublishing')}</p>
          <p className="text-sm text-muted-foreground">
            {progress?.phase === 'scanning' && t('publishDialogScanningFiles')}
            {progress?.phase === 'rendering' && t('publishDialogRenderingPages')}
            {progress?.phase === 'uploading' && t('publishDialogUploadingFiles')}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4 py-8">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="text-center">
          <p className="font-medium text-red-600">{t('publishDialogFailed')}</p>
