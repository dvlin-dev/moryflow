...
- 2026-03-02：修复选区引用清理时机：`use-chat-prompt-input-controller` 改为仅在 `onSubmit` 成功后清空 editor selection reference；失败时保留引用；并改为比较 `captureVersion`，确保“同文本重复选中”也不会被旧提交成功回调误清空；已补充失败/并发场景回归测试。
