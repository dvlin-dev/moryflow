...
- 2026-03-05：Quick Chat 会话入口复用落地：`components/chat-pane-header.tsx` 提取 `ChatPaneSessionActions`（历史 `...` + 新会话 `+`）；`index.tsx` 在 `variant="mode"` 顶部新增右对齐会话操作区并复用同一逻辑（不渲染折叠按钮）。新增回归：`components/chat-pane-header.test.tsx` 覆盖会话切换/新建/hover 删除。
