...
export const useMessageActions = ({
  ...
  const onResend = useCallback(
    ...
    [sessionId, messages, setMessages, regenerate]
  )
  ...
  const onEditAndResend = useCallback(
    async (messageIndex: number, newContent: string) => {
      ...
      await window.desktopAPI.chat.truncateSession({ sessionId, index: messageIndex })
      // 替换内容
      await window.desktopAPI.chat.replaceMessage({ sessionId, index: messageIndex, content: newContent })
      // 更新本地状态，创建新的用户消息
  ...
  const onFork = useCallback(
    async (messageIndex: number) => {
      ...
      const newSession = await window.desktopAPI.chat.forkSession({ sessionId, atIndex: messageIndex })
      // 切换到新会话
