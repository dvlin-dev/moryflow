...
- Docker workspace 构建链路重构（2026-03-02）：Dockerfile 改为复制完整 workspace，统一执行 `pnpm build:packages` 后再构建 server，并通过 `pnpm --filter @moryflow/server deploy --prod` 导出运行时目录，移除手工拷贝 workspace 依赖白名单；`docker-entrypoint.sh` 改为调用本地 `./node_modules/.bin/prisma` 执行迁移，避免全局 prisma 依赖漂移。
