...
const DocumentIdSchema = z
  ...
  .max(128, 'documentId exceeds maximum length');
...
const InlineTextDocumentSchema = z.object({
  ...
  contentText: z
    .string()
    .min(1, 'contentText is required')
    .max(2_097_152, 'contentText exceeds 2 MB limit'),
});
...
export const WorkspaceContentBatchUpsertSchema = z.object({
  ...
  documents: z
    .array(WorkspaceContentDocumentSchema)
    .min(1)
    .max(500)
    .refine(
...
export const WorkspaceContentBatchDeleteSchema = z.object({
  workspaceId: WorkspaceIdSchema,
  documents: z
    .array(WorkspaceContentDeleteDocumentSchema)
    .min(1)
    .max(500)
    .refine(
      (docs) => new Set(docs.map((d) => d.documentId)).size === docs.length,
