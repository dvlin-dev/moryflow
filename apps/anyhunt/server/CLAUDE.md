...
- 2026-03-08：`src/sources/knowledge-source-deletion.service.ts` 的 delete 链路已收口成 durable async cleanup：请求路径在 source 标记 `DELETED` 后改为“尽力立即入队 + 入队失败只记 warn”，新增 `src/sources/source-cleanup-recovery.service.ts` 每 5 分钟扫描 `DELETED` source 重新补投 cleanup queue，避免 Redis/Bull 短暂故障把 source 永久卡在删除中间态；`processCleanupJob()` 的 graph cleanup 也降级为 best-effort，不再阻塞 source 硬删除。`src/sources/knowledge-source.repository.ts` 的 `createSource()` 还新增 `P2002` 收口，公开 `POST /sources` 在并发唯一键竞争下固定返回结构化 `409 KNOWLEDGE_SOURCE_ALREADY_EXISTS`，不再泄漏 500。
