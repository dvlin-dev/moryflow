...
- CDP provider 收口：移除 Browserbase/Browser Use 第三方 provider 代建链路，`connectCdp` 协议改为仅接受标准 `wsEndpoint/port`；`cdp-connector` 删除 provider 生命周期分支，补齐 `cdp-connector` 与 `ConnectCdpSchemaRefined` 回归测试
