...
> L3 Agent API（官方 AI SDK UI Stream）+ Browser Tools（基于 `@openai/agents-core`）

## Overview

Agent 模块提供 `/api/v1/agent` 能力：将用户的自然语言需求编排为一组工具调用（Browser Tools），并以官方 AI SDK UI stream 协议流式返回结果。
...
- 流式输出：官方 `createAiSdkUiMessageStreamResponse`（不再维护私有 SSE 事件协议）
...
- **Stream 协议约束**：仅允许官方 UI stream 协议，禁止新增私有 `event:` 类型映射
