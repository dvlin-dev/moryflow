...
- Better Auth Prisma adapter 缺包根因治理（2026-03-05）：`@anyhunt/anyhunt-server` 显式声明 `better-auth@^1.5.3` 与 `@better-auth/prisma-adapter@^1.5.3`，避免 deploy/runtime 解析到分包适配器时缺失 `@better-auth/prisma-adapter`；Docker 构建阶段新增 `scripts/assert-better-auth-prisma-adapter.mjs` 做依赖与入口加载 fail-fast 校验。
...
auth/
├── better-auth - 认证框架
├── @better-auth/prisma-adapter - Better Auth Prisma 适配器（显式运行时依赖）
├── prisma - 用户与 token 存储
