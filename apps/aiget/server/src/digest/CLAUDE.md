...
[POS]: `apps/aiget/server/src/digest/*` 的统一入口说明（供人类与 AI 协作）
...
Digest 是 Aiget Dev 的核心能力：智能内容订阅系统（订阅 → 抓取/筛选 → 生成摘要/叙事 → 投递到 Inbox/Webhook/Email）。
...
- Email 链接使用 `AIGET_WWW_URL` 生成（默认 `https://aiget.dev`），用于：
  - Inbox 查看链接
  - Unsubscribe 链接
