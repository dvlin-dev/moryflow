...
export class AgentTaskRepository {
  ...
  async getTaskForUser(taskId: string, userId: string): Promise<AgentTask | null> {
    return this.prisma.agentTask.findFirst({
      where: { id: taskId, userId },
    });
  }

  async updateTask(taskId: string, params: UpdateAgentTaskParams): Promise<AgentTask> {
  ...
  async updateTaskMetrics(
    taskId: string,
    params: Pick<UpdateAgentTaskParams, 'creditsUsed' | 'toolCallCount' | 'elapsedMs'>
  ): Promise<void> {
  ...
  async createCharge(params: CreateAgentTaskChargeParams): Promise<AgentTaskCharge> {
  ...
  private toUpdateData(params: UpdateAgentTaskParams): Prisma.AgentTaskUpdateInput {
    return {
      status: params.status,
      result: params.result === undefined ? undefined : this.toJson(params.result),
      error: params.error,
      creditsUsed: params.creditsUsed,
      toolCallCount: params.toolCallCount,
      elapsedMs: params.elapsedMs,
