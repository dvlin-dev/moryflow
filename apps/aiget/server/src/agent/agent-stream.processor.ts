...
import { AgentBillingService, type BillingParams } from './agent-billing.service';
...
export class AgentStreamProcessor {
  ...
  async *consumeStreamEvents(
    ...
    for await (const event of streamResult) {
      ...
      this.billingService.updateBillingFromUsage(billing, streamResult.state?._context?.usage);
      ...
      try {
        await this.billingService.applyQuotaCheckpoint(params.userId, taskId, billing);
      } catch (error) {
        abortController.abort(error);
        throw error;
      }
