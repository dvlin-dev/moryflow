...
export class AgentController {
  ...
  async createTask(
    ...
    try {
      // 使用流式执行
      for await (const event of this.agentService.executeTaskStream(input, user.id)) {
        if (!connectionClosed) {
          sendEvent(event);
        }
      }
  ...
  async getTaskStatus(@CurrentUser() user: CurrentUserDto, @Param('id') taskId: string) {
  ...
  @ApiOperation({
    ...
    description: 'Estimate the credits cost for a task based on prompt length and URLs.',
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description: 'Cancel a running or pending task. Returns credits consumed before cancellation.',
  })
  @ApiParam({ name: 'id', description: 'Task ID' })
  @ApiOkResponse({
  ...
  @ApiNotFoundResponse({ description: 'Task not found' })
  async cancelTask(@CurrentUser() user: CurrentUserDto, @Param('id') taskId: string) {
