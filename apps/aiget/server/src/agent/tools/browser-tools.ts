...
const snapshotSchema = z.object({
  interactive: z.boolean().optional().default(true).describe('仅返回可交互元素'),
  maxDepth: z.number().optional().describe('最大深度限制'),
});

const selectorSchema = z.object({
  selector: z.string().describe('元素选择器，支持 @ref 格式（如 @e1）或 CSS 选择器'),
});
...
const getBrowserContext = (runContext?: BrowserToolRunContext): BrowserAgentContext => {
...
export const typeTool = tool({
  name: 'browser_type',
  description: '在输入框中逐字输入文本。不会清空原有内容，适合追加输入或触发输入事件。',
...
export const pressTool = tool({
  name: 'browser_press',
  description: '模拟按下键盘按键。可用于提交表单（Enter）、取消操作（Escape）等。',
