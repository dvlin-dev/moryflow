...
vi.mock('@aiget/agents-core', async () => {
  const actual = await vi.importActual<typeof import('@aiget/agents-core')>('@aiget/agents-core');
  return {
    ...actual,
    run: vi.fn(),
  };
...
): AgentStreamProcessor => new AgentStreamProcessor(progressStore, billingService);
...
const createMockBillingService = (): AgentBillingService => {
  ...
  return {
    createBillingParams: vi.fn().mockImplementation((startTime: Date, maxCredits?: number) => ({
      maxCredits,
      currentCredits: 1,
      toolCallCount: 0,
      startTime,
    ...
    calculateCreditsFromStream: vi.fn().mockImplementation((result, billing) => {
...
describe('AgentService', () => {
  ...
  it('marks task cancelled when cancel flag is set during execution', async () => {
    ...
    mockRun.mockResolvedValue(createStreamResult({ inputTokens: 0, outputTokens: 0 }) as never);
