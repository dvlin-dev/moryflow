...
When a job completes, AIGet sends a POST request to your webhook URL.
...
async function handleCrawlComplete(data) {
  ...
  const response = await fetch(
    `https://api.aiget.dev/api/v1/crawl/${data.id}`,
...
def handle_crawl_complete(data):
    ...
    response = requests.get(
        f"https://api.aiget.dev/api/v1/crawl/{data['id']}",
        headers={'Authorization': 'Bearer lk_your_api_key'}
    )
...
If your webhook endpoint fails, AIGet retries:
...
const crawlResponse = await fetch('https://api.aiget.dev/api/v1/crawl', {
  method: 'POST',
  headers: {
