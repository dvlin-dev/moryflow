...
```bash
curl -X POST https://server.aiget.dev/api/v1/batch/scrape \
...
```javascript
// 创建批量抓取
const response = await fetch('https://server.aiget.dev/api/v1/batch/scrape', {
  method: 'POST',
  headers: {
...
const checkStatus = async (id) => {
  const res = await fetch(`https://server.aiget.dev/api/v1/batch/scrape/${id}`, {
...
console.log('结果:', status.data.data);
```
...
response = requests.post(
    'https://server.aiget.dev/api/v1/batch/scrape',
    headers={
        'Authorization': 'Bearer lk_your_api_key',
        'Content-Type': 'application/json',
    },
...
while True:
    status = requests.get(
        f'https://server.aiget.dev/api/v1/batch/scrape/{batch_id}',
        headers={'Authorization': 'Bearer lk_your_api_key'},
    ).json()
