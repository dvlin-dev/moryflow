...
export function QuickPlayground() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      {/* Query Input */}
      <QueryInput onSubmit={handleSearch} isLoading={isLoading} disabled={!canSubmit} />

      {/* Preset Buttons */}
      <PresetButtons onSelect={handleSearch} isLoading={isLoading} disabled={!canSubmit} />
      ...
      {turnstileSiteKey && (
        <div className="text-center text-sm">
          {captchaToken ? (
            ...
            <span className="text-gray-500">Complete the verification above to enable search</span>
          )}
        </div>
      )}

      {/* Stats Bar */}
      <StatsBar searchTime={result?.searchTime ?? null} totalFound={result?.totalFound ?? null} />

      {/* Search Results */}
      <SearchResults isLoading={isLoading} error={error} results={result?.results ?? null} />
    </div>
  );
}
