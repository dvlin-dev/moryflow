...
export class LlmService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const baseUrl = this.configService.get<string>('LLM_API_URL', '');
...
Only return the JSON array, no other text.`;
    ...
    try {
      return JSON.parse(result.content) as ExtractedEntity[];
    } catch {
      this.logger.warn(
        `Failed to parse entity extraction result: ${result.content}`,
      );
      return [];
    }
  }
...
Only return the JSON array, no other text.`;
    ...
    try {
      return JSON.parse(result.content) as ExtractedRelation[];
    } catch {
      this.logger.warn(
        `Failed to parse relation extraction result: ${result.content}`,
      );
      return [];
    }
  }
