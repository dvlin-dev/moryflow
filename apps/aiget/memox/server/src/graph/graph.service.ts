 ...
 *
 * [PROTOCOL]: When modifying this file, you MUST update this header and apps/aiget/memox/server/src/graph/CLAUDE.md
 */
...
export class GraphService {
  ...
  async traverse(
    ...
    while (queue.length > 0 && visitedNodes.size < limit) {
      ...
      if (current.depth < maxDepth) {
        const relations = await this.relationRepository.findByEntity(
          apiKeyId,
          entity.id,
        );
        ...
        for (const rel of relations) {
          ...
          if (
            ...
            !options.relationTypes.includes(rel.type)
          ) {
            continue;
          }
  ...
  async getNeighbors(
    ...
    for (const rel of relations) {
      ...
      const neighbor = await this.entityRepository.findById(
        apiKeyId,
        neighborId,
      );
  ...
  private async reconstructPath(
    ...
    // 添加源节点
    const sourceEntity = await this.entityRepository.findById(
      apiKeyId,
      sourceId,
    );
    if (sourceEntity) {
