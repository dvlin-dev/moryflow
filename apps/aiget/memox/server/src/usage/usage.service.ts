...
export class UsageService {
  ...
  async recordUsage(
    ...
    this.logger.debug(
      `Recorded usage: user=${userId}, type=${type}, quantity=${quantity}`,
    );
  }
  ...
  async getUserStats(userId: string): Promise<{
    ...
    return {
      totalMemories:
        totalRecords.find((r) => r.type === 'MEMORY')?._sum.quantity ?? 0,
      totalApiCalls:
        totalRecords.find((r) => r.type === 'API_CALL')?._sum.quantity ?? 0,
      thisMonthMemories:
        monthlyRecords.find((r) => r.type === 'MEMORY')?._sum.quantity ?? 0,
      thisMonthApiCalls:
        monthlyRecords.find((r) => r.type === 'API_CALL')?._sum.quantity ?? 0,
    };
  }
