...
export function SearchResults({ isLoading, error, results }: SearchResultsProps) {
  if (isLoading) {
    return (
      <div className="flex h-[300px] items-center justify-center border border-border bg-muted/30">
        <div className="flex flex-col items-center gap-3">
          <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" />
          <span className="font-mono text-sm text-muted-foreground">Searching memories...</span>
        </div>
      </div>
    );
  }
