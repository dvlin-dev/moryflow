...
```typescript
import { searchModels, getModelById, getProviders } from '@anyhunt/model-registry-data';
...
```bash
# 手动同步
pnpm --filter @anyhunt/model-registry-data sync

# 构建时自动同步（prebuild 钩子）
pnpm --filter @anyhunt/model-registry-data build
```
