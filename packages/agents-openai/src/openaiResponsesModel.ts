...
type ResponseShellCallOutput = OpenAI.Responses.ResponseInputItem.ShellCallOutput;
type ResponseShellCallOutputContent = OpenAI.Responses.ResponseFunctionShellCallOutputContent;
type ResponseApplyPatchCallOutput = OpenAI.Responses.ResponseInputItem.ApplyPatchCallOutput;
...
function getToolChoice(toolChoice?: ModelSettingsToolChoice): ToolChoice | undefined {
  ...
  if (output.type === 'image') {
    ...
    if (typeof output.image === 'string' && output.image.length > 0) {
      ...
      if (typeof imageObj.url === 'string' && imageObj.url.length > 0) {
        ...
        const referencedId =
          (typeof imageObj.fileId === 'string' && imageObj.fileId.length > 0 && imageObj.fileId) ||
          (typeof imageObj.id === 'string' && imageObj.id.length > 0 ? imageObj.id : undefined);
        if (referencedId) {
  ...
  if (output.type === 'file') {
    ...
    if (typeof fileValue === 'string') {
      ...
      if (typeof fileValue.data === 'string' && fileValue.data.length > 0) {
        ...
        const referencedId =
          (typeof fileValue.id === 'string' && fileValue.id.length > 0 && fileValue.id) ||
          (typeof (fileValue as any).fileId === 'string' && (fileValue as any).fileId.length > 0
...
function normalizeLegacyFileFromOutput(value: Record<string, any>): {
  ...
  const referencedId =
    ...
    (typeof value.fileId === 'string' && value.fileId.length > 0 ? value.fileId : undefined);
  if (referencedId) {
...
function getImageInlineMediaType(source: Record<string, any>): string | undefined {
...
function getInputItems(input: ModelRequest['input']): OpenAI.Responses.ResponseInputItem[] {
  ...
  return input.map((item): OpenAI.Responses.ResponseInputItem => {
    ...
    if (item.type === 'shell_call') {
      const action: OpenAI.Responses.ResponseInputItem.ShellCall['action'] = {
        ...
        max_output_length:
          typeof item.action.maxOutputLength === 'number' ? item.action.maxOutputLength : null,
      };
    ...
    if (item.type === 'shell_call_output') {
      ...
      const sanitizedOutputs: ResponseShellCallOutputContent[] = shellOutputs.map((entry) => {
    ...
    if (item.type === 'hosted_tool_call') {
      ...
      if (item.providerData?.type === 'mcp_list_tools' || item.name === 'mcp_list_tools') {
        const providerData = item.providerData as ProviderData.HostedMCPListTools;
        ...
        const providerData = item.providerData as ProviderData.HostedMCPApprovalRequest;
        ...
        const providerData = item.providerData as ProviderData.HostedMCPApprovalResponse;
          ...
          outcome:
            entry?.outcome?.type === 'timeout'
              ...
              : {
                  type: 'exit' as const,
                  exitCode:
                    typeof entry?.outcome?.exit_code === 'number' ? entry.outcome.exit_code : null,
                },
        }));
      }
...
export class OpenAIResponsesModel implements Model {
  ...
  async #fetchResponse(request: ModelRequest, stream: false): Promise<OpenAI.Responses.Response>;
  async #fetchResponse(
    request: ModelRequest,
    stream: boolean
  ): Promise<Stream<OpenAI.Responses.ResponseStreamEvent> | OpenAI.Responses.Response> {
    ...
    const shouldSendTools =
      tools.length > 0 || request.toolsExplicitlyProvided === true || !request.prompt;
