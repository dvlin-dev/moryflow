...
import type { AgentInputItem, Session } from '@anyhunt/agents-core';
...
export async function startOpenAIConversationsSession(client?: OpenAI): Promise<string> {
...
export class OpenAIConversationsSession implements Session {
  ...
  async getItems(limit?: number): Promise<AgentInputItem[]> {
    ...
    const toAgentItems = (item: APIConversationItem): AgentInputItem[] => {
      ...
      const outputItems = (item as APIConversationItem & { output?: unknown }).output;
...
const INPUT_CONTENT_TYPES = new Set(['input_text', 'input_image', 'input_file', 'input_audio']);
...
function isResponseOutputItemArray(value: unknown): value is OpenAI.Responses.ResponseOutputItem[] {
