...
export function itemsToMessages(items: ModelRequest['input']): ChatCompletionMessageParam[] {
...
function normalizeFunctionCallOutputForChat(
  ...
  throw new UserError(
    'Only text tool outputs are supported for chat completions. Got item: ' + JSON.stringify(output)
  );
}
...
export function convertHandoffTool(handoff: SerializedHandoff): ChatCompletionTool {
  return {
    type: 'function',
    function: {
