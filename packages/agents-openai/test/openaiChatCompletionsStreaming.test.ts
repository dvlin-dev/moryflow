...
describe('convertChatCompletionsStreamToResponses', () => {
  it('converts chunks to protocol events', async () => {
    async function* stream(): AsyncGenerator<ChatCompletionChunk, void, unknown> {
      ...
      yield makeChunk({
        tool_calls: [{ index: 0, id: 'call', function: { name: 'fn', arguments: 'a' } }],
      });
    }
  ...
  it('strips leading {} from tool call arguments when followed by real args', async () => {
    ...
    async function* stream() {
      yield makeChunk({
        tool_calls: [{ index: 0, id: 'call1', function: { name: 'fn', arguments: '{}' } }],
      });
      yield makeChunk({
    ...
    const final = events[events.length - 1];
    const functionCall = final.response.output.find((o: any) => o.type === 'function_call');
