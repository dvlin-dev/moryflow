...
describe('getInputItems', () => {
  it('converts messages and tool calls/results', () => {
    ...
    expect(items.some((entry) => entry.type === 'function_call_output')).toBe(true);
    expect(items.some((entry) => entry.type === 'computer_call')).toBe(true);
    expect(items.some((entry) => entry.type === 'computer_call_output')).toBe(true);
    ...
    const shellCallOutput = items.find((entry) => entry.type === 'shell_call_output') as any;
    ...
    const applyPatchCall = items.find((entry) => entry.type === 'apply_patch_call') as any;
