...
import { withTrace, setTracingDisabled } from '@anyhunt/agents-core';
...
import type { SerializedOutputType } from '@anyhunt/agents-core';
...
describe('OpenAIChatCompletionsModel', () => {
    ...
    expect(result.output).toEqual([
      {
        ...
        role: 'assistant',
        status: 'completed',
        content: [{ type: 'audio', audio: 'zzz', providerData: { format: 'mp3' } }],
      },
    ]);
  });
  ...
  it('uses correct response_format for different output types', async () => {
    ...
    expect(client.chat.completions.create.mock.calls[0][0].response_format).toEqual({
      type: 'text',
    });
    ...
    expect(client.chat.completions.create.mock.calls[1][0].response_format).toEqual({
      type: 'json_schema',
      json_schema: {
        name: 'output',
        strict: true,
    ...
    expect(client.chat.completions.create.mock.calls[2][0].response_format).toEqual({
      type: 'json_schema',
      json_schema: {
        name: 'o',
        strict: true,
  ...
  it('populates usage from response_done event when initial usage is zero', async () => {
    ...
    vi.mocked(convertChatCompletionsStreamToResponses).mockImplementationOnce(async function* () {
      yield { type: 'first' } as any;
      yield { type: 'second' } as any;
      yield {
        type: 'response_done',
