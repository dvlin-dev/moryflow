...
describe('camelToSnakeCase', () => {
  ...
  it('converts snake_case keys to snake_case', () => {
    expect(camelOrSnakeToSnakeCase({ foo_bar_buz: 1, baz_qux: 2, foo_bar: 3 })).toEqual({
      foo_bar_buz: 1,
      baz_qux: 2,
      foo_bar: 3,
    });
  ...
  it('converts mixed keys to snake_case', () => {
    expect(camelOrSnakeToSnakeCase({ foo_barBuz: 1, bazQux: 2, foo_bar: 3 })).toEqual({
      foo_bar_buz: 1,
      baz_qux: 2,
      foo_bar: 3,
    });
  ...
  it('leaves already snake_case keys as is', () => {
    expect(camelOrSnakeToSnakeCase({ already_snake: 1, also_snake_case: 2 })).toEqual({
      already_snake: 1,
      also_snake_case: 2,
    });
  });
