...
export const SourcesTrigger = ({
  className,
  count,
  formatLabel = (n) => `Used ${n} sources`,
  children,
  ...props
}: SourcesTriggerProps) => (
  <CollapsibleTrigger
    className={cn('flex items-center gap-2 transition-colors duration-fast', className)}
    {...props}
  >
    {children ?? (
      <>
...
export const Source = ({ href, title, children, ...props }: SourceProps) => (
  <a
    className="flex items-center gap-2 transition-colors duration-fast hover:text-primary/80"
    href={href}
    rel="noreferrer"
    target="_blank"
    {...props}
