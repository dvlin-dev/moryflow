...
当任务需要创建文档、报告、代码或其他产物时，必须先规划文件落点：

- 先用 \`ls\`/\`glob\`/\`read\` 了解 Vault 目录结构，再决定落盘位置
- 优先写入语义匹配的现有目录（如 \`docs/\`、\`notes/\`、\`drafts/\`、\`projects/\`）
- 除非用户明确指定，否则不要把新产物直接丢到 Vault 根目录
- 若确实没有合适目录，先创建语义清晰的子目录，再写入文件
- 多文件产物应放在同一任务目录，保持结构可读、可追踪
