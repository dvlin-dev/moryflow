...
export function shellTool(
  ...
  const needsApproval: ShellApprovalFunction =
    typeof options.needsApproval === 'function'
      ...
      : async () => (typeof options.needsApproval === 'boolean' ? options.needsApproval : false);
...
export function applyPatchTool(
  options: Partial<Omit<ApplyPatchTool, 'type' | 'editor' | 'needsApproval'>> & {
    editor: Editor;
...
export function hostedMcpTool<Context = UnknownContext>(
  ...
  if ('serverUrl' in options) {
    ...
    const providerData: ProviderData.HostedMCPTool<Context> =
      typeof options.requireApproval === 'undefined' || options.requireApproval === 'never'
        ? {
            type: 'mcp',
 ...
 */
type ToolExecuteFunction<TParameters extends ToolInputParameters, Context = UnknownContext> = (
  input: ToolExecuteArgument<TParameters>,
  context?: RunContext<Context>,
  details?: { toolCall: protocol.FunctionCallItem }
 ...
 * @param Context The context of the tool
 */
export type ToolOptions<TParameters extends ToolInputParameters, Context = UnknownContext> =
...
export function tool<
  ...
  Context = UnknownContext,
  Result = string,
>(options: ToolOptions<TParameters, Context>): FunctionTool<Context, TParameters, Result> {
