...
import { isArrayBufferView, isNodeBuffer, isSerializedBufferSnapshot } from './utils/smartString';
...
export class Runner extends RunHooks<any, AgentOutputType<unknown>> {
  ...
  async run<TAgent extends Agent<any, any>, TContext = undefined>(
    ...
    const serverManagesConversation =
      Boolean(effectiveOptions.conversationId) || Boolean(effectiveOptions.previousResponseId);
  ...
  #getInputGuardrailDefinitions<TContext, TAgent extends Agent<TContext, AgentOutputType>>(
  ...
  #splitInputGuardrails<TContext, TAgent extends Agent<TContext, AgentOutputType>>(
    state: RunState<TContext, TAgent>
  ) {
  ...
  async #resolveModelForAgent<TContext>(
    ...
    const explictlyModelSet =
      (agent.model !== undefined && agent.model !== Agent.DEFAULT_MODEL_PLACEHOLDER) ||
      (this.config.model !== undefined && this.config.model !== Agent.DEFAULT_MODEL_PLACEHOLDER);
   ...
   * @internal
   */
  async #runStreamLoop<TContext, TAgent extends Agent<TContext, AgentOutputType>>(
    ...
    try {
      while (true) {
        ...
        if (result.state._currentStep.type === 'next_step_final_output') {
          ...
          result.state._currentAgent = result.state._currentStep?.newAgent as TAgent;
   ...
   * @internal
   */
  async #runIndividualStream<TContext, TAgent extends Agent<TContext, AgentOutputType>>(
    agent: TAgent,
    input: string | AgentInputItem[] | RunState<TContext, TAgent>,
    options?: StreamRunOptions<TContext>,
    ensureStreamInputPersisted?: () => Promise<void>,
  ...
  async #runInputGuardrails<TContext, TAgent extends Agent<TContext, AgentOutputType>>(
    state: RunState<TContext, TAgent>,
   ...
   */
  async #prepareModelCall<TContext, TAgent extends Agent<TContext, AgentOutputType>>(
    state: RunState<TContext, TAgent>,
    ...
    const previousResponseId =
      serverConversationTracker?.previousResponseId ?? options.previousResponseId;
...
export function selectModel(
  ...
  if (
    (typeof agentModel === 'string' && agentModel !== Agent.DEFAULT_MODEL_PLACEHOLDER) ||
...
async function applyCallModelInputFilter<TContext>(
  ...
  const cloneInputItems = (items: AgentInputItem[], map?: WeakMap<object, AgentInputItem>) =>
    items.map((item) => {
  ...
  try {
    ...
    return {
      modelInput: {
        ...
        instructions:
          typeof result.instructions === 'undefined' ? systemInstructions : result.instructions,
      },
      sourceItems,
  ...
  prepareInput(
    ...
    if (!this.sentInitialInput) {
      ...
      this.remainingInitialInput = initialItems.filter(
        (item): item is AgentInputItem => Boolean(item) && typeof item === 'object'
      );
      this.sentInitialInput = true;
...
function adjustModelSettingsForNonGPT5RunnerModel(
  ...
  if (
    ...
    (typeof runnerModel !== 'string' || !gpt5ReasoningSettingsRequired(runnerModel)) &&
 ...
 */
async function prepareAgentArtifacts<TContext, TAgent extends Agent<TContext, AgentOutputType>>(
  state: RunState<TContext, TAgent>
...
function takeAgentInputFromPool(pool: AgentInputItemPool, key: string): AgentInputItem | undefined {
...
function removeAgentInputFromPool(pool: AgentInputItemPool, item: AgentInputItem) {
...
function agentInputSerializationReplacer(_key: string, value: unknown): unknown {
...
function toAgentInputList(originalInput: string | AgentInputItem[]): AgentInputItem[] {
