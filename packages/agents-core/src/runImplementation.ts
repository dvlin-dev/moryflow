...
export function processModelResponse<TContext>(
  ...
  const applyPatchTool = tools.find((t): t is ApplyPatchTool => t.type === 'apply_patch');
  ...
  for (const output of modelResponse.output) {
    if (output.type === 'message') {
      ...
      if (
        ...
        const providerData = output.providerData as ProviderData.HostedMCPApprovalRequest;
...
export async function resolveTurnAfterModelResponse<TContext>(
  ...
  if (processedResponse.mcpApprovalRequests.length > 0) {
    for (const approvalRequest of processedResponse.mcpApprovalRequests) {
      const toolData = approvalRequest.mcpTool.providerData as ProviderData.HostedMCPTool<TContext>;
...
function normalizeFileValue(value: Record<string, any>): FileReferenceValue | null {
...
function normalizeFileObjectCandidate(value: unknown): FileReferenceValue | null {
  if (!isRecord(value)) {
    return null;
  }
...
function normalizeLegacyFileValue(value: Record<string, any>): FileReferenceValue | null {
  const filename =
    typeof value.filename === 'string' && value.filename.length > 0 ? value.filename : undefined;
  const mediaType =
    typeof value.mediaType === 'string' && value.mediaType.length > 0 ? value.mediaType : undefined;
...
function toInlineImageString(data: string | Uint8Array, mediaType?: string): string {
...
export async function executeFunctionToolCalls<TContext = UnknownContext>(
  ...
  try {
    ...
    throw new ToolCallError(`Failed to run function tools: ${e}`, e as Error, state);
  }
}
    ...
    async (handoffSpan) => {
      ...
      newStepItems.push(
        new RunHandoffOutputItem(
          getToolCallOutputItem(actualHandoff.toolCall, getTransferMessage(newAgent)),
          agent,
          newAgent
        )
      );
 ...
 */
export async function checkForFinalOutputFromTools<TContext, TOutput extends AgentOutputType>(
  agent: Agent<TContext, TOutput>,
...
function getRunItemStreamEventName(item: RunItem): RunItemStreamEventName | undefined {
...
function enqueueRunItemStreamEvent(result: StreamedRunResult<any, any>, item: RunItem): void {
 ...
 */
export function toInputItemList(input: string | AgentInputItem[]): AgentInputItem[] {
 ...
 */
export function extractOutputItemsFromRunItems(items: RunItem[]): AgentInputItem[] {
  return items
...
function normalizeItemsForSessionPersistence(items: AgentInputItem[]): AgentInputItem[] {
...
function sanitizeValueForSession(value: unknown, context?: SessionBinaryContext): unknown;
function sanitizeValueForSession(value: unknown, context: SessionBinaryContext = {}): unknown {
...
function stripTransientCallIds(value: unknown): unknown {
  ...
  const shouldStripId = isProtocolItem && shouldStripIdForType(record.type as string);
  for (const [key, entry] of Object.entries(record)) {
...
type StructuredToolOutput = ToolOutputText | ToolOutputImage | ToolOutputFileContent;
 ...
 */
function normalizeStructuredToolOutputs(output: unknown): StructuredToolOutput[] | null {
...
function buildItemReferenceMap(items: AgentInputItem[]): Map<string, AgentInputItem[]> {
