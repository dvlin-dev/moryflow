...
const ImageUrlObjectSchema = z.object({
  url: z.string().describe('Publicly accessible URL pointing to the image content'),
});

const ImageFileIdObjectSchema = z.object({
  fileId: z.string().describe('OpenAI file ID referencing uploaded image content'),
});
...
const FileDataObjectSchema = z.object({
  ...
  mediaType: z.string().describe('IANA media type describing the file contents'),
...
export const HostedToolCallItem = ItemBase.extend({
   ...
   */
  arguments: z.string().describe('The arguments of the hosted tool call').optional(),
...
export const FunctionCallResultItem = ItemBase.extend({
   ...
   */
  output: z
    .union([z.string(), ToolCallOutputContent, z.array(ToolCallStructuredOutput)])
    .describe(
...
export const UsageData = z.object({
  ...
  inputTokensDetails: z
    .union([z.record(z.string(), z.number()), z.array(z.record(z.string(), z.number()))])
    .optional(),
