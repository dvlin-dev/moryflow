...
type CompletedRunResult<TContext, TAgent extends Agent<TContext, any>> = RunResult<
  TContext,
  TAgent
> & {
...
const agentToolRunResults = new WeakMap<protocol.FunctionCallItem, AnyAgentRunResult>();
...
export type HandoffsOutputUnion<Handoffs extends readonly (Agent<any, any> | Handoff<any, any>)[]> =
  ...
  implements AgentConfiguration<TContext, TOutput>
{
  ...
  constructor(config: AgentOptions<TContext, TOutput>) {
    ...
    if (
      ...
      (typeof config.model !== 'string' || !gpt5ReasoningSettingsRequired(config.model)) &&
   ...
   */
  async getSystemPrompt(runContext: RunContext<TContext>): Promise<string | undefined> {
   ...
   * runContext and the agent instance.
   */
  async getPrompt(runContext: RunContext<TContext>): Promise<Prompt | undefined> {
   ...
   */
  async getMcpTools(runContext: RunContext<TContext>): Promise<Tool<TContext>[]> {
   ...
   */
  async getEnabledHandoffs(runContext: RunContext<TContext>): Promise<Handoff<any, any>[]> {
