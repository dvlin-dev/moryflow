...
export class RunState<TContext, TAgent extends Agent<any, any>> {
   ...
   */
  public _lastProcessedResponse: ProcessedResponse<TContext> | undefined = undefined;
  /**
  ...
  static async fromString<TContext, TAgent extends Agent<any, any>>(
    ...
    // Rebuild the context
    //
    const context = new RunContext<TContext>(stateJson.context.context as TContext);
    ...
    for (const [agentName, toolNames] of Object.entries(stateJson.toolUseTracker)) {
      state._toolUseTracker.addToolUse(agentMap.get(agentName) as TAgent, toolNames);
    }
    ...
    state._inputGuardrailResults = stateJson.inputGuardrailResults as InputGuardrailResult[];
 ...
 * @internal
 */
export function buildAgentMap(initialAgent: Agent<any, any>): Map<string, Agent<any, any>> {
 ...
 * @internal
 */
export function deserializeSpan(trace: Trace, serializedSpan: SerializedSpanType): Span<any> {
...
async function deserializeProcessedResponse<TContext = UnknownContext>(
  ...
  const tools = new Map(
    allTools.filter((tool) => tool.type === 'function').map((tool) => [tool.name, tool])
  );
  const computerTools = new Map(
    allTools.filter((tool) => tool.type === 'computer').map((tool) => [tool.name, tool])
  );
  ...
  const result = {
    ...
    applyPatchActions: (serializedProcessedResponse.applyPatchActions ?? []).map(
      (applyPatchAction) => {
    ...
    mcpApprovalRequests: (serializedProcessedResponse.mcpApprovalRequests ?? []).map(
      (approvalRequest) => ({
        requestItem: new RunToolApprovalItem(
          approvalRequest.requestItem.rawItem as unknown as protocol.HostedToolCallItem,
          currentAgent
        ),
