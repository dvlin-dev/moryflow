...
// circular dependency resolution issues caused by other exports in '@moryflow/agents-core/_shims'
...
> implements EventEmitter<EventTypes> {
  ...
  on<K extends keyof EventTypes>(type: K, listener: (...args: EventTypes[K]) => void) {
    ...
    const wrapper = ((event: CustomEvent) => listener(...(event.detail ?? []))) as EventListener;
    wrappers.add(wrapper);
    this.#target.addEventListener(eventType, wrapper);
    return this;
  }

  off<K extends keyof EventTypes>(type: K, listener: (...args: EventTypes[K]) => void) {
    const eventType = type as string;
  ...
  once<K extends keyof EventTypes>(type: K, listener: (...args: EventTypes[K]) => void) {
