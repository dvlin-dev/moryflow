...
export function hasJsonSchemaObjectShape(value: unknown): value is LooseJsonObjectSchema {
  return (
    typeof value === 'object' &&
    value !== null &&
...
export function zodJsonSchemaCompat(input: ZodObjectLike): JsonObjectSchema<any> | undefined {
...
function unwrapDecorators(value: unknown): unknown {
  ...
  while (DECORATOR_WRAPPERS.has(readZodType(current) ?? '')) {
    const def = readZodDefinition(current);
    const next =
      def?.innerType ?? def?.schema ?? def?.base ?? def?.type ?? def?.wrapped ?? def?.underlying;
    if (!next || next === current) {
      return current;
    }
    current = next;
...
function extractFirst(def: Record<string, unknown> | undefined, ...keys: string[]): unknown {
  if (!def) {
    return undefined;
  }
