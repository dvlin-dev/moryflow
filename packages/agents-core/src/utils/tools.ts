...
import { zodJsonSchemaCompat, hasJsonSchemaObjectShape } from './zodJsonSchemaCompat';
...
function buildJsonSchemaFromZod(inputType: ZodObjectLike): JsonObjectSchema<any> | undefined {
...
export function getSchemaAndParserFromInputType<T extends ToolInputParameters>(
  ...
  if (isZodObject(inputType)) {
    const useFallback = (originalError?: unknown) => {
      ...
      const errorMessage =
        originalError instanceof Error ? ` Upstream helper error: ${originalError.message}` : '';
