 ...
 */
function defaultHandoffToolDescription<TContext, TOutput extends AgentOutputType>(
  agent: Agent<TContext, TOutput>
) {
...
>(agent: Agent<TContext, TOutput>, config: HandoffConfig<TInputType, TContext> = {}) {
  ...
  async function onInvokeHandoff(context: RunContext<any>, inputJsonString?: string) {
    if (parser) {
      if (!inputJsonString) {
