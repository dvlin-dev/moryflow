...
describe('handoff()', () => {
  it('throws UserError when only onHandoff or inputType provided', () => {
    ...
    expect(() => handoff(agent, { inputType: z.object({}) })).toThrow(UserError);
  });
  ...
  it('parses JSON and reports errors', async () => {
    ...
    await expect(h.onInvokeHandoff({} as any, '')).rejects.toBeInstanceOf(ModelBehaviorError);
    await expect(h.onInvokeHandoff({} as any, 'bad')).rejects.toBeInstanceOf(ModelBehaviorError);
  });
