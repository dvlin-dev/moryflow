...
describe('processModelResponse', () => {
  it('should correctly process message outputs', () => {
    ...
    const result = processModelResponse(modelResponse, TEST_AGENT, [TEST_TOOL], []);

    expect(result.newItems).toHaveLength(2);
    expect(result.newItems[0]).toBeInstanceOf(ToolCallItem);
    expect(result.newItems[0].rawItem).toEqual(TEST_MODEL_RESPONSE_WITH_FUNCTION.output[0]);
    ...
    expect(result.newItems[1].rawItem).toEqual(TEST_MODEL_RESPONSE_WITH_FUNCTION.output[1]);
  ...
  it('throws when shell action emitted without shell tool', () => {
    ...
    expect(() => processModelResponse(modelResponse, TEST_AGENT, [TEST_TOOL], [])).toThrow(
      ModelBehaviorError
    );
  });
  ...
  it('queues apply_patch actions when editor tool registered', () => {
    ...
    const result = processModelResponse(modelResponse, TEST_AGENT, [editor], []);
...
describe('getTracing', () => {
  it('should return the correct tracing value', () => {
    ...
    expect(getTracing(tracingDisabled, tracingIncludeSensitiveData)).toEqual(false);
    expect(getTracing(tracingDisabled, tracingIncludeSensitiveDataDisabled)).toEqual(false);
    expect(getTracing(tracingEnabled, tracingIncludeSensitiveData)).toEqual(true);
    expect(getTracing(tracingEnabled, tracingIncludeSensitiveDataDisabled)).toEqual(
...
describe('saveToSession', () => {
  ...
  it('persists tool outputs when resuming a turn after approvals', async () => {
    ...
    const agent = textAgent as unknown as Agent<UnknownContext, AgentOutputType>;
    const session = new MemorySession();
    const context = new RunContext<UnknownContext>(undefined as UnknownContext);
    const state = new RunState<UnknownContext, Agent<UnknownContext, AgentOutputType>>(
      context,
      'hello',
      agent,
      10
    ...
    await saveToSession(session, toInputItemList(state._originalInput), preApprovalResult);
    ...
    const last = session.items[session.items.length - 1] as protocol.FunctionCallResultItem;
  ...
  it('persists HITL tool outputs when approval items are not the last generated entries', async () => {
    ...
    const autoOutputItem = new ToolCallOutputItem(autoOutputRaw, textAgent, 'Fetched image.');

    state._generatedItems = [approvalToolCallItem, autoToolCallItem, approvalItem, autoOutputItem];
    state._currentStep = {
      type: 'next_step_interruption',
      data: {
        interruptions: [approvalItem],
    ...
    const processedResponse: ProcessedResponse<UnknownContext> = {
      newItems: [approvalToolCallItem, autoToolCallItem, approvalItem, autoOutputItem],
      handoffs: [],
      functions: [
        {
...
describe('prepareInputItemsWithSession', () => {
  ...
  it('only persists new inputs when callbacks prepend history duplicates', async () => {
    ...
    const result = await prepareInputItemsWithSession([newItem], session, (history, newItems) => {
  ...
  it('omits session history from prepared input when includeHistoryInPreparedInput is false', async () => {
    ...
    const session = new StubSession([historyItem]);
    const result = await prepareInputItemsWithSession('fresh input', session, undefined, {
      includeHistoryInPreparedInput: false,
    });
...
describe('checkForFinalOutputFromTools', () => {
  ...
  it("stop_on_first_tool returns NOT_FINAL_OUTPUT when first isn't function output", async () => {
    ...
    const res = await checkForFinalOutputFromTools(agent, [approvalResult], state);
    expect(res.isFinalOutput).toBe(false);
  });
...
describe('addStepToRunResult', () => {
  it('emits the correct RunItemStreamEvents for each item type', () => {
    ...
    const handoffCallItem = new HandoffCallItem(TEST_MODEL_FUNCTION_CALL, agent);
  ...
  it('maintains event order when mixing pre-streamed and step items', () => {
    ...
    expect(captured).toEqual(['message_output_created', 'tool_called', 'tool_output']);
  });
});
...
describe('processModelResponse edge cases', () => {
  it('throws when model references unknown tool', () => {
    ...
    expect(() => processModelResponse(response, TEST_AGENT, [TEST_TOOL], [])).toThrow(
      ModelBehaviorError
    );
  });
  ...
  it('classifies functions, handoffs and computer actions', () => {
    ...
    const result = processModelResponse(response, TEST_AGENT, [TEST_TOOL, computer], [h]);
...
describe('maybeResetToolChoice additional case', () => {
  it('keeps tool choice when agent has not used tools', () => {
    ...
    expect(maybeResetToolChoice(agent, tracker, settings).toolChoice).toBe('auto');
  });
});
...
describe('executeFunctionToolCalls', () => {
  ...
  function makeTool(needs: boolean | (() => Promise<boolean>)): FunctionTool<any, any, any> {
    return tool({
      name: 'hi',
      description: 't',
      parameters: z.object({}),
  ...
  it('returns approval item when not yet approved', async () => {
    ...
    vi.spyOn(state._context, 'isToolApproved').mockReturnValue(undefined as any);
    const invokeSpy = vi.spyOn(t, 'invoke');

    const res = await withTrace('test', () =>
      executeFunctionToolCalls(state._currentAgent, [{ toolCall, tool: t }], runner, state)
    );
  ...
  it('runs tool and emits events on success', async () => {
    ...
    expect(end).toHaveBeenCalledWith(state._context, state._currentAgent, t, 'ok', { toolCall });
    expect(res[0].runItem).toBeInstanceOf(ToolCallOutputItem);
  ...
  it('propagates nested run result interruptions when provided by agent tools', async () => {
    const t = makeTool(false);
    const nestedAgent = new Agent({ name: 'Nested' }) as Agent<unknown, AgentOutputType>;
    const nestedState = new RunState(new RunContext(), '', nestedAgent, 1);
    const approval = new ToolApprovalItem(TEST_MODEL_FUNCTION_CALL, nestedAgent);
  ...
  it('handles multiple handoffs by rejecting extras', async () => {
    ...
    expect(
      res.newStepItems.some(
        (i) => i instanceof ToolCallOutputItem && (i.rawItem as any).callId === '2'
      )
    ).toBe(true);
  });
...
describe('empty execution helpers', () => {
  it('handles empty function and computer calls', async () => {
    ...
    const fn = await withTrace('test', () => executeFunctionToolCalls(agent, [], runner, state));
...
describe('resolveTurnAfterModelResponse', () => {
  ...
  it('does not finalize when tools are used in the same turn (structured output); runs again', async () => {
    ...
    const structuredResponse: ModelResponse = {
      output: [{ ...TEST_MODEL_FUNCTION_CALL }, fakeModelMessage('{"foo":"bar"}')],
      usage: new Usage(),
    } as any;
    ...
    const structuredState = new RunState(new RunContext(), 'test input', structuredAgent, 1);
  ...
  it('throws descriptive error when structured output validation fails', async () => {
    ...
    const processedResponse = processModelResponse(response, structuredAgent, [], []);
  ...
  it('does not finalize after computer actions in the same turn; runs again', async () => {
    ...
    const processedResponse = processModelResponse(computerResponse, computerAgent, [computer], []);

    const computerState = new RunState(new RunContext(), 'test input', computerAgent, 1);
  ...
  it('does not duplicate previously persisted model items when resuming after approvals', async () => {
    ...
    const processedResponse = processModelResponse(response, TEST_AGENT, [TEST_TOOL], []);
    ...
    const persistedMessages = result.generatedItems.filter((item) => {
      return item instanceof MessageOutputItem && item.rawItem.id === 'message-resume';
    });
  ...
  it('does not finalize when hosted MCP approval happens in the same turn; runs again', async () => {
    ...
    const processedResponse = processModelResponse(approvalResponse, approvalAgent, [mcpTool], []);

    const approvalState = new RunState(new RunContext(), 'test input', approvalAgent, 1);
  ...
  it('preserves pending hosted MCP approvals when resuming an interrupted turn', async () => {
    ...
    const resumedState = new RunState(new RunContext(), 'test input', approvalAgent, 1);
...
describe('resolveInterruptedTurn', () => {
  it('rewinds persisted count only for pending approval placeholders', async () => {
    ...
    expect(state._currentTurnPersistedItemCount).toBe(generatedItems.length - 1);
  ...
  it('dispatches approved computer actions when resuming an interruption', async () => {
    const fakeComputer: Computer = {
      ...
      scroll: vi.fn(async (_x: number, _y: number, _sx: number, _sy: number) => {}),
    ...
    expect((toolOutputs[0].rawItem as protocol.ComputerCallResultItem).callId).toBe(
      computerCall.callId
    );
