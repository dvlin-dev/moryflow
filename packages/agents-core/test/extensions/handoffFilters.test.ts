...
import { TEST_AGENT, TEST_MODEL_FUNCTION_CALL, fakeModelMessage } from '../stubs';
...
const shellCallResult: protocol.ShellCallResultItem = {
  ...
  callId: 'shell-call',
  output: [{ stdout: 'hi', stderr: '', outcome: { type: 'exit', exitCode: 0 } }],
};
...
describe('removeAllTools', () => {
  ...
  test('removes run tool and handoff items from run collections', () => {
    const message = new RunMessageOutputItem(fakeModelMessage('ok'), TEST_AGENT);
    const anotherMessage = new RunMessageOutputItem(fakeModelMessage('still here'), TEST_AGENT);
