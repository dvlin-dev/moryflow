...
describe('Agent', () => {
  ...
  it('should throw if name is missing', () => {
    ...
    expect(() => new Agent({ name: '' } as any)).toThrow('Agent must have a name.');
  });
  ...
  it('should clone an agent with modified values', () => {
    ...
    expect(clonedAgent.handoffDescription).toBe(originalAgent.handoffDescription);
  ...
  it('should generate a tool from an agent', async () => {
    ...
    const result2 = await tool.invoke({} as any, '{"input":"hey how are you?"}');
  ...
  it('allows configuring needsApproval when using an agent as a tool', async () => {
    ...
    const decision = await tool.needsApproval(new RunContext(), rawArgs, 'call-id');

    expect(approval).toHaveBeenCalledWith(expect.any(RunContext), rawArgs, 'call-id');
  ...
  it('passes runConfig and runOptions to the runner when used as a tool', async () => {
    ...
    const runSpy = vi.spyOn(Runner.prototype, 'run').mockImplementation(async () => mockResult);
    ...
    expect(runnerInstance.config.modelSettings).toEqual(runConfig.modelSettings);
  });
  ...
  it('filters tools using isEnabled predicates', async () => {
    const conditionalTool = tool({
      ...
      parameters: z.object({}),
      execute: async () => 'ok',
      isEnabled: ({ runContext }: { runContext: RunContext<unknown>; agent: Agent<any, any> }) =>
    ...
    const disabledTools = await agent.getAllTools(new RunContext({ allowed: false }));
    expect(disabledTools).toEqual([]);

    const enabledTools = await agent.getAllTools(new RunContext({ allowed: true }));
  ...
  it('respects isEnabled option on Agent.asTool', async () => {
    ...
    const disabled = await hostAgent.getAllTools(new RunContext({ enabled: false }));
    expect(disabled).toEqual([]);

    const enabled = await hostAgent.getAllTools(new RunContext({ enabled: true }));
  ...
  it('enables agent tools based on language preference predicates', async () => {
      ...
      tools: [
        ...
        frenchAgent.asTool({
          ...
          isEnabled: ({ runContext }) =>
            ['french_spanish', 'european'].includes(runContext.context.languagePreference),
        }),
        italianAgent.asTool({
          toolName: 'respond_italian',
          toolDescription: 'Respond in Italian.',
          isEnabled: ({ runContext }) => runContext.context.languagePreference === 'european',
        }),
      ],
    });
    ...
    await expect(collect('french_spanish')).resolves.toEqual(['respond_spanish', 'respond_french']);
