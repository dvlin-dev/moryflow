...
import { RunToolApprovalItem as ToolApprovalItem, RunMessageOutputItem } from '../src/items';
import { applyPatchTool, computerTool, shellTool } from '../src/tool';
import * as protocol from '../src/types/protocol';
import { TEST_MODEL_MESSAGE, FakeComputer, FakeShell, FakeEditor } from './stubs';
...
describe('RunState', () => {
  ...
  it('returns history including original input and generated items', () => {
    ...
    state._generatedItems.push(new RunMessageOutputItem(TEST_MODEL_MESSAGE, agent));
  ...
  it('throws error if schema version is missing or invalid', async () => {
    ...
    await expect(() => RunState.fromString(agent, JSON.stringify(jsonVersion))).rejects.toThrow(
  ...
  it('approve updates context approvals correctly', () => {
    ...
    expect(state._context.isToolApproved({ toolName: 'toolX', callId: 'cid123' })).toBe(true);
  });

  it('returns undefined when approval status is unknown', () => {
    const context = new RunContext();
    expect(context.isToolApproved({ toolName: 'unknownTool', callId: 'cid999' })).toBeUndefined();
  });
  ...
  it('reject updates context approvals correctly', () => {
    ...
    expect(state._context.isToolApproved({ toolName: 'toolY', callId: 'cid456' })).toBe(false);
  });
  ...
  it('reject permanently when alwaysReject option is passed', () => {
    ...
    expect(state._context.isToolApproved({ toolName: 'toolZ', callId: 'cid789' })).toBe(false);
  ...
  it('serializes and restores guardrail results', async () => {
    ...
    expect(newState._inputGuardrailResults).toEqual(state._inputGuardrailResults);
...
describe('deserialize helpers', () => {
  ...
  it('deserializeProcessedResponse restores computer actions', async () => {
    ...
    const restored = await RunState.fromString(agent, state.toString());
    expect(restored._lastProcessedResponse?.computerActions[0]?.computer).toBe(tool);
  });
  ...
  it('deserializeProcessedResponse restores apply_patch actions', async () => {
    ...
    const restored = await RunState.fromString(agent, state.toString());
    expect(restored._lastProcessedResponse?.applyPatchActions[0]?.applyPatch).toBe(editorTool);
  });
  ...
  it('fromString tolerates agents gaining MCP servers after serialization', async () => {
    ...
    expect(restored._lastProcessedResponse?.hasToolsOrApprovalsToRun()).toBe(false);
  });
  ...
  it('fromString tolerates serialized traces with new MCP servers', async () => {
    ...
    const state = new RunState(new RunContext(), 'input', agentWithoutMcp, 1);
    state._trace = trace;
    state._currentAgentSpan = createAgentSpan({ data: { name: agentWithoutMcp.name } }, trace);
    state._lastProcessedResponse = {
      newItems: [],
      functions: [],
      handoffs: [],
  ...
  it('deserializeProcessedResponse restores currentStep', async () => {
      ...
      mcpApprovalRequests: [
        {
          requestItem: {
            ...
            name: 'fetch_generic_url_content',
            arguments: '{"url":"https://raw.githubusercontent.com/openai/codex/main/README.md"}',
            toJSON: function (): any {
          ...
          mcpTool: {
            ...
            providerData: {
              ...
              require_approval: {
                ...
                never: {
                  tool_names: ['search_codex_code', 'fetch_codex_documentation'],
                },
              },
            },
          },
    ...
    expect(restored._lastProcessedResponse?.mcpApprovalRequests[0].mcpTool).toEqual(
      state._lastProcessedResponse?.mcpApprovalRequests[0].mcpTool
    );
    expect(
      restored._lastProcessedResponse?.mcpApprovalRequests[0].requestItem.rawItem.providerData
    ).toEqual(
      state._lastProcessedResponse?.mcpApprovalRequests[0].requestItem.rawItem.providerData
    );
  });
});
