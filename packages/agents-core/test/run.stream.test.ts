...
describe('Runner.run (streaming)', () => {
  ...
  it('emits agent_updated_stream_event with new agent on handoff', async () => {
    ...
    const update = events.find(
      (e): e is RunAgentUpdatedStreamEvent => e.type === 'agent_updated_stream_event'
    );
  ...
  it('streams tool_called before the tool finishes executing', async () => {
    ...
    class BlockingStreamModel implements Model {
      ...
      async *getStreamedResponse(_req: ModelRequest): AsyncIterable<StreamEvent> {
        const currentCall = this.#callCount++;
        const response = currentCall === 0 ? toolResponse : finalMessageResponse;
        yield {
          type: 'response_done',
          response: {
            id: `resp-${currentCall}`,
    ...
    const toolCalledIndex = collected.findIndex(
      (event) => event.type === 'run_item_stream_event' && event.name === 'tool_called'
    );
    const toolOutputIndex = collected.findIndex(
      (event) => event.type === 'run_item_stream_event' && event.name === 'tool_output'
    );
  ...
  it('emits run item events in the order items are generated', async () => {
    ...
    class SequencedStreamModel implements Model {
      ...
      async *getStreamedResponse(_req: ModelRequest): AsyncIterable<StreamEvent> {
        const response = this.#turn === 0 ? firstTurnResponse : secondTurnResponse;
        this.#turn += 1;
        yield {
          type: 'response_done',
          response: {
  ...
  describe('server-managed conversation state', () => {
      ...
      async *getStreamedResponse(request: ModelRequest): AsyncIterable<StreamEvent> {
        ...
        yield {
          ...
          response: {
            ...
            output: JSON.parse(JSON.stringify(turn.output)) as protocol.ModelItem[],
          },
        } as StreamEvent;
      }
    }

    const buildTurn = (items: protocol.ModelItem[], responseId?: string): Turn => ({
    ...
    it('only sends new items when using conversationId across turns', async () => {
      const model = new TrackingStreamingModel([
        buildTurn([fakeModelMessage('a_message'), buildToolCall('call-1', 'foo')], 'resp-1'),
        buildTurn([fakeModelMessage('b_message'), buildToolCall('call-2', 'bar')], 'resp-2'),
    ...
    it('keeps server tracker aligned with filtered inputs when streaming', async () => {
      const model = new TrackingStreamingModel([
        buildTurn([fakeModelMessage('call the tool'), buildToolCall('call-1', 'value')], 'resp-1'),
      ...
      expect(
        secondInput.some(
          (item) => item.type === 'message' && getFirstTextContent(item) === 'First input'
        )
      ).toBe(true);
      expect(
        secondInput.some(
    ...
    it('only sends new items and updates previousResponseId across turns', async () => {
      const model = new TrackingStreamingModel([
        buildTurn([fakeModelMessage('a_message'), buildToolCall('call-1', 'foo')], 'resp-789'),
  ...
  it('skips persisting streaming input when an input guardrail triggers', async () => {
    ...
    await expect(result.completed).rejects.toBeInstanceOf(InputGuardrailTripwireTriggered);
...
class ImmediateStreamingModel implements Model {
  ...
  async *getStreamedResponse(_request: ModelRequest): AsyncIterable<StreamEvent> {
    ...
    const output = this.response.output.map((item) => protocol.OutputModelItem.parse(item));
    yield {
      type: 'response_done',
      response: {
        id: 'r',
