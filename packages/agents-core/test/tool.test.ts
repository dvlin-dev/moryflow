...
import { applyPatchTool, computerTool, hostedMcpTool, shellTool, tool } from '../src/tool';
import { z } from 'zod';
...
describe('Tool', () => {
  ...
  it('shellTool needsApproval boolean becomes function', async () => {
    ...
    const approved = await t.needsApproval(new RunContext(), { commands: [] }, 'id');
...
describe('tool.invoke', () => {
  ...
  it('isEnabled boolean becomes function', async () => {
    ...
    const enabled = await t.isEnabled(new RunContext(), new Agent({ name: 'Test Agent' }));
  ...
  it('supports object argument in isEnabled option', async () => {
    ...
    const disabled = await t.isEnabled(new RunContext({ feature: false }), agent);
