...
import { Agent, run, setDefaultModelProvider, setTracingDisabled } from '../src/index';
import { z } from 'zod';
...
describe('Agent + handoffs', () => {
  ...
  it('filters handoffs using isEnabled predicates', async () => {
    ...
    const disabled = await mainAgent.getEnabledHandoffs(new RunContext({ allow: false }));
    expect(disabled).toEqual([]);

    const enabled = await mainAgent.getEnabledHandoffs(new RunContext({ allow: true }));
  ...
  it('enables layered handoffs based on language preference', async () => {
    ...
    await expect(collect('french_spanish')).resolves.toEqual(['spanish_agent', 'french_agent']);
