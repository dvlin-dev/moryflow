...
import { beforeAll, beforeEach, afterEach, describe, expect, expectTypeOf, it, vi } from 'vitest';
import { z } from 'zod';
import {
  Agent,
...
import { Model, ModelProvider, ModelRequest, ModelSettings } from '../src/model';
...
describe('Runner.run', () => {
  ...
  describe('basic', () => {
    ...
    it('sholuld handle structured output', async () => {
      ...
      const result = await runner.run(agent, 'What is the weather in San Francisco?');

      expect(result.finalOutput).toEqual({ city: 'San Francisco' });
      expectTypeOf(result.finalOutput).toEqualTypeOf<{ city: string } | undefined>();
    });
    ...
    it('sets overridePromptModel when agent supplies a prompt and explicit model', async () => {
      class CapturingModel implements Model {
        ...
        async *getStreamedResponse(_request: ModelRequest): AsyncIterable<protocol.StreamEvent> {
          yield* [];
  ...
  describe('additional scenarios', () => {
    ...
    it('resumes from serialized RunState', async () => {
      const agent = new Agent({
        name: 'Resume',
        model: new FakeModel([{ output: [fakeModelMessage('hi')], usage: new Usage() }]),
      });
    ...
    it('output guardrail tripwire throws', async () => {
      ...
      const agent = new Agent({
        name: 'Out',
        model: new FakeModel([{ output: [fakeModelMessage('x')], usage: new Usage() }]),
      });
    ...
    it('switches agents via handoff', async () => {
      const agentB = new Agent({
        name: 'B',
        model: new FakeModel([{ output: [fakeModelMessage('done B')], usage: new Usage() }]),
      });
    ...
    it('records one model response per turn for streaming runs', async () => {
      ...
      class SimpleStreamingModel implements Model {
        ...
        async *getStreamedResponse(req: ModelRequest): AsyncIterable<protocol.StreamEvent> {
    ...
    it('max turn exceeded throws', async () => {
      const agent = new Agent({
        name: 'Max',
        model: new FakeModel([{ output: [fakeModelMessage('nope')], usage: new Usage() }]),
      });
      await expect(run(agent, 'x', { maxTurns: 0 })).rejects.toBeInstanceOf(MaxTurnsExceededError);
    });
    ...
    it('does nothing when no input guardrails are configured', async () => {
      ...
      const agent = new Agent({
        name: 'NoIG',
        model: new FakeModel([{ output: [fakeModelMessage('ok')], usage: new Usage() }]),
      });
    ...
    it('getTurnInput assembles history correctly', () => {
      const msgItem = new MessageOutputItem(TEST_MODEL_MESSAGE, new Agent({ name: 'X' }));
      ...
      const result2 = getTurnInput([{ type: 'message', role: 'user', content: 'a' }], [msgItem]);
    ...
    describe('sessions', () => {
      ...
      it('uses session history and stores run results', async () => {
        ...
        const historyItem = fakeModelMessage('earlier message') as AgentInputItem;
        ...
        const savedAssistant = session.added[0][1] as protocol.AssistantMessageItem;
      ...
      it('allows list inputs when session input callback is provided', async () => {
        ...
        expect(
          recordedInput[0].type === 'message' && 'role' in recordedInput[0] && recordedInput[0].role
        ).toBe('user');
        expect(getFirstTextContent(recordedInput[0])).toBe('Keep this history item');
        expect(
          recordedInput[1].type === 'message' && 'role' in recordedInput[1] && recordedInput[1].role
        ).toBe('user');
      ...
      it('supports async session input callback', async () => {
        ...
        const session = new MemorySession([user('Older message'), user('Newest history')]);
        const runner = new Runner();
      ...
      it('does not duplicate history when callback clones entries', async () => {
        ...
        await runner.run(agent, [user('Fresh input')], {
          ...
          sessionInputCallback: (incomingHistory, newItems) => {
            const clonedHistory = incomingHistory.map((item) => structuredClone(item));
            const clonedNewItems = newItems.map((item) => structuredClone(item));
      ...
      it('persists reordered new items ahead of matching history', async () => {
        ...
        expect(persistedUsers).toHaveLength(1);
        expect(getFirstTextContent(persistedUsers[0])).toBe('Repeatable message');
      });
      ...
      it('throws when session input callback returns invalid data', async () => {
        ...
        await expect(
          runner.run(agent, 'Hello', {
            session,
            sessionInputCallback: () => 'not-an-array' as unknown as AgentInputItem[],
          })
        ).rejects.toThrow('Session input callback must return an array of AgentInputItem objects.');
      });
      ...
      it('stores function tool call and structured output in session', async () => {
        ...
        expect(savedFunctionCall.arguments).toBe(JSON.stringify({ city: 'San Francisco' }));
  ...
  describe('callModelInputFilter', () => {
    ...
    class FilterStreamingModel implements Model {
      ...
      async *getStreamedResponse(request: ModelRequest): AsyncIterable<protocol.StreamEvent> {
        this.lastRequest = request;
        yield {
          type: 'response_done',
          response: {
    ...
    it('modifies model input for non-streaming runs', async () => {
      ...
      expect(model.lastRequest?.systemInstructions).toBe('Base instructions ::filtered');
      ...
      expect(sentInput[0].type === 'message' && 'role' in sentInput[0] && sentInput[0].role).toBe(
        'user'
      );
    ...
    it('supports async filters for streaming runs', async () => {
      ...
      expect(streamingModel.lastRequest?.systemInstructions).toBe('Stream instructions ::stream');
      ...
      expect(
        streamInput[0].type === 'message' && 'role' in streamInput[0] && streamInput[0].role
      ).toBe('user');
    ...
    it('does not duplicate existing session history when filters run', async () => {
      ...
      expect(session.added).toHaveLength(1);
      const [persisted] = session.added;
      const persistedUsers = persisted.filter((item) => 'role' in item && item.role === 'user');
      expect(persistedUsers).toHaveLength(1);
    ...
    it('does not persist raw inputs when filters drop every item', async () => {
      ...
      const userItems = persisted.filter((item) => 'role' in item && item.role === 'user');
    ...
    it('throws when filter returns invalid data', async () => {
      ...
      await expect(runner.run(agent, 'Hello')).rejects.toThrow('ModelInputData');
    });
    ...
    it('prefers per-run callModelInputFilter over runner config', async () => {
      ...
      expect(model.lastRequest?.systemInstructions).toBe('override instructions');
      const sentInput = model.lastRequest?.input as AgentInputItem[];
      expect(Array.isArray(sentInput)).toBe(true);
      expect(sentInput).toHaveLength(1);
    ...
    it('keeps server conversation tracking aligned with filtered inputs', async () => {
      ...
      const runner = new Runner({
        callModelInputFilter: ({ modelData }) => {
          ...
          if (filterCalls === 1) {
            return {
              instructions: modelData.instructions,
              input: modelData.input.slice(1).map((item) => structuredClone(item)),
            };
          }
          return modelData;
        },
      ...
      const result = await runner.run(agent, [user('First input'), user('Second input')], {
      ...
      const secondMessages = secondInput.filter((item) => item.type === 'message');
    ...
    it('preserves providerData when saving streaming session items', async () => {
      ...
      expect(getFirstTextContent(savedAssistant)).toBe('assistant with metadata');
    });
  });
  ...
  describe('gpt-5 default model adjustments', () => {
    ...
    it('strips GPT-5-only settings when the runner model is not a GPT-5 string', async () => {
      ...
      expect((requestSettings.providerData as any)?.reasoning_effort).toBeUndefined();
    ...
    it('keeps GPT-5-only settings when the agent relies on the default model', async () => {
      ...
      expect((requestSettings.providerData as any)?.reasoning_effort).toBe('medium');
  ...
  describe('server-managed conversation state', () => {
    ...
    const buildResponse = (items: protocol.ModelItem[], responseId?: string): ModelResponse => ({
      output: JSON.parse(JSON.stringify(items)) as protocol.ModelItem[],
      usage: new Usage(),
      responseId,
    });

    const buildToolCall = (callId: string, arg: string): protocol.FunctionCallItem => ({
      id: callId,
      type: 'function_call',
      name: 'test',
      callId,
    ...
    it('preserves user input when the session callback only reuses history with conversationId', async () => {
      ...
      const persistedHistory: AgentInputItem[] = [assistant('Persisted reply from history')];
    ...
    it('only sends new items when using conversationId across turns', async () => {
      const model = new TrackingModel([
        buildResponse([fakeModelMessage('a_message'), buildToolCall('call-1', 'foo')], 'resp-1'),
        buildResponse([fakeModelMessage('b_message'), buildToolCall('call-2', 'bar')], 'resp-2'),
    ...
    it('only sends new items and updates previousResponseId across turns', async () => {
      const model = new TrackingModel([
        buildResponse([fakeModelMessage('a_message'), buildToolCall('call-1', 'foo')], 'resp-789'),
    ...
    it('sends newly appended generated items when resuming', async () => {
      ...
      const extraMessage = new MessageOutputItem(fakeModelMessage('cached note'), agent);
      ...
      expect(secondItems[0]).toMatchObject({
        type: 'message',
        content: expect.arrayContaining([expect.objectContaining({ text: 'cached note' })]),
      });
    ...
    it('sends only approved items when mixing function and MCP approvals', async () => {
      ...
      const model = new TrackingModel([
        buildResponse([mcpApprovalCall, buildToolCall('call-mixed', 'foo')], 'resp-mixed-1'),
      ...
      expect(secondResult.state._currentStep?.type).toBe('next_step_interruption');
    });
