...
export class SandboxManager {
  ...
  async execute(
    ...
    try {
      ...
      if (!filterResult.allowed) {
        ...
        throw new CommandNotAllowedError(filterResult.reason ?? 'Command not allowed');
      }
      ...
      if (filterResult.requiresConfirmation) {
        ...
        if (!onCommandConfirm) {
          throw new CommandNotAllowedError(filterResult.reason ?? 'Command requires confirmation');
        }
  ...
  addAuthorizedPath(path: string): void {
    this.pathAuth.addPersistent(path);
  }
