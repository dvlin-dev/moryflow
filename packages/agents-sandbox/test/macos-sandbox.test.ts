...
describe('MacOSSandbox', () => {
  ...
  const createConfig = (mode: 'ask' | 'full_access' = 'ask'): SandboxConfig => ({
  ...
  describe('Seatbelt profile 内容', () => {
    it('ask 模式应该包含敏感路径保护', async () => {
      const config = createConfig('ask');
      const wrapped = await sandbox.wrapCommand('ls', config);
    ...
    it('full_access 模式不应使用 seatbelt 包装', async () => {
      const config = createConfig('full_access');
      const wrapped = await sandbox.wrapCommand('ls', config);
  ...
  describe('profile 结构', () => {
    ...
    it('应该导入 system.sb 基线策略，避免 deny default 导致命令空输出失败', async () => {
      const config = createConfig();
      const wrapped = await sandbox.wrapCommand('pwd', config);

      expect(wrapped).toContain('(import "system.sb")');
    });
