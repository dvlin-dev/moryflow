...
- 业务侧**禁止**深路径 import（例如 `@moryflow/tiptap/editors/...`、`@moryflow/tiptap/nodes/...`），统一从 `@moryflow/tiptap` 根入口导入。
- 样式统一从 `@moryflow/tiptap/styles/notion-editor.scss` 引入（内部会聚合节点样式），避免业务侧逐个引入节点 scss。
...
- `@moryflow/tiptap/styles/notion-editor.scss`
